...
return new class extends Migration
{
    public function up()
    {
        ...
        Schema::create('instrumen_prodi_submissions', function (Blueprint $table) {
            ...
            $table->foreignId('pengajuan_ami_id')->nullable()->constrained('pengajuan_amis')->onDelete('cascade');
            $table->foreignId('indikator_prodi_id')->constrained('indikator_prodis')->onDelete('cascade');
            $table->decimal('nilai', 5, 2);
            $table->text('deskripsi');
            $table->timestamps();

            // Using a shorter name for the unique index
            $table->unique(['indikator_prodi_id', 'auditee_id'], 'ips_instrumen_prodi_unique');
        });
    }
