...
class DokumenAmiSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach ($files as $filename) {
            $categoriesToSave = ['umum']; // default umum selalu

            if (stripos($filename, 'Auditor') !== false) {
                $categoriesToSave[] = 'auditor';
            } elseif (stripos($filename, 'Auditee') !== false) {
                $categoriesToSave[] = 'auditee';
            }
            // kalau ada 'Asesor' gak tambah kategori selain umum

            foreach ($categoriesToSave as $kategori) {
                ...
                if (Storage::disk('public')->exists($path)) {
                    ...
                    DokumenAmi::create([
                        ...
                        'kategori_dokumen' => ucfirst($kategori),
                        'deskripsi_dokumen' => '-',
                        'file_dokumen' => $path,
