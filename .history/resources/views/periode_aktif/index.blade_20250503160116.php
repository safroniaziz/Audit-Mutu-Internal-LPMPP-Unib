                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_unker_table">
                        ...
                        <tbody class="text-gray-600 fw-semibold">
                            @forelse ($periodeAktifs as $index => $periodeAktif)
                                ...
                                <tr>
                                    <td colspan="6" style="padding: 0; border-bottom: 1px dashed #ccc; padding-bottom: 0.75rem;">
                                        <div style="background-color: #ffecec; border-top: 2px dashed #dee2e6; padding: 1rem 1.25rem;">
                                            <div class="row">
                                                <div class="col-md-4 mb-2">
                                                   ...
                                                   <button type="button" class="btn btn-primary btn-sm rounded-pill px-3 py-2 mt-2"
                                               ...
                                               @else
                                                   ...
                                                   <button type="button" class="btn btn-success btn-sm rounded-pill px-3 py-2 mt-2"
                                                       data-bs-toggle="modal"
                                                       data-bs-target="#aturJadwalModal"
                                                       data-jadwal="login"
                                                       data-periode-id="{{ $periodeAktif->id }}">
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#aturJadwalModal').on('show.bs.modal', function (event) {
                ...
                // Fetch existing jadwal data if available
                $.ajax({
                    url: "/periode_aktif/get-jadwal-data",
                    type: "GET",
                    data: {
                    ...
                    success: function(response) {
                        // Initialize daterangepicker with existing values if available
                        if (response.status === 'success' && response.data) {
                            // Format dates for daterangepicker
                            var startDate = moment(response.data.waktu_mulai).format('YYYY-MM-DD');
                            var endDate = moment(response.data.waktu_selesai).format('YYYY-MM-DD');
                            ...
                            // Initialize empty daterangepicker if no data
                            $('#dateRange').daterangepicker({
                                locale: {
                                    format: 'YYYY-MM-DD'
                                },
                    ...
                    error: function() {
                        // If error, initialize empty daterangepicker
                        $('#dateRange').daterangepicker({
                            locale: {
                                format: 'YYYY-MM-DD'
                            },
