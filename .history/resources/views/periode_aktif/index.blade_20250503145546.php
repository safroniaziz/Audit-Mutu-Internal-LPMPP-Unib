...
@section('content')
    ...
    Atur
                                                    ...
                                                    <button class="btn btn-sm btn-outline-primary mt-1" data-bs-toggle="modal" data-bs-target="#aturJadwalModal" data-jadwal="login">Atur</button>
                                                </div>
                                                <div class="col-md-4 mb-2">
                                                    <div class="fw-bold">↳ Jadwal Audit Oleh Auditor:</div>
                                                    <div class="text-muted">Belum diatur</div>
                                                    <button class="btn btn-sm btn-outline-primary mt-1" data-bs-toggle="modal" data-bs-target="#aturJadwalModal" data-jadwal="audit">Atur</button>
                                                </div>
                                                <div class="col-md-4 mb-2">
                                                    <div class="fw-bold">↳ Jadwal Pengisian Data Oleh Auditee:</div>
                                                    <div class="text-muted">Belum diatur</div>
                                                    <button class="btn btn-sm btn-outline-primary mt-1" data-bs-toggle="modal" data-bs-target="#aturJadwalModal" data-jadwal="data">Atur</button>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                ...
                <div class="modal fade" id="aturJadwalModal" tabindex="-1" aria-labelledby="aturJadwalModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <form method="POST" action="{{ route('aturJadwal.store') }}">
                            @csrf
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="aturJadwalModalLabel">Atur Jadwal</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <div id="modalBodyContent">
                                        <!-- Konten modal akan diubah berdasarkan tombol yang dipilih -->
                                    </div>
                                </div>
...
@push('scripts')
    ...
    <script>
        ...
        const modal = document.getElementById('aturJadwalModal');
        modal.addEventListener('show.bs.modal', function (event) {
            ...
            if (jadwal === 'login') {
                content = `
                    <label for="jadwal_login_start">Tanggal & Waktu Mulai Login Auditee & Auditor</label>
                    <input type="datetime-local" name="jadwal_login_start" class="form-control" required>
                    <label for="jadwal_login_end">Tanggal & Waktu Selesai Login Auditee & Auditor</label>
                    <input type="datetime-local" name="jadwal_login_end" class="form-control" required>
                `;
            } else if (jadwal === 'audit') {
                content = `
                    <label for="jadwal_audit_start">Tanggal & Waktu Mulai Audit Oleh Auditor</label>
                    <input type="datetime-local" name="jadwal_audit_start" class="form-control" required>
                    <label for="jadwal_audit_end">Tanggal & Waktu Selesai Audit Oleh Auditor</label>
                    <input type="datetime-local" name="jadwal_audit_end" class="form-control" required>
                `;
            } else if (jadwal === 'data') {
                content = `
                    <label for="jadwal_data_start">Tanggal & Waktu Mulai Pengisian Data Oleh Auditee</label>
                    <input type="datetime-local" name="jadwal_data_start" class="form-control" required>
                    <label for="jadwal_data_end">Tanggal & Waktu Selesai Pengisian Data Oleh Auditee</label>
                    <input type="datetime-local" name="jadwal_data_end" class="form-control" required>
                `;
            }
