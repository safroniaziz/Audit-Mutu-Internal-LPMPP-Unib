...
@push('name')
    <style>
        .ubah-password-btn:hover {
            background-color: #007bff; /* Ganti warna hover sesuai kebutuhan */
                ...
                <div class="card-header border-0 pt-6">
                    ...
                    <div class="card-toolbar">
                        <div class="d-flex justify-content-end" data-kt-administrator-table-toolbar="base">
                            <button type="button" id="addAdministratorButton" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#kt_modal" style="padding-top: .8rem; padding-bottom: .8rem;">
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_administrator_table">
                        <thead>
                            <tr class="text-start text-dark fw-bolder fs-7 text-uppercase gs-0 bg-light-primary">
                                ...
                                <th class=50pxps-3">No</th>
                                <th class=125px>Nama Lengkap</th>
                                <th class=175px>Email</th>
                                <th class=100px>Terdaftar Sejak</th>
                                <th class=100pxtext-center">Ubah Password</th>
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form id="ubahPasswordForm">
                                    <input type="hidden" name="administrator_id" id="passwordAdministratorId">
                                    <div class="mb-3">
...
@push('scripts')
    ...
    <script>
        ...
        $(document).on('click', '.restore-data', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('administrator.restore', ':id') }}".replace(':id', id),
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            _method: 'PUT'
        ...
        $(document).on('click', '.delete-permanent', function () {
            var id = $(this).data('id');
            var url = "{{ route('administrator.hapus_permanen', ':id') }}";
        ...
        $(document).on('click', '.ubah-password-btn', function () {
            ...
            $('#passwordAdministratorId').val(id);
            $('#newPassword').val('');
            $('#confirmPassword').val('');
        ...
        $('#ubahPasswordForm').submit(function (e) {
            ...
            button.text('Sedang Memproses...');  // Ubah teks tombol jadi "Sedang Memproses..."

            const id = $('#passwordAdministratorId').val();
            const formData = {
            ...
            $.ajax({
                url: `/administrator/${id}/ubah-password`,
                method: 'POST',
                data: formData,
                success: function (response) {
                    Swal.fire({
                ...
                complete: function () {
                    ...
                    button.text(originalButtonText);  // Kembalikan teks tombol ke semula
                }
            });
        });
