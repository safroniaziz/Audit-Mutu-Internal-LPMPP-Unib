                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if ($administrator->deleted_at)
                                            <!-- Tidak Aktif (Label Merah) -->
                                            <span class="badge badge-danger">
                                                <i class="ki-duotone ki-cross fs-5" style="color: white;"></i> Tidak Aktif
                                            </span>
                                        @else
                                            <!-- Aktif (Label Hijau) -->
                                            <span class="badge badge-success">
                                                <i class="ki-duotone ki-check fs-5" style="color: white;"></i> Aktif
                                            </span>
                                        @endif
                                    </td>
                                    <td class="text-center">
                                        <button type="button" class="btn btn-sm btn-light-primary editAdministratorButton"
                                        ...
                                        @if ($administrator->deleted_at)
                                            <!-- Aktifkan (Label Hijau) -->
                                            <button type="button" class="btn btn-sm btn-light-success restore-data" data-id="{{ $administrator->id }}">
                                                <i class="ki-duotone ki-refresh fs-5"></i> Aktifkan
                                            </button>
                                        @else
                                            <!-- Nonaktifkan (Label Merah) -->
                                            <button type="button" class="btn btn-sm btn-light-danger delete-data" data-id="{{ $administrator->id }}">
                                                <i class="ki-duotone ki-trash fs-5"></i> Nonaktifkan
                                            </button>
                                        @endif
                                    </td>
                                </tr>
...
@push('scripts')
        ...
        $(document).on('click', '.restore-data', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('administrator.restore', '') }}/" + id,  // Pastikan route restore sudah diset
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            _method: 'PUT'  // Menggunakan PUT untuk restore
                        },
    ...
    $('#methodField').val('PUT').trigger('change');  // Method jadi PUT
    $('#kt_modal_form').attr('action', `/administrator/${id}`);  // Atur URL form

    console.log('ID yang dipilih: ' + id);  // Cek ID yang dikirim

    $.ajax({
        url: `/administrator/${id}/edit`,  // URL untuk ambil data
        type: 'GET',
        success: function(response) {
        ...
        error: function(xhr, status, error) {
            console.log('Error AJAX: ', error);  // Log error kalau ada masalah
            Swal.fire({
                title: 'Gagal!',
