    ...
    <script>
        ...
        function updateMainTable(unitKerjaId) {
            ...
            $(document).on('click', '.detail-rsbProdi', function() {
                ...
                // Reset form
                $('#form-tambah-ind')[0].reset();
                $('#unit_kerja_id').val(unitKerjaId);

                // Tampilkan loading
                $('#ind-list').html('<tr><td colspan="6" class="text-center">Memuat data...</td></tr>');
                $('#ind_ikss_id').html('<option value="">Memuat data...</option>');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        // Buat array ID ind yang sudah terpasang
                        const indTerpasang = response.instrumen_terpasang || [];
