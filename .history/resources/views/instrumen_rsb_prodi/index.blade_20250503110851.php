...
@push('scripts')
    ...
    <script>
        ...
        $('.detail-rsbProdi').click(function () {
            const id = $(this).data('id');
            const url = $(this).data('url');

            $.get(url, function (response) {
                let isi = '<table class="table table-bordered"><thead><tr><th>No</th><th>Instrumen</th><th>Aksi</th></tr></thead><tbody>';
                response.semua_instrumen.forEach((item, index) => {
                    const sudahAda = response.instrumen_terpasang.some(i => i.id === item.id);
                    isi += `<tr>
                        <td>${index + 1}</td>
                        <td>${item.nama}</td>
                        <td>
                            <button class="btn btn-sm ${sudahAda ? 'btn-danger hapusInstrumen' : 'btn-success tambahInstrumen'}"
                                data-unit="${response.prodi.id}"
                                data-id="${item.id}">
                                ${sudahAda ? 'Hapus' : 'Tambah'}
                            </button>
                        </td>
                    </tr>`;
                });
                isi += '</tbody></table>';
                $('#isiModalInstrumenRsb').html(isi);
            });
        });

        $(document).on('click', '.tambahInstrumen', function () {
    const unit = $(this).data('unit');
    const id = $(this).data('id');

    $.post("{{ route('rsbProdi.tambahInstrumen') }}", {
        _token: '{{ csrf_token() }}',
        unit_kerja_id: unit,
        instrumen_ikss_id: id
    }, function () {
        $('.detail-rsbProdi[data-id="' + unit + '"]').click(); // reload modal
    });
});

$(document).on('click', '.hapusInstrumen', function () {
    const unit = $(this).data('unit');
    const id = $(this).data('id');
