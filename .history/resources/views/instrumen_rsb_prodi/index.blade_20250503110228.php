    ...
    <div id="kt_app_content" class="app-content flex-column-fluid">
        <div id="kt_app_content_container" class="app-container container-xxl">
            <div class="card">
                                        ...
                                        <td class="text-center">
                                            <div class="button-container">
                                                <button type="button"
                                                    ...
                                                    data-id="{{ $prodi->id }}"
                                                    data-url="{{ route('rsbProdi.tambah', $prodi->id) }}"
                ...
                <div class="modal fade" id="modelTambahInstrumenRsb" tabindex="-1" aria-labelledby="modelTambahInstrumenRsb" aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="modelTambahInstrumenRsb">Detail Instrumen</h5>
...
@push('scripts')
    ...
    <script>
        ...
        $(document).on('click', '.detail-rsbProdi', function () {
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    $('#modelTambahInstrumenRsb').modal('show');
                },
                error: function () {
        ...
        $(document).on('click', '.restore-data', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('rsbProdi.restore', ':id') }}".replace(':id', id),
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            _method: 'PUT'
        ...
        $('.edit-rsbProdi').click(function() {
            let id = $(this).data('id');
            let url = $(this).data('url');

            $('#kt_modal form')[0].reset();
            $('#methodField').val('PUT');
            $('#kt_modal_form').attr('action', "{{ route('rsbProdi.update', '') }}/" + id);
        ...
        $('.add-rsbProdi').click(function() {
            $('#kt_modal form')[0].reset();
            $('#methodField').val('POST');
            $('#kt_modal_form').attr('action', "{{ route('rsbProdi.store') }}");
        ...
        $('button[data-bs-target="#kt_modal"]:not(.edit-rsbProdi)').click(function() {
            $('#kt_modal form')[0].reset();
            $('#methodField').val('POST');
            $('#kt_modal_form').attr('action', "{{ route('rsbProdi.store') }}");
            $('#kt_modal .modal-title').text('Tambah Instrumen Rsb Prodi');
        ...
        // Script untuk delete
        $(document).on('click', '.nonaktifkan-rsbProdi', function() {
            let id = $(this).data('id');
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('rsbProdi.nonaktifkan', '') }}/" + id,
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            _method: 'DELETE'
        ...
        $(document).on('click', '[data-kt-rsbProdi-table-select="delete_selected"]', function() {
            const checkboxes = document.querySelectorAll('#kt_rsbProdi_table tbody input[type="checkbox"]:checked');
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('rsbProdi.nonaktifkanSelected') }}",
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            ids: ids
        ...
        $(document).on('click', '.delete-permanent', function () {
            var id = $(this).data('id');
            var url = "{{ route('rsbProdi.hapus_permanen', ':id') }}";
