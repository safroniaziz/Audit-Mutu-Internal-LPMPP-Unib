                                        ...
                                        <div class="col-md-12">
                                            <select class="form-select" id="satuan_standar_id" name="satuan_standar_id" required>
                                                ...
                                                @foreach ($satuanStandars as $satuan)
                                                    <option value="{{ $satuan->id }}">{{ $satuan->kode }} - {{ $satuan->sasaran }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
...
@push('scripts')
    ...
    <script>
        ...
        $('#satuan_standar_id').on('change', function () {
            ...
            if (satuanId) {
                $.ajax({
                    url: `/get-indikator-by-satuan/${satuanId}`,
                    type: 'GET',
                    success: function (data) {
                        data.forEach(function (item) {
                            $indikatorSelect.append(`<option value="${item.id}">${item.nama}</option>`);
                        });
                    },
                    error: function () {
        ...
        $('#indikator_ikss_id').on('change', function () {
            ...
            if (indikatorId) {
                $.ajax({
                    url: `/get-instrumen-by-indikator/${indikatorId}`,
                    type: 'GET',
                    success: function (data) {
                        data.forEach(function (item) {
                            $instrumenSelect.append(`<option value="${item.id}">${item.nama}</option>`);
                        });
                    },
                    error: function () {
