...
@push('scripts')
    ...
    <script>
                                ...
                                <div class="card-body py-3">
                                    ...
                                    <div class="d-flex flex-column">
                                        <div class="d-flex align-items-center mb-2">
                                            ...
                                            ${auditors.length > 0 ? `<span class="badge badge-light-primary fs-8 ms-auto">${auditors.length} Auditor</span>` : ''}
                                        </div>

                                        ${auditors.length > 0 ?
                                            auditors.map((auditor, auditorIndex) => {
                                                // Add null check for auditor.auditor
                                                const auditorData = auditor.auditor || {};
                                                const auditorName = auditorData.name || 'N/A';
                                                const auditorRole = auditor.role || 'unknown';
                                                ...
                                                return `
                                                    <div class="d-flex align-items-center mb-2">
                                                        ...
                                                        <div class="d-flex flex-column flex-grow-1">
                                                            <span class="text-dark fw-bold fs-7">${auditorName}</span>
                                                            <span class="text-muted fw-semibold fs-8">${auditorRole.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}</span>
                                                        </div>
                                                        <span class="badge badge-light-${auditorRole === 'ketua' ? 'danger' : 'info'} fs-8">
                                                            ${auditorRole === 'ketua' ? 'Ketua' : 'Anggota'}
                                                        </span>
                                                    </div>
                                                `;
                                            }).join('') :
        ...
        function loadExistingAssignments(penugasanId) {
            console.log('Loading existing assignments for ID:', penugasanId);
            ...
            fetch(`/penugasan-auditor/get-existing-assignments/${penugasanId}`)
                ...
                .then(data => {
                    ...
                    if (data.success && data.assignments) {
                        ...
                        document.getElementById('waktu_visitasi').value = '';

                        console.log('Assignments object:', data.assignments);

                        // Check if assignments is an array
                        if (Array.isArray(data.assignments)) {
                            console.log('Assignments is array, processing...');
                            ...
                            data.assignments.forEach((assignment, index) => {
                                ...
                                if (selectElement) {
                                    ...
                                    console.log(`Set ${selectId} to:`, assignment.auditor_id);
                                }
                            });
                        } else if (typeof data.assignments === 'object') {
                            console.log('Assignments is object, processing...');
                            ...
                            if (data.assignments.ketua) {
                                ...
                                if (auditor1Select) {
                                    ...
                                    console.log('Set auditor1 (ketua) to:', data.assignments.ketua.auditor_id);
                                }
                            }
                            if (data.assignments.anggota) {
                                ...
                                if (auditor2Select) {
                                    ...
                                    console.log('Set auditor2 (anggota) to:', data.assignments.anggota.auditor_id);
                                }
                            }
                            if (data.assignments.anggota_kedua) {
                                ...
                                if (auditor3Select) {
                                    ...
                                    console.log('Set auditor3 (anggota_kedua) to:', data.assignments.anggota_kedua.auditor_id);
                                }
                            }
                            ...
                            if (data.assignments.waktu_visitasi) {
                                ...
                                if (waktuVisitasiInput) {
                                    ...
                                    const formattedDate = visitasiDate.toISOString().slice(0, 16);
                                    waktuVisitasiInput.value = formattedDate;
                                    console.log('Set waktu_visitasi to:', formattedDate);
                                }
                            }
                        }
                    } else {
                        console.error('No success or assignments data:', data);
                    }
                })
                .catch(error => {
