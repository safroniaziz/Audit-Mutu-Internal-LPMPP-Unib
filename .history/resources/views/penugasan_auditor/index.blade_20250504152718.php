...
@push('scripts')
    ...
    <script>
        ...
        function loadAuditors() {
            fetch('/api/get-auditors')
                .then(response => response.json())
                .then(data => {
                    if (Array.isArray(data)) {
                        const datalistAuditor1 = document.getElementById('datalistAuditor1');
                        const datalistAuditor2 = document.getElementById('datalistAuditor2');
                        const datalistAuditor3 = document.getElementById('datalistAuditor3');

                        // Clear existing options
                        datalistAuditor1.innerHTML = '';
                        datalistAuditor2.innerHTML = '';
                        datalistAuditor3.innerHTML = '';

                        // Add options to datalists
                        data.forEach(auditor => {
                            const option1 = document.createElement('option');
                            option1.value = auditor.id;
                            option1.label = auditor.name;
                            datalistAuditor1.appendChild(option1);

                            const option2 = document.createElement('option');
                            option2.value = auditor.id;
                            option2.label = auditor.name;
                            datalistAuditor2.appendChild(option2);

                            const option3 = document.createElement('option');
                            option3.value = auditor.id;
                            option3.label = auditor.name;
                            datalistAuditor3.appendChild(option3);
                        });
                    }
                })
                .catch(error => {
