    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function renderCards(penugasanAuditors) {
                ...
                // Check audit activities for all penugasan auditors with cache busting
                const auditActivityChecks = penugasanAuditors.map(penugasanAuditor =>
                    fetch(`/penugasan-auditor/check-audit-activities/${penugasanAuditor.id}?_t=${Date.now()}`)
                        .then(response => response.json())
                        .then(data => ({
                            id: penugasanAuditor.id,
                            hasAuditActivities: data.success ? data.has_audit_activities : false,
                            activities: data.success ? data.activities : {}
                        }))
                        .catch(error => {
                            console.error('Error checking audit activities for ID:', penugasanAuditor.id, error);
                            return {
                                id: penugasanAuditor.id,
                                hasAuditActivities: false,
                                activities: {}
                            };
                        })
                );

                Promise.all(auditActivityChecks).then(results => {
                    const auditStatusMap = {};
                    results.forEach(result => {
                        auditStatusMap[result.id] = result.hasAuditActivities;
                        console.log(`Pengajuan ID ${result.id}: hasAuditActivities = ${result.hasAuditActivities}`, result.activities);
                    });

                    console.log('Final audit status map:', auditStatusMap);
                    console.log('Penugasan auditors:', penugasanAuditors);

                    renderCardsContent(penugasanAuditors, auditStatusMap);
                });
            }

            // Function to render cards content
            function renderCardsContent(penugasanAuditors, auditStatusMap) {
                console.log('Rendering cards with audit status map:', auditStatusMap);
                        ...
                        <div class="col-xl-4 col-lg-6 col-md-6 penugasan-card">
                            <div class="card h-100 shadow-sm hover-elevate-up">
                                ...
                                <div class="card-body py-3">
                                    ...
                                    ${auditStatusMap[penugasanAuditor.id] ? `
                                        <div class="alert alert-warning d-flex align-items-center p-3 mb-3" style="font-size: 0.75rem;">
                                            <i class="fas fa-exclamation-triangle text-warning me-2"></i>
                                            <div>
                                                <strong>Audit Sedang Berlangsung</strong><br>
                                                <small class="text-muted">Tim auditor sudah mulai melakukan aktivitas audit. Penugasan tidak dapat diubah untuk menjaga integritas proses audit.</small>
                                            </div>
                                        </div>
                                    ` : ''}
                                </div>
                                    ...
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div class="d-flex align-items-center gap-2">
                                            ...
                                            ` : ''}
                                                                                ...
                                                                                <div class="d-flex gap-2">
                                            ${(() => {
                                                const hasAuditActivities = auditStatusMap[penugasanAuditor.id];
                                                console.log(`Pengajuan ${penugasanAuditor.id}: hasAuditActivities = ${hasAuditActivities}`);
                                                return hasAuditActivities ? `
                                                    <div class="flex-grow-1 position-relative">
                                                        <button type="button" class="btn btn-sm btn-secondary w-100" disabled
                                                                style="cursor: not-allowed; opacity: 0.6;"
                                                                data-bs-toggle="tooltip" data-bs-placement="top"
                                                                title="Tidak dapat diedit karena tim auditor sudah mulai melakukan aktivitas audit (desk evaluation, visitasi, atau penilaian instrumen prodi). Perubahan penugasan dapat mempengaruhi jadwal audit yang sedang berlangsung.">
                                                            <i class="fas fa-lock me-1"></i>
                                                            ${auditors.length > 0 ? 'Tidak Dapat Edit' : 'Tidak Dapat Assign'}
                                                        </button>
                                                        <div class="position-absolute top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center"
                                                             style="pointer-events: none; z-index: 1;">
                                                            <span class="badge badge-light-danger fs-7 px-2 py-1">
                                                                <i class="fas fa-exclamation-triangle me-1"></i>
                                                                Audit Aktif
                                                            </span>
                                                        </div>
