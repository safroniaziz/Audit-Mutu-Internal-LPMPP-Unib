                    ...
                    <div class="col-xl-4 col-lg-6 col-md-6 penugasan-card">
                        <div class="card h-100 shadow-sm hover-elevate-up">
                            ...
                            <div class="card-header border-0 pt-4 pb-2">
                                ...
                                <!-- Status Badge - Below auditee name -->
                                <div class="mt-3">
                                    <span class="badge badge-light-info fs-8 shadow-sm px-3 py-2" style="background-color: white; border: 1px solid #e4e6ea;" id="audit-status-{{ $penugasanAuditor->id }}">
                            ...
                            <div class="card-body py-3">
                                ...
                                <div class="d-flex flex-column">
                                    ...
                                    @php
                                        $validAuditors = $penugasanAuditor->auditors->filter(function($auditor) {
                                        ...
                                        @foreach ($validAuditors as $auditor)
                                            <div class="d-flex align-items-center mb-2">
                                                ...
                                                <div class="d-flex flex-column flex-grow-1">
                                                    <span class="text-gray-800 text-hover-primary mb-1">{{ $auditor->auditor->name }}</span>
...
@push('scripts')
    ...
    <script>
                                ...
                                <div class="card-body py-3">
                                    ...
                                    <div class="row g-3 mb-4">
                                        ...
                                        <div class="col-6">
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div class="d-flex flex-column">
                                                    <span class="text-muted fw-semibold fs-8">Nomor Surat</span>
                                                    <span class="fw-bold fs-7">${periodeAktif.nomor_surat || 'N/A'}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="d-flex flex-column">
                                        <div class="d-flex align-items-center mb-2">
                                            ...
                                            ${auditors.length > 0 ? `
                                                <span class="badge badge-light-primary fs-8 ms-auto">${auditors.length} Auditor</span>
                                            ` : ''}
                                        </div>
                                        ...
                                        ${auditors.length > 0 ?
                                            auditors.map(auditor => `
                                                <div class="d-flex align-items-center mb-2">
                                                    ...
                                                    <div class="d-flex flex-column flex-grow-1">
                                                        <span class="text-dark fw-bold fs-7">${auditor.auditor?.name || 'N/A'}</span>
                                                        <span class="text-muted fw-semibold fs-8">${auditor.role ? auditor.role.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) : 'N/A'}</span>
                                                    </div>
                                                    <span class="badge badge-light-${auditor.role === 'ketua' ? 'danger' : 'info'} fs-8">
                                                        ${auditor.role === 'ketua' ? 'Ketua' : 'Anggota'}
                                                    </span>
                                ...
                                <div class="card-footer pt-0">
                                    ...
                                    <div class="d-flex gap-2">
                                        ...
                                        <button type="button" class="btn btn-sm btn-warning"
                                                onclick="resetPenugasanAuditor('${penugasanAuditor.id}')"
                                                id="reset-btn-${penugasanAuditor.id}"
            ...
            function updateAuditStatus(pengajuanId) {
                ...
                checkAuditActivities(pengajuanId).then(auditorActivities => {
                    if (auditorActivities) {
                        ...
                        if (activeAuditors === 0) {
                            statusBadge.className = 'badge badge-light-success fs-8 shadow-sm px-3 py-2';
                            statusBadge.style = 'background-color: white; border: 1px solid #e4e6ea;';
                            statusBadge.innerHTML = '<i class="fas fa-edit me-1"></i>Dapat Diedit';
                        } else if (activeAuditors === totalAuditors) {
                            statusBadge.className = 'badge badge-light-warning fs-8 shadow-sm px-3 py-2';
                            statusBadge.style = 'background-color: white; border: 1px solid #e4e6ea;';
                            statusBadge.innerHTML = '<i class="fas fa-lock me-1"></i>Semua Auditor Terkunci';
                        } else {
                            statusBadge.className = 'badge badge-light-info fs-8 shadow-sm px-3 py-2';
                            statusBadge.style = 'background-color: white; border: 1px solid #e4e6ea;';
                    ...
                    statusBadge.className = 'badge badge-light-danger fs-8 shadow-sm px-3 py-2';
                    statusBadge.style = 'background-color: white; border: 1px solid #e4e6ea;';
        ...
        function applySelectiveFormRestrictions(auditorActivities) {
            ...
            if (auditorActivities.pendamping && auditorActivities.pendamping.has_activities) {
                ...
                if (auditor2Select) {
                    ...
                    hasRestrictedAuditors = true;
                    restrictedAuditorNames.push(auditorActivities.pendamping.auditor_name);
                }
            }

            if (auditorActivities.pendamping_kedua && auditorActivities.pendamping_kedua.has_activities) {
                ...
                if (auditor3Select) {
                    ...
                    hasRestrictedAuditors = true;
                    restrictedAuditorNames.push(auditorActivities.pendamping_kedua.auditor_name);
                }
            }
            ...
            // Enable submit button - always allow saving since waktu visitasi can always be changed
            if (submitBtn) {
            ...
            warning.innerHTML = `
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <p class="mb-0 mt-2"><small class="text-muted">Note: Waktu visitasi tetap dapat diubah dan disimpan.</small></p>
                    </div>
                </div>
            `;
                ...
                .then(data => {
                    ...
                    if (data.success && data.assignments) {
                        ...
                        if (Array.isArray(data.assignments)) {
                            ...
                            if (data.assignments.pendamping) {
                                const auditor2Select = document.getElementById('auditor2');
                                if (auditor2Select) {
                                    auditor2Select.value = data.assignments.pendamping.auditor_id;
                                }
                            }
                            if (data.assignments.pendamping_kedua) {
                                const auditor3Select = document.getElementById('auditor3');
                                if (auditor3Select) {
                                    auditor3Select.value = data.assignments.pendamping_kedua.auditor_id;
                                }
                            }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (submitBtn) {
                submitBtn.addEventListener('click', function() {
                    ...
                    // Always use save-penugasan-auditor (it handles both create and update)
                    const url = '/penugasan-auditor/save-penugasan-auditor';
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            ...
                            // Determine if this is an update or create based on button text
