    ...
    <script>
        ...
        function loadExistingAssignments(penugasanId) {
            fetch(`/penugasan-auditor/get-existing-assignments/${penugasanId}`)
                .then(response => response.json())
                .then(data => {
                    // Add small delay to ensure select elements are loaded
                    setTimeout(() => {
                    if (data.success && data.assignments) {
                        ...
                        if (Array.isArray(data.assignments)) {
                            ...
                            console.log('Setting assignments:', data.assignments);
                            ...
                            if (data.assignments.ketua) {
                                ...
                                if (auditor1Select) {
                                    ...
                                    console.log('Set ketua (auditor1):', data.assignments.ketua.auditor_id);
                                }
                            }
                            if (data.assignments.anggota) {
                                const auditor2Select = document.getElementById('auditor2');
                                console.log('Found auditor2 select:', auditor2Select);
                                if (auditor2Select) {
                                    auditor2Select.value = data.assignments.anggota.auditor_id;
                                    console.log('Set anggota (auditor2):', data.assignments.anggota.auditor_id);
                                } else {
                                    console.log('auditor2 select element not found');
                                }
                            }
                            if (data.assignments.anggota_kedua) {
                                const auditor3Select = document.getElementById('auditor3');
                                console.log('Found auditor3 select:', auditor3Select);
                                if (auditor3Select) {
                                    auditor3Select.value = data.assignments.anggota_kedua.auditor_id;
                                    console.log('Set anggota_kedua (auditor3):', data.assignments.anggota_kedua.auditor_id);
                                } else {
                                    console.log('auditor3 select element not found');
                                }
                            }
