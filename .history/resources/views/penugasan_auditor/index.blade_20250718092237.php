                ...
                .then(data => {
                    if (data.success && data.assignments) {
                        ...
                        if (assignments.ketua) {
                            document.getElementById('auditor1').value = assignments.ketua.auditor_id;
                        }

                        // Set anggota auditor (auditor2)
                        if (assignments.anggota) {
                            document.getElementById('auditor2').value = assignments.anggota.auditor_id;
                        }

                        // Set anggota kedua (auditor3) if exists
                        if (assignments.anggota_kedua) {
                            document.getElementById('auditor3').value = assignments.anggota_kedua.auditor_id;
                        }

                        // Set waktu visitasi if exists
                        if (assignments.waktu_visitasi) {
                            document.getElementById('waktu_visitasi').value = assignments.waktu_visitasi;
                        }

                        // Update button text to indicate update mode
                        document.getElementById('btnSimpanPenugasan').innerHTML =
                            '<i class="fas fa-check fs-2 me-1"></i>Update Penugasan';
                    }
                })
                .catch(error => {
                    ...
                    document.getElementById('btnSimpanPenugasan').innerHTML =
                        '<i class="fas fa-check fs-2 me-1"></i>Simpan Penugasan';
                });
        }
