                            ...
                            <tr class="text-start text-dark fw-bolder fs-7 text-uppercase gs-0 bg-light-primary">
                                <th class="w-10px pe-2 ps-4">
                                    <div class="form-check form-check-sm form-check-custom form-check-solid ms-3 me-3">
                                        <input class="form-check-input bg-white" type="checkbox" data-kt-check="true" data-kt-check-target="#kt_penugasanAuditor_table .form-check-input" value="1" />
                                    </div>
                                </th>
                                ...
                                <tr>
                                    <td class="w-10px pe-2 ps-4">
                                        <div class="form-check form-check-custom form-check-primary form-check-sm ms-3 me-3">
                                            <input class="form-check-input" type="checkbox" value="{{ $penugasanAuditor->id }}" />
                                        </div>
                                    </td>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $penugasanAuditor->kode_satuan }}</td>
                                    <td>{{ $penugasanAuditor->sasaran }}</td>
                                    <td class="text-center">
                                        @if ($penugasanAuditor->deleted_at)
                                    ...
                                    <td class="text-center">
                                        <div class="button-container">
                                            <button type="button" class="btn btn-sm btn-light-success edit-penugasanAuditor"
                                                data-id="{{ $penugasanAuditor->id }}"
                                                data-url="{{ route('penugasanAuditor.edit', $penugasanAuditor->id) }}"
                                                data-bs-toggle="modal" data-bs-target="#kt_modal">
                                                <i class="fas fa-edit fa-sm"></i>&nbsp;</i> Edit
                                            </button>
                                            @if ($penugasanAuditor->deleted_at)
                                                <button type="button" class="btn btn-sm btn-light-primary restore-data" data-id="{{ $penugasanAuditor->id }}">
                                                    <i class="fas fa-sync-alt fa-sm"></i>&nbsp;Aktifkan
                                                </button>

                                                <button type="button" class="btn btn-sm btn-light-danger delete-permanent" data-id="{{ $penugasanAuditor->id }}">
                                                    <i class="fas fa-trash-alt fa-sm"></i>&nbsp;Hapus
                                                </button>
                                            @else
                                                <button type="button" class="btn btn-sm btn-light-danger nonaktifkan-penugasanAuditor" data-id="{{ $penugasanAuditor->id }}">
...
@push('scripts')
    ...
    <script>
        ...
        $(document).on('click', '.restore-data', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('penugasanAuditor.restore', ':id') }}".replace(':id', id),
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            _method: 'PUT'
        ...
        $('.edit-penugasanAuditor').click(function() {
            let id = $(this).data('id');
            let url = $(this).data('url');

            $('#kt_modal form')[0].reset();
            $('#methodField').val('PUT');
            $('#kt_modal_form').attr('action', "{{ route('penugasanAuditor.update', '') }}/" + id);
        ...
        $('button[data-bs-target="#kt_modal"]:not(.edit-penugasanAuditor)').click(function() {
            $('#kt_modal form')[0].reset();
            $('#methodField').val('POST');
            $('#kt_modal_form').attr('action', "{{ route('penugasanAuditor.store') }}");
        ...
        // Script untuk delete
        $(document).on('click', '.nonaktifkan-penugasanAuditor', function() {
            let id = $(this).data('id');
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('penugasanAuditor.nonaktifkan', '') }}/" + id,
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            _method: 'DELETE'
        ...
        $(document).on('click', '[data-kt-penugasanAuditor-table-select="delete_selected"]', function() {
            const checkboxes = document.querySelectorAll('#kt_penugasanAuditor_table tbody input[type="checkbox"]:checked');
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('penugasanAuditor.nonaktifkanSelected') }}",
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            ids: ids
        ...
        $(document).on('click', '.delete-permanent', function () {
            var id = $(this).data('id');
            var url = "{{ route('penugasanAuditor.hapus_permanen', ':id') }}";
