...
@push('scripts')
    ...
    <script>
                                        ...
                                        <div class="d-flex gap-2">
                                            ${auditStatusMap[penugasanAuditor.id] ? `
                                                <button type="button" class="btn btn-sm btn-secondary flex-grow-1" disabled
                                                        title="Tidak dapat diedit karena audit sudah dimulai">
        ...
        function setPenugasanId(id) {
            // First check if there are any audit activities
            fetch(`/penugasan-auditor/check-audit-activities/${id}`)
                .then(response => response.json())
                .then(data => {
                    if (data.success && data.has_audit_activities) {
                        // Show warning and prevent edit
                        Swal.fire({
                            title: 'Tidak Dapat Mengedit!',
                            text: 'Tim auditor sudah mulai melakukan aktivitas audit (desk evaluation, visitasi, atau penilaian instrumen prodi). Perubahan penugasan dapat mempengaruhi jadwal audit yang sedang berlangsung.',
                            icon: 'warning',
                    ...
                    // If no audit activities, proceed with edit
