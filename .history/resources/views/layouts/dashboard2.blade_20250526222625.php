                                        ...
                                        <div class="card-body pt-9 pb-0">
                                            ...
                                            <ul class="nav nav-stretch nav-line-tabs nav-line-tabs-2x border-transparent fs-5 fw-bold">
                                                ...
                                                @php
                                                    ...
                                                    // Check if each Sasaran Strategis has been filled
                                                    $allSSFilled = true;
                                                    $debugInfo = [];

                                                    foreach ($allSatuanStandar as $ss) {
                                                        // Get required instruments for this SS
                                                        $requiredInstruments = App\Models\InstrumenIkss::whereHas('indikatorKinerja', function($query) use ($ss) {
                                                            $query->where('satuan_standar_id', $ss->id);
                                                        })
                                                        ->where(function($query) use ($user) {
                                                            $query->where('is_wajib', true)
                                                                  ->where(function($q) use ($user) {
                                                                      $q->where('jenjang', 'Semua')
                                                                        ->orWhere('jenjang', optional($user->unitKerja)->jenjang);
                                                                  });
                                                        })
                                                        ->count();

                                                        // Get filled instruments for this SS
                                                        $filledInstruments = App\Models\IkssAuditee::whereHas('instrumen.indikatorKinerja', function($query) use ($ss) {
                                                            $query->where('satuan_standar_id', $ss->id);
                                                        })
                                                        ->where('auditee_id', $unitKerjaId)
                                                        ->where('periode_id', $periodeAktif->id)
                                                        ->where('status_target', 1)  // Only count selected instruments
                                                        ->count();

                                                        $hasFilledRequiredIkss = $filledInstruments >= $requiredInstruments;

                                                        $debugInfo[$ss->kode_satuan] = [
                                                            'required' => $requiredInstruments,
                                                            'filled' => $filledInstruments,
                                                            'is_complete' => $hasFilledRequiredIkss
                                                        ];

                                                        if (!$hasFilledRequiredIkss) {
                                                            $allSSFilled = false;
                                                        }
                                                    }

                                                    // Log debug information
                                                    \Illuminate\Support\Facades\Log::info('IKSS Validation Debug:', [
                                                        'user_id' => $user->id,
                                                        'unit_kerja_id' => $unitKerjaId,
                                                        'periode_id' => $periodeAktif->id,
                                                        'all_ss_filled' => $allSSFilled,
                                                        'ss_details' => $debugInfo
                                                    ]);

                                                    $sudahMengisiIkss = $allSSFilled;
                                                ...
                                                @if(config('app.env') === 'local')
                                                    <div class="alert alert-info">
                                                        ...
                                                        <p>All SS Filled: {{ $allSSFilled ? 'Yes' : 'No' }}</p>
                                                    </div>
                                                @endif
