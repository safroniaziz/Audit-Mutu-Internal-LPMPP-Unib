...
@push('scripts')
    <script>
        $(document).ready(function () {
            $(document).ready(function () {
    // Mengubah daftar kriteria berdasarkan indikator yang dipilih
    $('select[name="indikator_instrumen_id"]').on('change', function () {
        let indikatorId = $(this).val(); // Mendapatkan ID indikator yang dipilih

        if (indikatorId) {
            // Melakukan request untuk mengambil kriteria berdasarkan indikator
            $.ajax({
                url: '/instrumen-prodi/indikator/' + indikatorId + '/kriteria', // Menyesuaikan dengan URL route yang telah dibuat
                type: 'GET',
                success: function (response) {
                    let kriteriaSelect = $('select[name="indikator_instrumen_kriteria_id"]');
                    kriteriaSelect.empty(); // Menghapus opsi yang lama

                    // Menambahkan opsi default
                    kriteriaSelect.append('<option disabled selected>-- pilih kriteria instrumen --</option>');

                    // Menambahkan kriteria yang baru
                ...
                error: function (xhr, status, error) {
                    ...
                    // Menampilkan alert error jika terjadi masalah
                    Swal.fire({
                        icon: 'error',
