...
@push('scripts')
    <script>
        $(document).ready(function () {
            // Ketika dropdown indikator dipilih
    ...
    if (indikatorId) {
        $.ajax({
            ...
            success: function (response) {
                ...
                // Isi kriteria sesuai response dari server
                response.forEach(function (kriteria) {
                    // Tentukan apakah kriteria ini dipilih
                    let selected = (kriteria.id == '{{ old("indikator_instrumen_kriteria_id", $data->indikator_instrumen_kriteria_id ?? "") }}') ? 'selected' : '';
                    kriteriaSelect.append('<option value="' + kriteria.id + '" ' + selected + '>' + kriteria.nama_kriteria + '</option>');
                });
            },
        ...
        // Jika indikator tidak dipilih, kosongkan dropdown kriteria
        $('select[name="indikator_instrumen_kriteria_id"]').empty();
        $('select[name="indikator_instrumen_kriteria_id"]').append('<option disabled selected>-- pilih kriteria instrumen --</option>');
    }
});

// Jika form sudah memiliki data yang sudah disubmit sebelumnya, update select kriteria
$(document).ready(function () {
    let indikatorId = $('select[name="indikator_instrumen_id"]').val();
    if (indikatorId) {
        // Jika sudah ada indikator yang dipilih, fetch kriteria berdasarkan indikator
        $.ajax({
            url: '/instrumen-prodi/indikator/' + indikatorId + '/kriteria',
            type: 'GET',
            success: function (response) {
