        ...
        <div class="modal-content">
            ...
            <div class="modal-body d-flex flex-column scroll-y px-10" style="flex-grow: 1;">
                <form id="kt_modal_form" class="form d-flex flex-column" style="flex-grow: 1;">
                    ...
                    <div class="fv-row mb-5" id="jenjang_container" style="display: none;">
...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function () {
            // Check initial state of is_wajib when modal opens (for edit)
            function checkInitialWajib() {
                const isWajib = $('#is_wajib').val();
                if (isWajib === '1') {
                    $('#jenjang_container').show();
                    $('select[name="jenjang"]').prop('required', true);
                } else {
                    $('#jenjang_container').hide();
                    $('select[name="jenjang"]').prop('required', false);
                }
            }

            // Run on modal show (for edit)
            $('#kt_modal').on('shown.bs.modal', function () {
                checkInitialWajib();
            });
            ...
            $('#is_wajib').change(function() {
                if ($(this).val() === '1') {
                    $('#jenjang_container').slideDown();
                    $('select[name="jenjang"]').prop('required', true);
                } else {
                    $('#jenjang_container').slideUp();
                    $('select[name="jenjang"]').prop('required', false);
                    // Reset jenjang value when hiding
                    $('select[name="jenjang"]').val('');
                }
            });

            // Set selected jenjang when editing
            function setSelectedJenjang(jenjang) {
                if (jenjang) {
                    $('select[name="jenjang"]').val(jenjang);
                }
            }
