...
@push('scripts')
        ...
        if (indikatorId) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    // Set the selected kriteria_id after populating options
                    @if(isset($data)) // Assuming you're passing $data to the view in Laravel
                        let selectedKriteriaId = '{{ old('indikator_instrumen_kriteria_id', $data->indikator_instrumen_kriteria_id) }}';
                        kriteriaSelect.val(selectedKriteriaId);
                    @endif
                },
    ...
    // If editing, populate the kriteria select immediately based on the selected indikator_instrumen_id
    @if(isset($data))
        let selectedIndikatorId = '{{ old('indikator_instrumen_id', $data->indikator_instrumen_id) }}';
        $('select[name="indikator_instrumen_id"]').val(selectedIndikatorId).trigger('change');
    @endif
