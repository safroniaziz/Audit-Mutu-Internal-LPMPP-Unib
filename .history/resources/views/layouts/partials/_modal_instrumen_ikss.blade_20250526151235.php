    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            window.populateModal = function(data) {
                ...
                // Handle is_wajib and jenjang first
                const isWajib = data.is_wajib == 1 || data.is_wajib === true;
                console.log('Is Wajib:', isWajib); // Debug log

                // Set is_wajib value
                $('#is_wajib').val(isWajib ? '1' : '0');

                // Immediately handle jenjang visibility
                if (isWajib) {
                    $('#jenjang_container').css('display', 'block'); // Force show
                    $('select[name="jenjang"]').prop('required', true);
                    if (data.jenjang) {
                        $('select[name="jenjang"]').val(data.jenjang);
                    }
                } else {
                    $('#jenjang_container').css('display', 'none'); // Force hide
