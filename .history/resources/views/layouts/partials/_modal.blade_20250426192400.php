...
$(document).ready(function () {
    ...
    $('#tombolTambah').on('click', function () {
        ...
        $('#methodField').val('POST'); // default ke POST
    });

    // Klik tombol edit
    $('.tombolEdit').on('click', function () {
        isEdit = true;
        editId = $(this).data('id'); // ambil id dari tombol edit

        // Prefill input
        $('#kode_unit_kerja').val($(this).data('kode'));
        $('#nama_unit_kerja').val($(this).data('nama'));
        $('#jenis_unit_kerja').val($(this).data('jenis')).trigger('change');
    ...
    $('#kt_modal_form').on('submit', function (e) {
        ...
        formData += '&_token={{ csrf_token() }}'; // token csrf

        let ajaxUrl = isEdit
            ? `{{ url('unit-kerja') }}/${editId}` // URL update
            : `{{ route('unitKerja.store') }}`;    // URL store

        $.ajax({
            url: ajaxUrl,
            type: "POST", // tetap POST, pakai spoofing _method
            data: formData,
            dataType: "json",
