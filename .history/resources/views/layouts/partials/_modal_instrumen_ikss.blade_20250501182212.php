        ...
        if (element) {
            ClassicEditor
                .create(element, {
                    ...
                    // Disable auto-grow functionality built into CKEditor
                    autoParagraph: false
                })
                .then(editor => {
                    // Custom auto-grow logic
                    ...
                    const adjustHeight = () => {
                        ...
                        // Get the scrollHeight (full content height)
                        const scrollHeight = editable.scrollHeight;

                        // Set height to match content with a small minimum
                        const newHeight = Math.max(40, scrollHeight);
                        editable.style.height = `${newHeight}px`;

                        // Add scrollbar only if content gets very large
                        if (scrollHeight > 300) {
