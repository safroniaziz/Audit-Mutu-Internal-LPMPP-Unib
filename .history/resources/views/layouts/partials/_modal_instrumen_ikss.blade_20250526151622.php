...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function () {
            // Hide jenjang container initially
            $('#jenjang_container').hide();

            // Function to populate modal for edit
            window.populateModal = function(data) {
                console.log('=== DEBUG MODAL POPULATION ===');
                console.log('Raw data:', data);
                console.log('is_wajib value:', data.is_wajib);
                console.log('jenjang value:', data.jenjang);
                ...
                // Set is_wajib and handle jenjang visibility
                console.log('Setting is_wajib select to:', data.is_wajib ? '1' : '0');
                $('#is_wajib').val(data.is_wajib ? '1' : '0');

                // Force jenjang visibility based on is_wajib
                if (data.is_wajib) {
                    console.log('is_wajib is true, showing jenjang container');
                    $('#jenjang_container').show();
                    console.log('Setting jenjang select to:', data.jenjang);
                    $('select[name="jenjang"]').val(data.jenjang);

                    // Double check visibility and values after short delay
                    setTimeout(() => {
                        console.log('=== VISIBILITY CHECK ===');
                        console.log('Jenjang container display:', $('#jenjang_container').css('display'));
                        console.log('Is jenjang container visible:', $('#jenjang_container').is(':visible'));
                        console.log('Current jenjang value:', $('select[name="jenjang"]').val());
                        console.log('Current is_wajib value:', $('#is_wajib').val());
                    }, 100);
                } else {
                    console.log('is_wajib is false, hiding jenjang container');
                    $('#jenjang_container').hide();
                }
            ...
            // Toggle jenjang field visibility based on is_wajib selection
