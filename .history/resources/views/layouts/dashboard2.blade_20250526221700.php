                                        ...
                                        <div class="card-body pt-9 pb-0">
                                            ...
                                            <ul class="nav nav-stretch nav-line-tabs nav-line-tabs-2x border-transparent fs-5 fw-bold">
                                                ...
                                                @elseif (Route::is('auditee.pengajuanAmi*'))
                                                    ...
                                                    <!-- Add this before the menu item -->
                                                    @php
                                                        // Get all SS completion status
                                                        $allSSCompleted = true;
                                                        $dataTerpilih = session('dataTerpilih', []);

                                                        if (Auth::check() && Auth::user()->unit_kerja_id) {
                                                            // Get all instrumen from database grouped by SS
                                                            $dataIkssProdi = App\Models\UnitKerja::with(['indikatorKinerjas.instrumen'])
                                                                ...
                                                                foreach ($unit->indikatorKinerjas->groupBy('satuan_standar_id') as $ssId => $ssIndikators) {
                                                                    ...
                                                                    foreach ($ssIndikators as $indikator) {
                                                                        foreach ($indikator->instrumen as $instrumen) {
                                                                            ...
                                                                            if ($isWajib || isset($dataTerpilih['pilihan_'.$instrumen->id])) {
                                                                                $selectedInstruments++;
                                                                            }
                                                                        }
                                                                    }

                                                                    // If any SS is not fully completed, set allSSCompleted to false
                                                                    if ($totalInstruments === 0 || $selectedInstruments !== $totalInstruments) {
                                                                        $allSSCompleted = false;
                                                                        break; // We can break early since we found an incomplete SS
                                                                    }
                                                                }
                                                                if (!$allSSCompleted) break;
                                                            }
                                                        }

                                                        // Override the existing $sudahMengisiIkss variable
                                                        $sudahMengisiIkss = $allSSCompleted;
                                                    @endphp
