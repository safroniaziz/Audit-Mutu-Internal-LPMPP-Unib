            ...
            <div class="card">
                <div class="card-header border-0 pt-6">
                    ...
                    <div class="card-title">
                        <div class="d-flex align-items-center position-relative my-1">
                            ...
                            <input type="text" data-kt-rsbF-table-filter="search" class="form-control form-control-solid w-250px ps-12" placeholder="Cari Instrumen Rsb Prodi" />
                        </div>
                    </div>
                    <div class="card-toolbar">
                        <div class="d-flex justify-content-end" data-kt-rsbF-table-toolbar="base">
                ...
                <div class="card-body pt-0">
                    ...
                    <div class="table-responsive">
                        <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_rsbF_table">
                            <thead>
                                <tr class="text-start text-dark fw-bolder fs-7 text-uppercase gs-0 bg-light-primary">
                                    <th class="w-10px pe-2 ps-4">
                                        <div class="form-check form-check-sm form-check-custom form-check-solid ms-3 me-3">
                                            <input class="form-check-input bg-white" type="checkbox" data-kt-check="true" data-kt-check-target="#kt_rsbF_table .form-check-input" value="1" />
                                        </div>
                                    </th>
                                    <th class=" ps-3">No</th>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <div class="button-container">
                                                <button type="button"
                                                    class="btn btn-sm btn-light-primary detail-rsbF"
                                                    data-id="{{ $prodi->id }}"
                                                    data-url="{{ route('rsbF.modal', $prodi->id) }}"
...
@push('scripts')
    ...
    <script>
        ...
        $('#satuan_standar_id').on('change', function () {
            ...
            let $instrumenSelect = $('#instrumen_ikss_id');

            $indikatorSelect.empty().append('<option value="">Pilih Indikator IKSS</option>');
            $instrumenSelect.empty().append('<option value="">Pilih Instrumen IKSS</option>');
        ...
        $('#indikator_ikss_id').on('change', function () {
            ...
            if (indikatorId) {
                $.ajax({
                    url: `/rsb-prodi/get-instrumen-by-indikator/${indikatorId}`,
                    type: 'GET',
                    success: function (data) {
                        data.forEach(function (item) {
                            $instrumenSelect.append(`<option value="${item.id}">${item.indikator}</option>`);
                        });
                    },
                    error: function () {
        ...
        function updateMainTable(unitKerjaId) {
            ...
            $(document).on('click', '.detail-rsbF', function() {
            ...
            $('#form-tambah-instrumen').on('submit', function(e) {
                ...
                const modalUrl = $('.detail-rsbF[data-id="' + unitKerjaId + '"]').data('url');
                ...
                $.ajax({
                    url: "{{ route('rsbF.tambahInstrumen') }}",
                    type: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}",
