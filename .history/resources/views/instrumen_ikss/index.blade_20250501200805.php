...
@push('scripts')
    ...
    <script>
        ...
        // Your edit button click handler with a different approach
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        // For CKEditor fields, we'll need to reset and reinitialize them
                        // This is a brute force approach but should work
                        editorFields.forEach(id => {
                            const fieldName = id.replace('kt_docs_ckeditor_', '');

                            // Find the current CKEditor div
                            const editorDiv = document.querySelector(`#${id}`).nextElementSibling;
                            if (editorDiv && editorDiv.classList.contains('ck-editor')) {
                                // Remove it
                                editorDiv.remove();
                                ...
                                // Set its value
                                document.querySelector(`#${id}`).value = data[fieldName] || '';
                                ...
                                ClassicEditor
                                    ...
                                    .catch(error => {
                                        console.error(`Error reinitializing ${id}:`, error);
                                    });
                            }
                        });
                    }
    ...
    // Reset dropdown
    $('select[name="indikator_kinerja_id"]')
        .empty()
        .append('<option disabled selected>-- pilih indikator_kinerja --</option>');

    savedKriteriaId = null;

    // Destroy and reset all CKEditor instances
    if (typeof editorInstances !== 'undefined') {
        for (const [key, editor] of Object.entries(editorInstances)) {
            if (editor && typeof editor.destroy === 'function') {
                editor.destroy().then(() => {
                    const textarea = document.querySelector(`#kt_docs_ckeditor_${key}`);
                    if (textarea) {
                        textarea.style.display = '';
                        textarea.value = '';
                    }
                }).catch(error => {
                    console.warn('Gagal destroy editor:', error);
                });
            }
        }
        editorInstances = {};
