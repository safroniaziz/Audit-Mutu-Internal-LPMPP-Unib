    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                // Use stored editor instances to set data
                try {
                    // Log available editors for debugging
                    console.log("Available editor instances:", Object.keys(editorInstances));

                    // Check and set data for each editor if it exists
                    if (editorInstances.indikator) {
                        console.log("Setting indikator data:", data.indikator);
                        editorInstances.indikator.setData(data.indikator || '');
                    }

                    if (editorInstances.satuan) {
                        console.log("Setting satuan data:", data.satuan);
                        editorInstances.satuan.setData(data.satuan || '');
                    }

                    if (editorInstances.pertanyaan) {
                        console.log("Setting pertanyaan data:", data.pertanyaan);
                        editorInstances.pertanyaan.setData(data.pertanyaan || '');
                    }

                    if (editorInstances.sumber) {
                        console.log("Setting sumber data:", data.sumber);
                        editorInstances.sumber.setData(data.sumber || '');
                    }

                    if (editorInstances.uraian) {
                        console.log("Setting uraian data:", data.uraian);
                        editorInstances.uraian.setData(data.uraian || '');
                    }

                    if (editorInstances.penilaian) {
                        console.log("Setting penilaian data:", data.penilaian);
                        editorInstances.penilaian.setData(data.penilaian || '');
                    }
                } catch (error) {
