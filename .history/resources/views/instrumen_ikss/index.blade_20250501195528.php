...
@push('styles')
    <style>
        #detailinstrumenIkssContent td p {
            margin-bottom: 2px !important;
            margin-top: 2px !important;
        }

        #detailinstrumenIkssContent{
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <button type="button"
                                                ...
                                                data-url="{{ route('instrumenIkss.show', $instrumenIkss->id) }}"
                                                data-bs-toggle="modal"
                                                data-bs-target="#modalDetailinstrumenIkss">
                    ...
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="modalDetailinstrumenIkssLabel">Detail instrumen Ikss</h5>
...
@push('scripts')
    ...
    <script>
        ...
        $(document).on('click', '.detail-instrumenIkss', function () {
            const url = $(this).data('url');

            $('#detailinstrumenIkssContent').html('<div class="text-center">Memuat data...</div>');
                ...
                success: function (res) {
                    let html = `
                        <table class="table table-borderless">
                            <tr><td style="width: 200px;"><strong>Tujuan</strong></td><td style="width: 10px;">:</td><td>${res.tujuan ?? '-'}</td></tr>
                            <tr><td><strong>Indikator</strong></td><td>:</td><td>${res.indikator ?? '-'}</td></tr>
                            <tr><td><strong>Satuan</strong></td><td>:</td><td>${res.satuan ?? '-'}</td></tr>
                            <tr><td><strong>Pertanyaan</strong></td><td>:</td><td>${res.pertanyaan ?? '-'}</td></tr>
                            <tr><td><strong>Target</strong></td><td>:</td><td>${res.target ?? '-'}</td></tr>
                            <tr><td><strong>Sumber</strong></td><td>:</td><td>${res.sumber ?? '-'}</td></tr>
                            <tr><td><strong>Uraian</strong></td><td>:</td><td>${res.uraian ?? '-'}</td></tr>
                            <tr><td><strong>Penilaian</strong></td><td>:</td><td>${res.penilaian ?? '-'}</td></tr>
                            <tr><td><strong>Jenis Auditee</strong></td><td>:</td><td>${res.jenis_auditee ?? '-'}</td></tr>
                            <tr><td><strong>Is Wajib</strong></td><td>:</td><td>${res.is_wajib == 1 ? 'Ya' : 'Tidak'}</td></tr>
                        </table>
                    `;
                    // Gunakan .html(), bukan .text(), agar HTML diresapi
                    $('#detailinstrumenIkssContent').html(html);
                    $('#modalDetailinstrumenIkss').modal('show');
                },
                error: function () {
                    $('#detailinstrumenIkssContent').html('<div class="text-danger text-center">Gagal memuat data.</div>');
                }
            });
        });
        ...
        // Your edit button click handler
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                // Set regular form field values
                $('#kt_modal select[name="indikator_kinerja_id"]').val(data.indikator_kinerja_id);
                $('#kt_modal input[name="target"]').val(data.target);
                $('#kt_modal select[name="jenis_auditee"]').val(data.jenis_auditee);
                $('#kt_modal select[name="is_wajib"]').val(data.is_wajib);

                // Set CKEditor values - using the proper method for CKEditor 5
                try {
                    // For indikator field
                    const editorIndikator = document.querySelector('#kt_docs_ckeditor_indikator');
                    if (editorIndikator && editorIndikator.ckeditorInstance) {
                        editorIndikator.ckeditorInstance.setData(data.indikator || '');
                    }

                    // For satuan field
                    const editorSatuan = document.querySelector('#kt_docs_ckeditor_satuan');
                    if (editorSatuan && editorSatuan.ckeditorInstance) {
                        editorSatuan.ckeditorInstance.setData(data.satuan || '');
                    }

                    // For pertanyaan field
                    const editorPertanyaan = document.querySelector('#kt_docs_ckeditor_pertanyaan');
                    if (editorPertanyaan && editorPertanyaan.ckeditorInstance) {
                        editorPertanyaan.ckeditorInstance.setData(data.pertanyaan || '');
                    }

                    // For sumber field
                    const editorSumber = document.querySelector('#kt_docs_ckeditor_sumber');
                    if (editorSumber && editorSumber.ckeditorInstance) {
                        editorSumber.ckeditorInstance.setData(data.sumber || '');
                    }

                    // For uraian field
                    const editorUraian = document.querySelector('#kt_docs_ckeditor_uraian');
                    if (editorUraian && editorUraian.ckeditorInstance) {
                        editorUraian.ckeditorInstance.setData(data.uraian || '');
                    }

                    // For penilaian field
                    const editorPenilaian = document.querySelector('#kt_docs_ckeditor_penilaian');
                    if (editorPenilaian && editorPenilaian.ckeditorInstance) {
                        editorPenilaian.ckeditorInstance.setData(data.penilaian || '');
                    }

                    // For tujuan field
                    const editorTujuan = document.querySelector('#kt_docs_ckeditor_tujuan');
                    if (editorTujuan && editorTujuan.ckeditorInstance) {
                        editorTujuan.ckeditorInstance.setData(data.tujuan || '');
                    }
                } catch (error) {
