...
@php
    ...
    // Get active step from URL hash or use default
    $activeStep = request()->get('step');
    if (!$activeStep || !isset($ssCompletionStatus[$activeStep])) {
        $activeStep = array_key_first($ssCompletionStatus);
    }
@endphp
...
@push('styles')
    <style>
        ...
        .wizard-step:hover:not(.disabled) .step-number {
            background: #009EF7 !important;
            color: #FFFFFF !important;
            border-color: #009EF7 !important;
        }

        .wizard-step:hover:not(.disabled) .step-label,
        .wizard-step:hover:not(.disabled) .step-desc {
            color: #009EF7 !important;
        }

        .wizard-step:hover:not(.disabled):not(:last-child):after {
            background: #009EF7 !important;
        }
        ...
        .wizard-step.completed + .wizard-step.disabled,
        .wizard-step.active + .wizard-step.disabled {
            cursor: pointer;
            opacity: 0.8;
        }
...
<script>
    $(document).ready(function() {
        ...
        // Check if there's a hash in URL indicating we should scroll
        if (window.location.hash === '#scroll-top') {
            // Remove the hash
            window.history.replaceState(null, null, window.location.pathname);
            // Scroll to wizard nav
        ...
        function initializeWizard() {
            ...
            // Get active step from URL or use first step
            let activeStepId;
            const urlParams = new URLSearchParams(window.location.search);
            activeStepId = urlParams.get('step');

            if (!activeStepId) {
                activeStepId = $('.wizard-step:first').data('step');
            }
            ...
            $(`.wizard-step[data-step="${activeStepId}"]`).addClass('active');
        }
        ...
        $('.wizard-step').click(function() {
            ...
            // 3. Previous step is completed (for next step)
            if (stepElement.hasClass('completed') ||
                stepElement.hasClass('active') ||
                stepElement.prev('.wizard-step').hasClass('completed')) {
                showStep(stepId);
            }
        });
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            ...
                            beforeSend: function() {
                                ...
                                $('.next-step, .submit-final-step').prop('disabled', true);
                            },
                                ...
                                if (response.status === 'success' || response.success) {
                                    Swal.fire({
                                        ...
                                        if (result.isConfirmed) {
                                            ...
                                            if (nextStepId) {
                                                window.location.href = window.location.pathname + '?step=' + nextStepId + '#scroll-top';
                                            }
                            ...
                            error: function(xhr) {
                                ...
                                $('.next-step, .submit-final-step').prop('disabled', false);
