...
@php
    ...
    foreach ($groupedData as $indikatorId => $indikatorData) {
        foreach ($indikatorData['kriterias'] as $kriteriaId => $kriteriaData) {
            foreach ($kriteriaData['instrumens'] as $instrumenProdi) {
                ...
                // More robust check: ensure nilaiAuditor exists, has count > 0, and has valid nilai (including 0)
                if ($instrumenProdi->nilaiAuditor &&
                    $instrumenProdi->nilaiAuditor->count() > 0 &&
                    !is_null($instrumenProdi->nilaiAuditor->first()->nilai)) {
                    $completedInstrumen++;
                }
            }
        }
    ...
    // Debug: Log completion status for troubleshooting
    Log::info('Penilaian Instrumen Prodi Completion Status:', [
        'pengajuan_id' => $pengajuan->id,
        'auditor_id' => Auth::id(),
        'total_instrumen' => $totalInstrumen,
        'completed_instrumen' => $completedInstrumen,
            ...
            <div class="mb-8">
                    ...
                    <div class="d-flex align-items-center mb-3">
                        ...
                        <span class="text-muted fs-7 ms-2">
                            (Total: {{ $totalInstrumen }} instrumen)
                        </span>
                    </div>
                                ...
                                <div class="d-flex flex-column pe-0 pe-sm-10">
                                    ...
                                    <div class="fs-6">
                                        ...
                                        <div class="fw-semibold text-gray-700 mt-1">
                                            Progress: {{ $completedKriteriaInstrumen }}/{{ $totalKriteriaInstrumen }} instrumen dinilai lengkap
                                            @if($totalKriteriaInstrumen > 0)
                                                <span class="text-muted fs-7">({{ number_format(($completedKriteriaInstrumen / $totalKriteriaInstrumen) * 100, 1) }}%)</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                                            ...
                                                            <div class="mb-4">
                                                                ...
                                                                <div class="rating-container">
                                                                    <div class="row g-3">
                                                                        ...
                                                                        <div class="col-6 col-md-3">
                                                                            <input type="radio" name="nilai[{{ $instrumenProdi->id }}]" value="0" id="nilai0_{{ $instrumenProdi->id }}" class="rating-input" {{ ($instrumenProdi->nilaiAuditor && $instrumenProdi->nilaiAuditor->first() && $instrumenProdi->nilaiAuditor->first()->nilai == 0) ? 'checked' : '' }} required>
                                                                            <label for="nilai0_{{ $instrumenProdi->id }}" class="rating-card">
                                                                                <div class="rating-number">0</div>
                                                                                <div class="rating-label">Tidak Ada</div>
                                                                                <div class="rating-desc">Tidak ditemukan</div>
                                                                            </label>
                                                                        </div>
                                                                    </div>
                                                                </div>
...
<script>
    ...
    // Approve penilaian button (for both completion page and regular page)
    ...
    // Approve and back button (for completion page only)
    $(document).on('click', '#approve-and-back-btn', function() {
        const pengajuanId = $(this).data('id');
    ...
    function updateKriteriaStepStatus(kriteriaId, isCompleted) {
        ...
        if (isCompleted) {
            ...
            // Update kriteria data structure to reflect completion
            updateKriteriaDataStructure(kriteriaId, true);

            console.log('Updated kriteria', kriteriaId, 'to completed status');
        }
    }

    // Update kriteria data structure without reload
    function updateKriteriaDataStructure(kriteriaId, isCompleted) {
        // Update the kriteria accessibility for next kriteria
        const currentIndex = $('.wizard-step').index($(`.wizard-step[data-kriteria="${kriteriaId}"]`));
        const nextKriteriaElement = $('.wizard-step').eq(currentIndex + 1);

        if (nextKriteriaElement.length > 0) {
            const nextKriteriaId = nextKriteriaElement.attr('data-kriteria');

            // Make next kriteria accessible
            nextKriteriaElement.removeClass('disabled').addClass('accessible');
            nextKriteriaElement.attr('data-accessible', 'true');

            // Remove lock icon from next kriteria
            nextKriteriaElement.find('.step-status i.fa-lock').removeClass('fa-lock text-muted').addClass('fa-circle text-primary');

            console.log('Made next kriteria', nextKriteriaId, 'accessible');
        }
    }
    ...
    // Show completion status after finishing all kriteria
    function showCompletionStatus() {
        // Hide all kriteria content
        $('.kriteria-content').hide();
        ...
        const completionHtml = `
            <div class="text-center py-10">
                ...
                <h2 class="fw-bold text-success mb-4">🎉 Penilaian Instrumen Prodi Selesai!</h2>
                <p class="fs-5 text-gray-600 mb-8">
                    Semua kriteria telah berhasil dinilai dan disimpan.
                    <br>Penilaian Instrumen Prodi untuk auditee ini telah selesai.
                </p>
                <div class="d-flex justify-content-center gap-3">
                    <button type="button" class="btn btn-success btn-lg" id="approve-and-back-btn" data-id="{{ $pengajuan->id }}">
                        <i class="bi bi-check-circle me-2"></i>
                        Setujui & Lanjut
                    </button>
                    <button type="button" class="btn btn-light btn-lg" onclick="showAllKriteria()">
        ...
        // Replace form container with completion message
        $('#penilaianForm').html(completionHtml);

        // Scroll to top
        document.documentElement.scrollTo({ top: 0, behavior: 'smooth' });
    }

    // Show all kriteria after completion
    function showAllKriteria() {
        // Reload the page to show all kriteria
        window.location.reload();
    }
