                ...
                <form action="{{ route('auditor.audit.beritaAcara',[$pengajuan->id]) }}" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('catatan_visitasi') is-invalid @enderror" id="catatan_visitasi" name="catatan_visitasi" rows="5" placeholder="Isi catatan visitasi disini...">{{ old('catatan_visitasi', $pengajuan->catatan_visitasi ?? '') }}</textarea>
                            @error('catatan_visitasi')
...
<script>
    ...
    if (beritaAcaraForm) {
        beritaAcaraForm.addEventListener('submit', function(e) {
            // Form akan submit normal (tidak prevent default)
            // Modal akan tertutup otomatis karena redirect
        });
    }
</script>
