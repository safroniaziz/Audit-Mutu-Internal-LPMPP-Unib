        ...
        <div class="card mb-8">
            <div class="card-header">
                <div class="card-title">
                    <h3 class="fw-bold text-gray-800">Penilaian Instrumen Prodi</h3>
                </div>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        <div class="d-flex align-items-center mb-3">
                            <span class="fw-semibold text-gray-600 me-2">Auditee:</span>
                            <span class="fw-bold text-gray-800">{{ $pengajuan->auditee->nama ?? '-' }}</span>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="d-flex align-items-center mb-3">
                            <span class="fw-semibold text-gray-600 me-2">Periode:</span>
                            <span class="fw-bold text-gray-800">{{ $pengajuan->periodeAktif->nama ?? '-' }}</span>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="row g-3">
                        @foreach($allKriterias as $kriteria)
                            @php
                                ...
                                $stepClass = $isKriteriaCompleted ? 'border-success' : 'border-light';
                                $textClass = $isKriteriaCompleted ? 'text-success' : 'text-gray-600';
                            @endphp
                            <div class="col-md-3 col-sm-6">
                                <div class="card border {{ $stepClass }} kriteria-card" data-kriteria="{{ $kriteriaId }}" onclick="showKriteriaContent('{{ $kriteriaId }}')" style="cursor: pointer;">
                                    <div class="card-body text-center p-4">
                                        <div class="bg-light-primary rounded-circle d-inline-flex align-items-center justify-content-center mb-3" style="width: 50px; height: 50px;">
                                            <span class="fw-bold fs-4 text-primary">{{ $loop->iteration }}</span>
                                        </div>
                                        <h6 class="fw-bold {{ $textClass }} mb-1">{{ $kriteria['kriteria']->kode_kriteria }}</h6>
                                        <p class="text-muted fs-7 mb-2">{{ $kriteria['kriteria']->nama_kriteria }}</p>
                                        <div class="d-flex justify-content-center align-items-center">
                                            <span class="badge {{ $isKriteriaCompleted ? 'badge-light-success' : 'badge-light-warning' }} fs-8">
...
<script>
    function showKriteriaContent(kriteriaId) {
        ...
        // Update kriteria cards
        $('.kriteria-card').removeClass('border-primary').addClass('border-light');
        $(`.kriteria-card[data-kriteria="${kriteriaId}"]`).removeClass('border-light').addClass('border-primary');

        // Scroll to top of content
        $('.kriteria-card')[0].scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
