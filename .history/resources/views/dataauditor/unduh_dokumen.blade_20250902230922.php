...
<script>
    ...
    @endif
                ...
                // Spider Chart untuk Jaring Laba-laba Instrumen Prodi (Sama seperti Admin)
    @if($instrumenProdiNilai->count() > 0)
        ...
        if (spiderChartProdiCtx) {
            ...
            // Siapkan data untuk chart Prodi (sama seperti admin)
            const prodiLabels = [];
            const prodiValues = [];

            instrumenProdiData.forEach(item => {
                // Gunakan kode kriteria sebagai label (jika ada)
                let label = `Kriteria ${item.id}`;
                if (item.instrumen_prodi && item.instrumen_prodi.kriteria_instrumen) {
                    const kriteria = item.instrumen_prodi.kriteria_instrumen;
                    if (kriteria.kode_kriteria) {
                        label = kriteria.kode_kriteria;
                    } else if (kriteria.nama_kriteria) {
                        label = kriteria.nama_kriteria.length > 20 ? kriteria.nama_kriteria.substring(0, 20) + '...' : kriteria.nama_kriteria;
                    }
                }
                prodiLabels.push(label);

                // Gunakan nilai numerik
                const nilai = parseInt(item.nilai) || 0;
                prodiValues.push(nilai);
            });

            // Data untuk Instrumen Prodi Radar Chart (sama seperti admin)
            const dataProdi = {
                labels: prodiLabels,
                datasets: [{
            ...
            // Config untuk chart Prodi (sama seperti admin)
            ...
            // Buat chart Prodi
            new Chart(spiderChartProdiCtx, configRadarProdi);
        }
    @endif
</script>
