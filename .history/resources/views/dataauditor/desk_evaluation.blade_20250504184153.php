                            ...
                            <form action="{{ route('auditor.audit.submitDeskEvaluation') }}" method="POST" enctype="multipart/form-data" id="formDeskEvaluation">
                                ...
                                @foreach ($groupedIkss as $kodeIkss => $ikssGroup)
                                    @php
                                        ...
                                        $allEvaluated = $ikssGroup->every(function($item) use ($deskEvaluation) {
                                            return isset($deskEvaluation[$item->id]);
                                        });
                                    @endphp
                                    ...
                                    <div class="card mb-5 {{ $allEvaluated ? 'border-success' : '' }}">
                                        <div class="card-header {{ $allEvaluated ? 'bg-success-subtle' : 'bg-light' }} pt-4">
                                            <div class="d-flex justify-content-between align-items-center">
                                                <h4>ID IKSS: {{ $kodeIkss }} – {{ $firstIkss->instrumen->indikatorKinerja->tujuan }}</h4>
                                                @if($allEvaluated)
                                                    <span class="badge bg-success">Semua Instrumen Sudah Dievaluasi</span>
                                                @endif
                                            </div>
                                        </div>
                                        <div class="card-body">
                                            ...
                                            @foreach ($ikssGroup as $index => $ikssAuditee)
                                                ...
                                                <div class="instrument-container mb-4 {{ $index > 0 ? 'mt-5 pt-4 border-top' : '' }}">
                                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                                        <h5>Instrumen #{{ $index + 1 }}: {{ $ikssAuditee->instrumen->indikator }}</h5>
                                                        @if($hasEvaluation)
                                                    ...
                                                    <div class="mb-4">
                                                        <div class="mt-3">
                                                            ...
                                                            <textarea rows="2" class="form-control" name="deskripsi[{{ $ikssAuditee->id }}]" {{ $hasEvaluation ? 'disabled' : '' }}>{{ $hasEvaluation ? $deskEvaluation[$ikssAuditee->id]->penilaian_rencana : '' }}</textarea>
                                                        </div>
                                                        <div class="mt-3">
                                                            <h6>Pertanyaan</h6>
                                                            <textarea rows="2" class="form-control" name="pertanyaan[{{ $ikssAuditee->id }}]" {{ $hasEvaluation ? 'disabled' : '' }}>{{ $hasEvaluation ? $deskEvaluation[$ikssAuditee->id]->pertanyaan : '' }}</textarea>
                                                        </div>
                                                        <div class="mt-3">
                                                            <h6>Nilai</h6>
                                                            <textarea rows="1" class="form-control" name="nilai[{{ $ikssAuditee->id }}]" {{ $hasEvaluation ? 'disabled' : '' }}>{{ $hasEvaluation ? $deskEvaluation[$ikssAuditee->id]->nilai : '' }}</textarea>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endforeach
                                ...
                                @php
                                    $evaluatedCount = $dataIkss->filter(function($item) use ($deskEvaluation) {
                                        return isset($deskEvaluation[$item->id]);
                                    })->count();
                                @endphp
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#formDeskEvaluation').on('submit', function(e) {
                ...
                $('textarea[name^="penilaian_rencana"], textarea[name^="pertanyaan"], textarea[name^="nilai"]').each(function() {
                    if($(this).val().trim() === '') {
                        valid = false;
