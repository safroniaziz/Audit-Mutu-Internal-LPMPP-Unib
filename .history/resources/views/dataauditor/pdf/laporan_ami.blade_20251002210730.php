...
<body>
    ...
    <div class="document-container">
        ...
        @php
            // Recompute SS chart payload right before rendering to ensure scope
            $ssLabels = [];
            $ssValues = [];
            ...
            $ssConfig['data']['labels'] = $ssLabels;
            $ssConfig['data']['datasets'][0]['data'] = $ssValues;
            $ssUrl = 'https://quickchart.io/chart?width=700&height=500&backgroundColor=white&c=' . urlencode(json_encode($ssConfig, JSON_UNESCAPED_SLASHES));
        @endphp
        @if(!empty($ssLabels))
        ...
        @php
            // Radar SS tepat di bawah tabel SS, samakan dengan unduh_dokumen (filter has_data, urutan as-is)
            $ssLabels = [];
            $ssValues = [];
            $ssItems = collect($sortedGrouped ?? [])->filter(function($i){ return !empty($i['has_data']); })->values();
            foreach ($ssItems as $item) {
                ...
                'options' => [
                    ...
                    'scales' => [
                        'r' => [
                            ...
                            'beginAtZero' => true,
                            'ticks' => [ 'stepSize' => 1, 'max' => 4, 'backdropColor' => 'transparent', 'color' => '#666', 'font' => ['size' => 10] ],
            ...
            $ssUrl = $qcBase . '?width=700&height=500&backgroundColor=white&c=' . urlencode(json_encode($ssConfig, JSON_UNESCAPED_SLASHES));
        @endphp
        ...
        @php
            // Recompute Prodi chart payload right before rendering
            $prodiLabels = [];
            $prodiValues = [];
            ...
            $prodiConfig['data']['labels'] = $prodiLabels;
            $prodiConfig['data']['datasets'][0]['data'] = $prodiValues;
            $prodiUrl = 'https://quickchart.io/chart?width=700&height=500&backgroundColor=white&c=' . urlencode(json_encode($prodiConfig, JSON_UNESCAPED_SLASHES));
        @endphp
        ...
        @php
            // Radar Prodi tepat di bawah tabel Prodi, samakan dengan unduh_dokumen
            $prodiLabels = [];
            $prodiValues = [];
            foreach (($kriteriaScores ?? []) as $item) {
                $prodiLabels[] = $item['kode_kriteria'] ?? '';
            ...
            $qcBase = 'https://quickchart.io/chart';
            $prodiUrl = $qcBase . '?width=700&height=500&backgroundColor=white&c=' . urlencode(json_encode($prodiConfig, JSON_UNESCAPED_SLASHES));
        @endphp
