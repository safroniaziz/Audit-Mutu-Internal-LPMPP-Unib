        ...
        <table class="tujuanAudit" style="width: 100%; border-collapse: collapse; margin-top: 5px;">
            <thead>
                <tr>
                    ...
                    <th colspan="3" style="padding: 10px; border: 1px solid #ddd; text-align: left; background-color: #00447c; font-size:12px; font-family: 'Roboto', sans-serif !important; color:white; font-weight: bold; text-align:center;">Penilaian Auditor</th>
                </tr>
                <tr>
            ...
            <tbody>
                @foreach ($pengajuanAmis->ikssAuditee as $index => $ikssAuditee)
                    @php
                        $nilaiKetua = null;
                        $nilaiPendamping = null;

                        foreach ($ikssAuditee->nilai as $nilai) {
                            $auditor = $nilai->auditor;
                            $penugasan = $auditor->penugasan->firstWhere('pengajuan_ami_id', $pengajuanAmis->id);

                            if ($penugasan) {
                                if ($penugasan->role === 'ketua') {
                                    $nilaiKetua = $nilai;
                                } elseif ($penugasan->role === 'pendamping') {
                                    $nilaiPendamping = $nilai;
                                }
                            }
                        }

                        $total = ($nilaiKetua?->nilai ?? 0) + ($nilaiPendamping?->nilai ?? 0);
                        $rataRata = $nilaiKetua && $nilaiPendamping ? $total / 2 : ($nilaiKetua?->nilai ?? $nilaiPendamping?->nilai ?? 0);
                    @endphp
                    <tr>
                        ...
                        <td style="padding: 10px; border: 1px solid #ddd; text-align:center;">{{ $nilaiKetua?->nilai ?? '-' }}</td>
                        <td style="padding: 10px; border: 1px solid #ddd; text-align:center;">{{ $nilaiPendamping?->nilai ?? '-' }}</td>
                        <td style="padding: 10px; border: 1px solid #ddd; text-align:center;">{{ $total }}</td>
                        <td style="padding: 10px; border: 1px solid #ddd; text-align:center;">{{ is_numeric($rataRata) ? number_format($rataRata, 2) : '-' }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
