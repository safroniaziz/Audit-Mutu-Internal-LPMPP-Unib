...
<script>
    ...
    @if($instrumenProdiNilai->count() > 0)
        ...
        if (spiderChartProdiCtx) {
            ...
            // Siapkan data untuk chart Prodi dengan fallback sederhana
            const labelsProdi = [];
            const dataProdi = [];

            instrumenProdiData.forEach((nilai, index) => {
                // Ambil nilai numerik dengan fallback ke 0
                const nilaiNumerik = parseInt(nilai.nilai) || 0;
                dataProdi.push(nilaiNumerik);

                // Label sederhana dengan fallback
                let label = `Kriteria ${index + 1}`;
                try {
                    if (nilai.instrumen_prodi && nilai.instrumen_prodi.kriteria_instrumen && nilai.instrumen_prodi.kriteria_instrumen.nama_kriteria) {
