...
<script>
    ...
    @endif
                                ...
                                // Spider Chart untuk Jaring Laba-laba Instrumen Prodi (Sama seperti RSB)
    @if($instrumenProdiNilai->count() > 0)
        ...
        if (spiderChartProdiCtx) {
            ...
            const labelsProdi = instrumenProdiData.map((item, index) => {
                ...
                if (item.instrumen_prodi && item.instrumen_prodi.kriteria_instrumen) {
                    ...
                    if (kriteria.nama_kriteria) {
                        label = kriteria.nama_kriteria.length > 30 ? kriteria.nama_kriteria.substring(0, 30) + '...' : kriteria.nama_kriteria;
                    }
                }
                return label;
            });
            
            const dataProdi = instrumenProdiData.map(item => parseInt(item.nilai) || 0);
                        ...
                        tooltip: {
                            callbacks: {
                                label: function(context) {
                                    const labels = ['Tidak Sesuai', 'Kurang Sesuai', 'Sesuai', 'Sangat Sesuai'];
                                    const nilai = context.parsed.r;
                                    const label = labels[value - 1] || 'Tidak ada';
                                    return `Nilai: ${nilai} (${label})`;
                                }
                            }
                        }
