            ...
            <div class="card-body">
                ...
                <div class="row g-5">
                    @foreach ($dokumenAmis as $dokumenAmi)
                        @php
    $extension = pathinfo($dokumenAmi->file_dokumen, PATHINFO_EXTENSION);
@endphp
                        <div class="col-md-6">
                            <div class="card overlay overflow-hidden shadow-sm h-100 document-card position-relative">
                                <div class="card-body p-0">
                                    <div class="overlay-wrapper bg-light d-flex align-items-center justify-content-center text-center flex-column px-3 py-4" style="height: 140px;">
                                        <i class="bi bi-file-earmark-pdf text-danger mb-2" style="font-size: 3.5rem;"></i>
                                        <span class="fw-bold text-dark fs-6">{{ $dokumenAmi->nama_dokumen }}</span>
                                        <small class="text-muted">PDF • {{ formatSizeUnits($dokumenAmi->size_dokumen) }} •     {{ \Carbon\Carbon::parse($dokumenAmi->created_at)->translatedFormat('d F Y') }}</small>
                                    </div>
