...
<style>
    ...
    /* Approved state - when penilaian is already approved */
    .wizard-step.disabled[style*="cursor: not-allowed"] {
        opacity: 0.6;
        cursor: not-allowed !important;
        pointer-events: none;
    }

    .wizard-step.disabled[style*="cursor: not-allowed"]:hover {
        transform: none;
        opacity: 0.6;
    }

    .wizard-step.disabled[style*="cursor: not-allowed"] .step-number {
        background: #E4E6EF !important;
        border-color: #E4E6EF !important;
        color: #A1A5B7 !important;
    }

    .wizard-step.disabled[style*="cursor: not-allowed"] .step-label,
    .wizard-step.disabled[style*="cursor: not-allowed"] .step-desc {
        color: #A1A5B7 !important;
    }
    ...
    .rating-input:disabled + .rating-card {
        opacity: 0.6;
        cursor: not-allowed;
        pointer-events: none;
    }
    ...
    <div class="card mb-5 mb-xl-10" id="kt_profile_details_view">
            <div class="card-header cursor-pointer">
                <div class="card-title m-0">
                    ...
                    <div class="card-body">
            <!-- Alert Section -->
            @if($pengajuan->status_penilaian_prodi === 'approved')
                <div class="alert alert-info d-flex align-items-start p-5 mb-10">
                    <div class="me-4">
                        <i class="bi bi-info-circle-fill fs-2 text-info"></i>
                    </div>
                    <div class="flex-grow-1">
                        <h4 class="fw-bold text-dark mb-2">📋 Penilaian Instrumen Prodi Telah Disetujui</h4>
                        <div class="fs-6 text-gray-700">
                            <p class="mt-4">
                                <strong>Status:</strong>
                                <span class="fw-semibold text-info">
                                    Penilaian instrumen prodi telah disetujui dan tidak dapat diubah lagi.
                    ...
                    <div class="mb-8">
                        ...
                        <div class="wizard-nav">
                            @foreach($allKriterias as $kriteria)
                                ...
                                @endphp
                                <div class="wizard-step {{ $stepClass }} {{ $pengajuan->status_penilaian_prodi === 'approved' ? 'disabled' : '' }}"
                                     data-kriteria="{{ $kriteriaId }}"
                                     data-accessible="{{ $isAccessible ? 'true' : 'false' }}"
                                     onclick="{{ $pengajuan->status_penilaian_prodi === 'approved' ? 'void(0)' : 'showKriteriaContent(\'' . $kriteriaId . '\', ' . ($isAccessible ? 'true' : 'false') . ')' }}"
                                     title="{{ $pengajuan->status_penilaian_prodi === 'approved' ? 'Penilaian sudah disetujui, tidak dapat diubah' : ($isKriteriaCompleted ? 'Kriteria selesai' : ($isAccessible ? 'Kriteria dapat diakses' : 'Selesaikan kriteria sebelumnya terlebih dahulu')) }}"
                                     style="{{ $pengajuan->status_penilaian_prodi === 'approved' ? 'cursor: not-allowed; opacity: 0.6;' : '' }}">
                    ...
                    @foreach($allKriterias as $kriteria)
                        <div class="kriteria-content {{ $kriteria['id'] == $activeKriteriaId ? 'active' : '' }}" id="instrumen-group-{{ $kriteria['id'] }}" style="display: {{ $kriteria['id'] == $activeKriteriaId ? 'block' : 'none' }}">
                                                                ...
                                                                <div class="rating-container">
                                                                    <div class="row g-3">
                                                                        <div class="col-6 col-md-3">
                                                                            <input type="radio" name="nilai[{{ $instrumenProdi->id }}]" value="4" id="nilai4_{{ $instrumenProdi->id }}" class="rating-input" {{ ($instrumenProdi->nilaiAuditor && $instrumenProdi->nilaiAuditor->first() && $instrumenProdi->nilaiAuditor->first()->nilai == 4) ? 'checked' : '' }} {{ $pengajuan->status_penilaian_prodi === 'approved' ? 'disabled' : '' }} required>
                                                                            <label for="nilai4_{{ $instrumenProdi->id }}" class="rating-card {{ $pengajuan->status_penilaian_prodi === 'approved' ? 'disabled' : '' }}">
                                                                        ...
                                                                        <div class="col-6 col-md-3">
                                                                            <input type="radio" name="nilai[{{ $instrumenProdi->id }}]" value="3" id="nilai3_{{ $instrumenProdi->id }}" class="rating-input" {{ ($instrumenProdi->nilaiAuditor && $instrumenProdi->nilaiAuditor->first() && $instrumenProdi->nilaiAuditor->first()->nilai == 3) ? 'checked' : '' }} {{ $pengajuan->status_penilaian_prodi === 'approved' ? 'disabled' : '' }} required>
                                                                            <label for="nilai3_{{ $instrumenProdi->id }}" class="rating-card {{ $pengajuan->status_penilaian_prodi === 'approved' ? 'disabled' : '' }}">
                                                                        ...
                                                                        <div class="col-6 col-md-3">
                                                                            <input type="radio" name="nilai[{{ $instrumenProdi->id }}]" value="2" id="nilai2_{{ $instrumenProdi->id }}" class="rating-input" {{ ($instrumenProdi->nilaiAuditor && $instrumenProdi->nilaiAuditor->first() && $instrumenProdi->nilaiAuditor->first()->nilai == 2) ? 'checked' : '' }} {{ $pengajuan->status_penilaian_prodi === 'approved' ? 'disabled' : '' }} required>
                                                                            <label for="nilai2_{{ $instrumenProdi->id }}" class="rating-card {{ $pengajuan->status_penilaian_prodi === 'approved' ? 'disabled' : '' }}">
                                                                        ...
                                                                        <div class="col-6 col-md-3">
                                                                            <input type="radio" name="nilai[{{ $instrumenProdi->id }}]" value="1" id="nilai1_{{ $instrumenProdi->id }}" class="rating-input" {{ ($instrumenProdi->nilaiAuditor && $instrumenProdi->nilaiAuditor->first() && $instrumenProdi->nilaiAuditor->first()->nilai == 1) ? 'checked' : '' }} {{ $pengajuan->status_penilaian_prodi === 'approved' ? 'disabled' : '' }} required>
                                                                            <label for="nilai1_{{ $instrumenProdi->id }}" class="rating-card {{ $pengajuan->status_penilaian_prodi === 'approved' ? 'disabled' : '' }}">
                                                            ...
                                                            <div class="notes-section">
                                                                ...
                                                                <div class="position-relative">
                                                                    <textarea name="catatan[{{ $instrumenProdi->id }}]"
                                                                              ...
                                                                              rows="4"
                                                                              placeholder="Tambahkan catatan, saran, atau observasi khusus untuk instrumen ini..."
                                                                              {{ $pengajuan->status_penilaian_prodi === 'approved' ? 'disabled' : '' }}>{{ $instrumenProdi->nilaiAuditor && $instrumenProdi->nilaiAuditor->first() ? $instrumenProdi->nilaiAuditor->first()->catatan : '' }}</textarea>
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-8" style="display: {{ $kriteria['id'] == $activeKriteriaId ? 'flex' : 'none' }};">
                                ...
                                @if($isLastKriteria)
                                    <button type="button" class="btn btn-success {{ $pengajuan->status_penilaian_prodi === 'approved' ? 'disabled' : '' }}" onclick="{{ $pengajuan->status_penilaian_prodi === 'approved' ? 'void(0)' : 'saveAndFinish()' }}" {{ $pengajuan->status_penilaian_prodi === 'approved' ? 'style=cursor:not-allowed;opacity:0.65;' : '' }}>
                                        <i class="fas fa-check me-2"></i>
                                        Simpan & Selesai
                                    </button>
                                @else
                                    <button type="button" class="btn btn-primary {{ $pengajuan->status_penilaian_prodi === 'approved' ? 'disabled' : '' }}" onclick="{{ $pengajuan->status_penilaian_prodi === 'approved' ? 'void(0)' : 'saveAndContinue(\'' . $nextKriteria['id'] . '\', ' . ($nextKriteria['accessible'] ? 'true' : 'false') . ')' }}" {{ $pengajuan->status_penilaian_prodi === 'approved' ? 'style=cursor:not-allowed;opacity:0.65;' : '' }}>
...
<script>
    ...
    function showKriteriaContent(kriteriaId, isAccessible) {
        ...
        setTimeout(() => {
            console.log('Scrolling to kriteria content...');
            const activeContent = document.querySelector('.kriteria-content.active');
            if (activeContent) {
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire({
                            ...
                            // Get the submitted data for progress update
                            const submittedData = {};
                            formData.forEach((value, key) => {
                                if (key.startsWith('nilai[')) {
                                    const instrumenId = key.replace('nilai[', '').replace(']', '');
                                    submittedData[instrumenId] = value;
                                }
                            });

                            updateKriteriaStepStatus(currentKriteriaId, true);
                            forceNavigateToKriteria(nextKriteriaId);
                            // Update progress bar with submitted data
                            updateProgressBar({ nilai: submittedData });
                            // Tambahkan scroll ke atas setelah navigasi
                            setTimeout(() => {
                                const activeContent = document.querySelector('.kriteria-content.active');
                                if (activeContent) {
                                    activeContent.scrollIntoView({ behavior: 'smooth', block: 'start' });
    ...
    function saveAndFinish() {
        // Check if penilaian is already approved
        if ({{ $pengajuan->status_penilaian_prodi === 'approved' ? 'true' : 'false' }}) {
            Swal.fire({
                icon: 'info',
                title: 'Penilaian Sudah Disetujui',
                text: 'Penilaian instrumen prodi telah disetujui dan tidak dapat diubah lagi.',
                confirmButtonText: 'OK'
            });
            return;
        }
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire({
                            ...
                            // Update alert section to show completion status
                            updateAlertSection();

                            // Stay on current kriteria and scroll to content
                            setTimeout(() => {
                                console.log('Scrolling after save and finish...');

                                // Try multiple scroll targets
                                const scrollTargets = [
                                    document.querySelector('.kriteria-content.active'),
                                    document.querySelector('.kriteria-content'),
                                    document.querySelector('#instrumen-group-' + currentKriteriaId),
                                ...
                                if (!scrolled) {
                                    console.log('No scroll target found, using window.scrollTo');
    ...
    function updateProgressBar(submittedData = null) {
        const currentKriteriaId = $('.kriteria-content.active').attr('id').replace('instrumen-group-', '');

        // If we have submitted data, use it to update progress
        if (submittedData && submittedData.nilai) {
            // Update the current kriteria step description based on submitted data
            const totalInstrumen = Object.keys(submittedData.nilai).length;
            const completedInstrumen = totalInstrumen; // All submitted are considered completed

            // Update kriteria step description
            $(`.wizard-step[data-kriteria="${currentKriteriaId}"] .step-desc`).text(`${completedInstrumen}/${totalInstrumen} Instrumen`);

            // Mark this kriteria as completed
            updateKriteriaStepStatus(currentKriteriaId, true);
        } else {
            // Fallback to counting radio buttons
            const totalInstrumen = $(`#instrumen-group-${currentKriteriaId} .rating-input`).length;
            const completedInstrumen = $(`#instrumen-group-${currentKriteriaId} .rating-input:checked`).length;

            // Update kriteria step description
            $(`.wizard-step[data-kriteria="${currentKriteriaId}"] .step-desc`).text(`${completedInstrumen}/${totalInstrumen} Instrumen`);
        }

        // Calculate overall progress across all kriterias
        let totalAllInstrumen = 0;
        let totalAllCompleted = 0;

        $('.wizard-step').each(function() {
            const kriteriaId = $(this).data('kriteria');
            const stepDesc = $(this).find('.step-desc').text();
            const match = stepDesc.match(/(\d+)\/(\d+)/);
            if (match) {
                const completed = parseInt(match[1]);
                const total = parseInt(match[2]);
                totalAllCompleted += completed;
                totalAllInstrumen += total;
            }
        });

        const overallProgress = totalAllInstrumen > 0 ? (totalAllCompleted / totalAllInstrumen) * 100 : 0;

        // Update main progress bar
        $('.bg-primary.rounded.h-8px').css('width', `${overallProgress}%`);
        $('.fw-bold.text-gray-800.fs-6').text(`${overallProgress.toFixed(1)}%`);

        // Update progress text
        if (totalAllCompleted === totalAllInstrumen) {
            $('.fw-semibold.text-gray-600.me-2').text('Semua instrumen telah selesai dinilai');
        } else {
            $('.fw-semibold.text-gray-600.me-2').text(`${totalAllCompleted} instrumen selesai dinilai`);
        }

        console.log('Progress updated:', {
            kriteriaId: currentKriteriaId,
            overallProgress: overallProgress,
            totalAllCompleted: totalAllCompleted,
            totalAllInstrumen: totalAllInstrumen,
    ...
    function updateAlertSection() {
        // Calculate if all kriteria are completed
        let totalAllInstrumen = 0;
        let totalAllCompleted = 0;
        ...
        const isAllCompleted = totalAllCompleted === totalAllInstrumen && totalAllInstrumen > 0;

        // Update alert section
        const alertSection = $('.alert');
        if (isAllCompleted) {
            alertSection.removeClass('alert-danger').addClass('alert-success');
            alertSection.find('.bi').removeClass('bi-exclamation-triangle-fill fs-2 text-danger').addClass('bi-check-circle-fill fs-2 text-success');
            alertSection.find('h4').text('✨ Penilaian Instrumen Prodi Selesai');
            alertSection.find('.fw-semibold').removeClass('text-danger').addClass('text-success');
            alertSection.find('.fw-semibold').html(`${totalAllCompleted} dari ${totalAllInstrumen} instrumen telah dinilai dengan lengkap.<br>Silakan klik tombol <strong>Setujui</strong> untuk melanjutkan ke tahap selanjutnya.`);
        } else {
            alertSection.removeClass('alert-success').addClass('alert-danger');
            alertSection.find('.bi').removeClass('bi-check-circle-fill fs-2 text-success').addClass('bi-exclamation-triangle-fill fs-2 text-danger');
            alertSection.find('h4').text('📊 Penilaian Instrumen Prodi');
            alertSection.find('.fw-semibold').removeClass('text-success').addClass('text-danger');
            alertSection.find('.fw-semibold').html(`${totalAllCompleted} dari ${totalAllInstrumen} instrumen telah dinilai.<br>Silakan lengkapi penilaian yang tersisa.`);
        }

        console.log('Alert section updated:', { isAllCompleted, totalAllCompleted, totalAllInstrumen });
    }
</script>
@endpush
