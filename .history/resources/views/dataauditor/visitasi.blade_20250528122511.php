...
@php
    ...
    if ($dataIkss->isNotEmpty()) {
        foreach ($dataIkss as $item) {
            $kodeIkss = $item->instrumen->indikatorKinerja->kode_ikss;

            // Initialize array for this kode_ikss if not exists
            if (!isset($ikssCompletionStatus[$kodeIkss])) {
            ...
            $ikssCompletionStatus[$kodeIkss]['total']++;

            // Check if this item has visitasi data
            if (isset($visitasi[$item->id])) {
                $ikssCompletionStatus[$kodeIkss]['completed']++;
                if ($ikssCompletionStatus[$kodeIkss]['completed'] == $ikssCompletionStatus[$kodeIkss]['total']) {
                    $ikssCompletionStatus[$kodeIkss]['is_completed'] = true;
        ...
        <div class="card-body">
            ...
            @if($dataIkss->isNotEmpty())
                ...
                <div class="wizard-nav">
                    @foreach($ikssCompletionStatus as $kodeIkss => $status)
                        @php
                            $isCompleted = $status['is_completed'];
                            $isActive = $kodeIkss == $activeStep;
                            $stepClass = $isCompleted ? 'completed' : ($isActive ? 'active' : 'disabled');
                        @endphp
                        <div class="wizard-step {{ $stepClass }}" data-step="{{ $kodeIkss }}">
                            <div class="step-number">{{ $loop->iteration }}</div>
                            <div class="step-label">{{ $kodeIkss }}</div>
                            <div class="step-desc">
                ...
                <div class="d-flex flex-column mb-10">
                    ...
                    <div class="d-flex align-items-center mb-2">
                        <span class="fs-6 fw-semibold text-gray-600">
                            ...
                            @else
                                {{ $completedInstruments }} dari {{ $totalInstruments }} instrumen selesai dievaluasi
                            @endif
                        </span>
                    </div>
                    ...
                    <div class="wizard-sections">
                        @foreach($ikssCompletionStatus as $kodeIkss => $status)
                            @php
                                ...
                                $isCompleted = $status['is_completed'];
                            @endphp
                            <div class="wizard-content {{ $isActive ? 'active' : '' }}" id="step-{{ $kodeIkss }}">
                                <div class="alert {{ $isCompleted ? 'alert-success' : 'alert-danger' }} d-flex flex-column flex-sm-row p-5 mb-10">
                                    ...
                                    <div class="d-flex flex-column pe-0 pe-sm-10">
                                        ...
                                        <div class="fs-6">
                                            <div class="fw-semibold text-gray-700">{{ $kodeIkss }} - {{ $ikssCompletionStatus[$kodeIkss]['completed'] }}/{{ $ikssCompletionStatus[$kodeIkss]['total'] }} instrumen dievaluasi lengkap</div>
                                        </div>
                                    </div>
                                </div>

                                @foreach ($dataIkss->where('instrumen.indikatorKinerja.kode_ikss', $kodeIkss) as $index => $ikssAuditee)
                                    @php
                                    ...
                                    <div class="card mb-5 {{ $hasEvaluation ? 'border-success' : '' }}">
                                        <div class="card-header {{ $hasEvaluation ? 'bg-success-subtle' : 'bg-light' }}">
                                            <div class="d-flex justify-content-between align-items-center">
                                                <div>
                                                    <h6 class="text-muted mb-1">Instrumen #{{ $index + 1 }}</h6>
