...
@php
    ...
    foreach ($dataIkss as $ikssAuditee) {
        ...
        // Check if this IKSS is evaluated
        ...
        <div class="card-body">
            ...
            @if($dataIkss->isNotEmpty())
                ...
                <form action="{{ route('auditor.audit.submitDeskEvaluation') }}" method="POST" enctype="multipart/form-data" id="formDeskEvaluation">
                    ...
                    @foreach($groupedIkss as $ssId => $group)
                        @php
                            $isActive = !$ssCompletionStatus[$ssId]['is_completed'] &&
                                      (!$lastCompletedStep || $lastCompletedStep == $ssId);
                        @endphp
...
<script>
    $(document).ready(function() {
        ...
        function initializeWizard() {
            // Show first step by default if none is active
            if ($('.wizard-content.active').length === 0) {
                const firstStepId = $('.wizard-step:first').data('step');
        ...
        function showStep(stepId) {
            ...
            $('.wizard-content').removeClass('active').css('opacity', 0);

            // Show selected content section with animation
            $(`#step-${stepId}`).addClass('active').css('opacity', 1);
            ...
            $('html, body').animate({
                scrollTop: $(`#step-${stepId}`).offset().top - 100
            }, 300);
