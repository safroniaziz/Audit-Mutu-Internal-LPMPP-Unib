...
@section('dashboardProfile')
    ...
    <div class="card shadow-sm border-0">
        ...
        <div class="card-body pt-0">
            ...
            <!-- Jaring Laba-laba Instrumen RSB -->
                ...
                <div class="card-header cursor-pointer pt-5 pb-3">
                    <div class="card-title m-0">
                        <h3 class="fw-semibold m-0 text-dark d-flex align-items-center gap-2">
                            ...
                            Jaring Laba-laba Instrumen RSB (Evaluasi)
                        </h3>
                    </div>
                </div>
                <div class="card-body pt-0">
                        ...
                        <div class="row g-6">
                            <div class="col-lg-8">
                                <div class="bg-light p-6 rounded">
                                    <canvas id="spiderChartRSB" width="400" height="400"></canvas>
                                </div>
                            </div>
                            <div class="col-lg-4">
                                <!-- Skala Penilaian -->
                                <div class="mb-6">
                                    <h6 class="fw-bold text-dark mb-3">Skala Penilaian</h6>
                                ...
                                <!-- Statistik RSB -->
                                <div class="mb-6">
                                    <h6 class="fw-bold text-dark mb-3">Statistik RSB</h6>
                                    @php
                                        $totalNilaiRSB = $evaluasiSubmissions->sum('nilai');
                                        $rataRataRSB = $evaluasiSubmissions->count() > 0 ? round($totalNilaiRSB / $evaluasiSubmissions->count(), 2) : 0;
                                        $nilaiTertinggiRSB = $evaluasiSubmissions->max('nilai');
                                        $nilaiTerendahRSB = $evaluasiSubmissions->min('nilai');
                                    @endphp
                                    <div class="d-flex flex-column gap-3">
                                        <div class="d-flex justify-content-between">
                                            <span class="fs-7 text-muted">Rata-rata:</span>
                                            <span class="fw-bold text-primary">{{ $rataRataRSB }}</span>
                                        </div>
                                        <div class="d-flex justify-content-between">
                                            <span class="fs-7 text-muted">Tertinggi:</span>
                                            <span class="fw-bold text-success">{{ $nilaiTertinggiRSB }}</span>
                                        </div>
                                        <div class="d-flex justify-content-between">
                                            <span class="fs-7 text-muted">Terendah:</span>
                                            <span class="fw-bold text-danger">{{ $nilaiTerendahRSB }}</span>
                                        </div>
                                        <div class="d-flex justify-content-between">
                                            <span class="fs-7 text-muted">Total Evaluasi:</span>
                                            <span class="fw-bold text-dark">{{ $evaluasiSubmissions->count() }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    @else
                        <div class="alert alert-info text-center py-8">
                            ...
                            <p class="text-muted mb-0">Lakukan evaluasi RSB terlebih dahulu untuk melihat jaring laba-laba nilai auditor.</p>
                        </div>
                    @endif
                </div>
            </div>

            <!-- Jaring Laba-laba Instrumen Prodi -->
            <div class="card card-bordered shadow-sm mb-8">
                <div class="card-header cursor-pointer pt-5 pb-3">
                    <div class="card-title m-0">
                        <h3 class="fw-semibold m-0 text-dark d-flex align-items-center gap-2">
                            <i class="fas fa-chart-line text-danger fs-2"></i>
                            Jaring Laba-laba Instrumen Prodi (Kuisioner)
                        </h3>
                    </div>
                </div>
                <div class="card-body pt-0">
                    @if($jawabanKuisioner->count() > 0)
                        <div class="row g-6">
                            <div class="col-lg-8">
                                <div class="bg-light p-6 rounded">
                                    <canvas id="spiderChartProdi" width="400" height="400"></canvas>
                                </div>
                            </div>
                            <div class="col-lg-4">
                                <!-- Skala Penilaian -->
                                ...
                                <!-- Statistik Prodi -->
                                <div class="mb-6">
                                    <h6 class="fw-bold text-dark mb-3">Statistik Prodi</h6>
                                    @php
                                        $totalKuisioner = $jawabanKuisioner->count();
                                        $totalNilaiProdi = 0;
                                        ...
                                        $rataRataProdi = $totalKuisioner > 0 ? round($totalNilaiProdi / $totalKuisioner, 2) : 0;
                                    @endphp
                                    <div class="d-flex flex-column gap-3">
                                        <div class="d-flex justify-content-between">
                                            <span class="fs-7 text-muted">Rata-rata:</span>
                                            <span class="fw-bold text-danger">{{ $rataRataProdi }}</span>
                                        </div>
                                        <div class="d-flex justify-content-between">
                                            <span class="fs-7 text-muted">Total Kuisioner:</span>
                                            <span class="fw-bold text-dark">{{ $totalKuisioner }}</span>
                                        </div>
                                        <div class="d-flex justify-content-between">
                                            <span class="fs-7 text-muted">Total Nilai:</span>
                                            <span class="fw-bold text-info">{{ $totalNilaiProdi }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    @else
                        <div class="alert alert-info text-center py-8">
                            <i class="fas fa-chart-line fs-1 text-info mb-3"></i>
                            <h5 class="text-dark mb-2">Belum Ada Data Kuisioner Prodi</h5>
                            <p class="text-muted mb-0">Lakukan pengisian kuisioner prodi terlebih dahulu untuk melihat jaring laba-laba nilai auditor.</p>
                        </div>
                    @endif
                </div>
            </div>
...
<script>
    ...
    if (beritaAcaraForm) {
        ...
        // Spider Chart untuk Jaring Laba-laba Nilai Auditor
    @if($evaluasiSubmissions->count() > 0)
        const spiderChartCtx = document.getElementById('spiderChart');
        if (spiderChartCtx) {
            // Data untuk chart
            ...
            const labels = evaluasiData.map(e => e.evaluasi.length > 30 ? e.evaluasi.substring(0, 30) + '...' : e.evaluasi);

            // Data untuk Instrumen RSB (Evaluasi)
            ...
            // Buat spider chart dengan 2 dataset
            new Chart(spiderChartCtx, {
                type: 'radar',
                data: {
                    labels: labels,
                    datasets: [
                        ...
                        tooltip: {
                            callbacks: {
                                label: function(context) {
                                    ...
                                    return `${datasetLabel}: ${nilai} (${label})`;
                                }
                            }
                        }
                    }
