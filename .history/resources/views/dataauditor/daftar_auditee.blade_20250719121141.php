                ...
                <div class="row g-4" id="auditeeCards">
                    @forelse($auditess as $index => $auditee)
                        <div class="col-xl-4 col-lg-6 col-md-6 audit-card"
                            ...
                            <div class="card h-100 shadow-sm hover-elevate-up {{ $auditee->audit_status['status'] === 'new' ? 'border-primary border-2' : '' }}">
                                <!--begin::Header-->
                                    ...
                                    <div class="d-flex align-items-center justify-content-between">
                                        ...
                                        <div class="text-end">
                                            @if($auditee->audit_status['status'] === 'new')
                                                <div class="badge badge-light-danger fs-7 mb-1">
                                            ...
                                            @elseif($auditee->audit_status['status'] === 'in_progress')
                                                <div class="badge badge-light-warning fs-7 mb-1">
                                                    <i class="fas fa-clock me-1"></i>
                                                    Dalam Proses
                                                </div>
                                            @elseif($auditee->audit_status['status'] === 'visitasi_waiting')
                                                <div class="badge badge-light-info fs-7 mb-1">
                                                    <i class="fas fa-calendar me-1"></i>
                                                    Menunggu Jadwal
                                                </div>
                                            @elseif($auditee->audit_status['status'] === 'visitasi_expired')
                                                <div class="badge badge-light-danger fs-7 mb-1">
                                            ...
                                            <div class="d-flex align-items-center">
                                                <div class="symbol symbol-30px me-2">
                                                    <span class="symbol-label bg-light-{{ $auditee->audit_status['color'] }}">
                                                        <i class="{{ $auditee->audit_status['icon'] }} text-{{ $auditee->audit_status['color'] }} fs-6"></i>
                                                    </span>
                                                </div>
                                                <div class="d-flex flex-column">
                                                    <span class="text-muted fw-semibold fs-8">Status</span>
                                                    <span class="fw-bold fs-7">{{ $auditee->audit_status['label'] }}</span>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-6">
                                            <div class="d-flex align-items-center">
                                                <div class="symbol symbol-30px me-2">
                                                    <span class="symbol-label bg-light-warning">
                                                        <i class="fas fa-calendar-alt text-warning fs-6"></i>
                                                    </span>
                                                </div>
                                                <div class="d-flex flex-column">
                                                    <span class="text-muted fw-semibold fs-8">Periode</span>
                                                    <span class="fw-bold fs-7">{{ $auditee->periodeAktif->siklus ?? 'N/A' }}/{{ $auditee->periodeAktif->tahun_ami ?? 'N/A' }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="d-flex flex-column">
                                        ...
                                        @if(config('app.debug'))
                                            ...
                                            <div class="alert alert-info p-2 mb-2">
                                                <small>
                                                    ...
                                                    Pengajuan ID: {{ $auditee->id }}<br>
                                                    Auditee: {{ $auditee->auditee->nama_unit_kerja }}<br>
                                                    Total Auditors: {{ $auditee->auditors->count() }}<br>
                                                    Overall Status: {{ $auditee->overall_audit_status }}<br>
                                                    Individual Status: {{ $auditee->audit_status['status'] }}<br>
                                                    @foreach($auditee->auditors as $debugAuditor)
                                                        - {{ $debugAuditor->auditor->name }} ({{ $debugAuditor->role }})<br>
                                                    @endforeach
                                                </small>
                                            </div>
                                            ...
                                            <div class="alert alert-success p-2 mb-2">
                                                <small>
                                                    ...
                                                    @foreach($auditee->auditors as $debugAuditor)
                                                        @php
                                                            $evalCount = \App\Models\EvaluasiSubmission::where('pengajuan_ami_id', $auditee->id)
                                                                ->where('user_id', $debugAuditor->user_id)
                                                                ->where('jenis', 'auditor')
                                                                ->count();
                                                            $masukanCount = \App\Models\EvaluasiMasukan::where('pengajuan_ami_id', $auditee->id)
                                                                ...
                                                                ->count();
                                                        @endphp
                                                        <strong>{{ $debugAuditor->auditor->name }}:</strong> Evaluasi: {{ $evalCount }}, Masukan: {{ $masukanCount }}<br>
                                                    @endforeach
                                                </small>
                                            </div>
                                        @endif
                                        ...
                                        @forelse ($auditee->auditors as $auditor)
                                            @php
                                                ...
                                                // Check if this auditor has completed their audit
                                                ...
                                                // Additional check for desk evaluation completion
                                                $deskEvaluationCount = \App\Models\IkssAuditeeNilai::where('pengajuan_ami_id', $auditee->id)
                                                    ->where('auditor_id', $auditor->user_id)
                                                    ->count();

                                                // Check for visitasi completion
                                                $visitasiCount = \App\Models\IkssAuditeeVisitasi::where('pengajuan_ami_id', $auditee->id)
                                                    ->where('auditor_id', $auditor->user_id)
                                                    ->count();

                                                // Option 1: Simple logic - if audit is completed overall, all auditors are completed
                                                if ($auditee->overall_audit_status === 'completed') {
                                                    $auditorCompleted = true;
                                                }
                                                // Option 2: Individual auditor completion check
                                                else {
                                                    $auditorCompleted = ($evaluasiCount > 0 && $masukanCount > 0) ||
                                                                       ($deskEvaluationCount > 0 && $visitasiCount > 0 && $evaluasiCount > 0 && $masukanCount > 0);
                                                }

                                                // If auditor has done any work, show as in progress
                                                $hasStarted = $deskEvaluationCount > 0 || $visitasiCount > 0 || $evaluasiCount > 0 || $masukanCount > 0;

                                                // Debug info for this auditor
                                                if(config('app.debug')) {
                                                    $debugInfo = [
                                                        'auditor_name' => $auditor->auditor->name,
                                                        'role' => $auditor->role,
                                                        'desk_evaluation' => $deskEvaluationCount,
                                                        'visitasi' => $visitasiCount,
                                                        'evaluasi' => $evaluasiCount,
                                                        'masukan' => $masukanCount,
                                                        'completed' => $auditorCompleted,
                                                        'has_started' => $hasStarted,
                                                        'audit_status_overall' => $auditee->overall_audit_status
                                                    ];
                                                }
                                            @endphp

                                            @if(config('app.debug'))
                                                <!-- Debug Info per Auditor -->
                                                <div class="alert alert-warning p-1 mb-1">
                                                    <small>
                                                        <strong>{{ $auditor->auditor->name }}:</strong><br>
                                                        Desk: {{ $deskEvaluationCount }}, Visitasi: {{ $visitasiCount }},
                                                        Evaluasi: {{ $evaluasiCount }}, Masukan: {{ $masukanCount }}<br>
                                                        Completed: {{ $auditorCompleted ? 'Yes' : 'No' }},
                                                        Started: {{ $hasStarted ? 'Yes' : 'No' }}<br>
                                                        Overall Status: {{ $auditee->overall_audit_status }}
                                                    </small>
                                                </div>
                                            @endif
                                ...
                                <div class="card-footer pt-0">
                                    <div class="d-flex gap-2">
                                        @if($auditee->audit_status['status'] === 'visitasi_waiting' || $auditee->audit_status['status'] === 'visitasi_expired')
                                            <button class="btn btn-sm btn-{{ $auditee->audit_status['status'] === 'visitasi_waiting' ? 'warning' : 'danger' }} flex-grow-1" disabled>
                                                <i class="fas fa-{{ $auditee->audit_status['status'] === 'visitasi_waiting' ? 'clock' : 'exclamation-triangle' }} me-1"></i>
                                        ...
                                        @else
                                            <a href="{{ route('auditor.audit.perjanjianKinerja', $auditee->id) }}"
                                               class="btn btn-sm btn-{{ $auditee->audit_status['status'] === 'new' ? 'primary' : 'light-primary' }} flex-grow-1">
                                                <i class="fas fa-{{ $auditee->audit_status['status'] === 'new' ? 'arrow-right' : 'file-alt' }} me-1"></i>
                                                @if($auditee->audit_status['status'] === 'new')
                                                    Mulai Audit
                                                @elseif($auditee->audit_status['status'] === 'completed')
                                                    Lihat Hasil
