            ...
            @if($dataIkss->isNotEmpty())
                ...
                <form action="{{ route('auditor.audit.submitVisitasi') }}" method="POST" enctype="multipart/form-data" id="formVisitasi">
                    ...
                    <div class="wizard-sections">
                        @foreach($ikssCompletionStatus as $satuanStandarId => $status)
                            <div class="wizard-content {{ $satuanStandarId == $activeStep ? 'active' : '' }}"
                                 id="step-{{ $satuanStandarId }}">

                                <div class="alert {{ $status['is_completed'] ? 'alert-success' : 'alert-info' }} d-flex flex-column flex-sm-row p-5 mb-10">
                                    <span class="svg-icon svg-icon-2hx {{ $status['is_completed'] ? 'svg-icon-success' : 'svg-icon-info' }} me-4 mb-5 mb-sm-0">
                                    ...
                                    <div class="d-flex flex-column pe-0 pe-sm-10">
                                        <h5 class="mb-1">{{ $status['satuan_standar']->sasaran }}</h5>
                                        <div class="fs-6">
                                            <div class="fw-semibold text-gray-700">
                                                Progress: {{ $status['completed'] }}/{{ $status['total'] }} instrumen selesai
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                @foreach($status['ikss'] as $kodeIkss => $ikssData)
                                    <div class="card mb-5">
                                        <div class="card-header">
                                            <h4 class="card-title">{{ $kodeIkss }}</h4>
