...
@section('dashboardProfile')
    ...
    <!-- Modal Laporan AMI yang Sudah Diperbaiki -->
    <!-- Modal Laporan AMI dengan Desain Profesional -->
<div class="modal fade" id="laporanAmiModal" tabindex="-1" aria-labelledby="laporanAmiModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-light">
                <h5 class="modal-title" id="laporanAmiModalLabel">
                    <i class="fas fa-file-alt me-2 text-primary"></i>Cetak Laporan Audit Mutu Internal
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('auditor.audit.laporanAmi', [$pengajuan->id]) }}" method="POST">
                @csrf
                <div class="modal-body">
                    <h6 class="text-secondary mb-3">Evaluasi Sistem Penjaminan Mutu Internal</h6>
                    ...
                    <table class="table table-hover">
                        <tbody>
                            ...
                            @foreach ($pertanyaan as $index => $item)
                                <tr>
                                    <td class="text-muted" width="5%">{{ $index + 1 }}.</td>
                                    <td width="60%">{{ $item['label'] }}</td>
                                    <td width="35%">
                                        <div class="d-flex align-items-center">
                                            @foreach ($item['opsi'] as $indexOpsi => $opsi)
                                                <div class="form-check me-3">
                                                    <input
                                                        ...
                                                        required
                                                    >
                                                    <label class="form-check-label" for="{{ $item['name'] }}_{{ $opsi }}">
                                                        {{ $opsi }}
                                                    </label>
                                                </div>
                                            @endforeach
