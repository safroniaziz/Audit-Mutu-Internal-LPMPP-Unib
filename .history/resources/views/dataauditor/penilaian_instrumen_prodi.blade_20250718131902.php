...
@php
    ...
    foreach ($groupedData as $indikatorData) {
        foreach ($indikatorData['kriterias'] as $kriteriaId => $kriteriaData) {
            ...
            // Determine if this kriteria is accessible
            $isAccessible = $previousCompleted;

            // Set active kriteria (first incomplete or first kriteria)
            if ($activeKriteriaId === null && (!$isKriteriaCompleted || $loop->first)) {
                    ...
                    @foreach($allKriterias as $kriteria)
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-8">
                            ...
                            @if($nextKriteria && $isKriteriaCompleted)
                                <button type="button" class="btn btn-success" onclick="showKriteriaContent('{{ $nextKriteria['id'] }}', {{ $nextKriteria['accessible'] ? 'true' : 'false' }})">
                                    Kriteria Berikutnya
                                    <i class="fas fa-arrow-right ms-2"></i>
                                </button>
                            @elseif($nextKriteria && !$isKriteriaCompleted)
                                <button type="button" class="btn btn-secondary disabled" disabled>
                                    Selesaikan Kriteria Ini Dulu
...
<script>
    function showKriteriaContent(kriteriaId, isAccessible) {
        ...
        if (!isAccessible) {
            Swal.fire({
                icon: 'warning',
                title: 'Kriteria Belum Dapat Diakses',
