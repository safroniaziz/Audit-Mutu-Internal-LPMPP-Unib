                                        ...
                                        <tr class="{{ $auditee->audit_status['status'] === 'new' ? 'table-primary' : '' }}">
                                                ...
                                                @forelse ($auditee->auditors as $auditor)
                                                    @php
                                                        ...
                                                        if ($auditor->user_id == Auth::user()->id) {
                                                            // For current user, use the audit status
                                                            $auditorCompleted = $auditee->audit_status['status'] === 'completed';
                                                        } else {
                                                            // For other auditors, check their completion status
                                                    ...
                                                    <div class="d-flex align-items-start mb-2">
                                                        ...
                                                        <div class="d-flex flex-column flex-grow-1 min-w-0">
                                                            <div class="d-flex align-items-center">
                                                                ...
                                                                @if($auditorCompleted)
                                                                    <i class="fas fa-check-circle fs-0 text-success ms-1" title="Audit Selesai"></i>
                                                                @endif
                                                            </div>
                                            ...
                                            <td>
                                                @if($auditee->audit_status['status'] === 'visitasi_waiting' || $auditee->audit_status['status'] === 'visitasi_expired')
                                                    <button class="btn btn-sm btn-{{ $auditee->audit_status['status'] === 'visitasi_waiting' ? 'warning' : 'danger' }} px-4" disabled>
                                                        <i class="fas fa-{{ $auditee->audit_status['status'] === 'visitasi_waiting' ? 'clock' : 'exclamation-triangle' }} fs-2 me-1"></i>
                                                ...
                                                @else
                                                    <a href="{{ route('auditor.audit.perjanjianKinerja', $auditee->id) }}"
                                                       class="btn btn-sm btn-{{ $auditee->audit_status['status'] === 'new' ? 'primary' : 'light-primary' }} px-4">
                                                        <i class="fas fa-{{ $auditee->audit_status['status'] === 'new' ? 'arrow-right' : 'file-alt' }} fs-2 me-1"></i>
