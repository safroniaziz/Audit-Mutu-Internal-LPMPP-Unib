                            ...
                            <div class="wizard-content {{ $isActive ? 'active' : '' }}" id="step-{{ $ssId }}">
                                ...
                                @foreach($group['ikss'] as $ikssAuditee)
                                        ...
                                        $hasPartialEvaluation = isset($deskEvaluation[$ikssAuditee->id]) &&
                                            (!empty($deskEvaluation[$ikssAuditee->id]->deskripsi) ||
                                             !empty($deskEvaluation[$ikssAuditee->id]->pertanyaan) ||
                                             (isset($deskEvaluation[$ikssAuditee->id]->nilai) && $deskEvaluation[$ikssAuditee->id]->nilai !== '' && $deskEvaluation[$ikssAuditee->id]->nilai !== null && is_numeric($deskEvaluation[$ikssAuditee->id]->nilai)));
                                    @endphp
    ...
    $(document).ready(function() {
            ...
            if (validateStep(currentStep)) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        currentStep.find('textarea, select').each(function() {
                            const element = $(this);
        ...
        function displayValidationErrors(errors) {
            Object.keys(errors).forEach(function(field) {
                ...
                if (id) {
                    const input = $(`textarea[name="${fieldName}[${id}]"], select[name="${fieldName}[${id}]"]`);
        ...
        function validateStep(step) {
            ...
            const requiredFields = step.find('textarea[required]:not([disabled]), select[required]:not([disabled])');
            let isValid = true;
            let firstError = null;
        ...
        // Handle select validation on change
            ...
            if (validateStep(currentStep)) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        // PERBAIKAN: Hanya ambil data dari step yang aktif
                        const formData = new FormData();
                        formData.append('_token', '{{ csrf_token() }}');
                        formData.append('pengajuan_id', '{{ $pengajuan->id }}');

                        // Hanya ambil data dari step yang sedang aktif
                        currentStep.find('textarea, select').each(function() {
                        ...
                        // Hanya ambil ikss_auditee_ids dari step yang aktif
                        currentStep.find('input[name="ikss_auditee_ids[]"]').each(function() {
                            formData.append('ikss_auditee_ids[]', $(this).val());
                        });
