...
@section('dashboardProfile')
    ...
    <div class="card shadow-sm border-0">
        ...
        <div class="card-body pt-0">
                        ...
                        <!-- Debug Info untuk Pengajuan AMI ID: {{ $pengajuan->id }} -->
            <div class="alert alert-info mb-4">
                <h6 class="fw-bold">Debug Info untuk Pengajuan AMI ID: {{ $pengajuan->id }}</h6>

                <!-- Basic Info -->
                <div class="row mb-3">
                    <div class="col-md-6">
                        <p><strong>Pengajuan ID:</strong> {{ $pengajuan->id }}</p>
                        <p><strong>Auditee:</strong> {{ $pengajuan->auditee->nama_unit_kerja ?? 'NULL' }}</p>
                        <p><strong>Periode:</strong> {{ $pengajuan->periodeAktif->siklus ?? 'NULL' }}/{{ $pengajuan->periodeAktif->tahun_ami ?? 'NULL' }}</p>
                    </div>
                    <div class="col-md-6">
                        <p><strong>Total Evaluasi Submissions:</strong> {{ $evaluasiSubmissions->count() }}</p>
                        <p><strong>Total Kuisioners Master:</strong> {{ $kuisioners->count() }}</p>
                ...
                @if($jawabanKuisioner->count() > 0)
                    <div class="mb-3">
                        <p><strong>Sample Jawaban Kuisioner ({{ $jawabanKuisioner->count() }} data):</strong></p>
                            ...
                            <table class="table table-sm table-bordered">
                                <thead class="table-light">
                                    <tr>
                                        ...
                                        <th>Kuisioner ID</th>
                                        <th>Opsi Text</th>
                                        <th>Penugasan Auditor ID</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($jawabanKuisioner->take(5) as $jawaban)
                                        <tr>
                                            <td>{{ $jawaban->id }}</td>
                                            <td>{{ $jawaban->pengajuan_id }}</td>
                                            <td>{{ $jawaban->kuisioner_id }}</td>
                                            <td>{{ $jawaban->opsi->opsi ?? 'NULL' }}</td>
                                            <td>{{ $jawaban->penugasan_auditor_id }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                ...
                @else
                    <div class="alert alert-warning mb-3">
                        <strong>No Kuisioner Data Found!</strong><br>
                        Query: <code>SELECT * FROM kuisioner_jawabans WHERE pengajuan_id = {{ $pengajuan->id }}</code>
                    </div>
                @endif

                <!-- Instrumen Prodi Data -->
                @if($instrumenProdiNilai->count() > 0)
                    <div class="mb-3">
                        <p><strong>Sample Instrumen Prodi Nilai ({{ $instrumenProdiNilai->count() }} data):</strong></p>
                        <div class="table-responsive">
                            <table class="table table-sm table-bordered">
                                <thead class="table-light">
                                    <tr>
                                        <th>ID</th>
                                        <th>Instrumen Prodi ID</th>
                                        <th>Auditor ID</th>
                                        <th>Nilai</th>
                                        <th>Kriteria</th>
                                    </tr>
                                ...
                                <tbody>
                                    @foreach($instrumenProdiNilai->take(5) as $nilai)
                                        <tr>
                                            <td>{{ $nilai->id }}</td>
                                            <td>{{ $nilai->instrumen_prodi_id }}</td>
                                            <td>{{ $nilai->auditor_id }}</td>
                                            <td>{{ $nilai->nilai }}</td>
                                            <td>{{ $nilai->instrumenProdi->kriteriaInstrumen->nama_kriteria ?? 'NULL' }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                ...
                @else
                    <div class="alert alert-warning mb-3">
                        <strong>No Instrumen Prodi Nilai Found!</strong><br>
                        Query: <code>SELECT * FROM instrumen_prodi_nilais WHERE pengajuan_ami_id = {{ $pengajuan->id }}</code>
                    </div>
                @endif
                ...
                @if($evaluasiSubmissions->count() > 0)
                    <div class="mb-3">
                        <p><strong>Sample Evaluasi Submissions ({{ $evaluasiSubmissions->count() }} data):</strong></p>
                        <div class="table-responsive">
                            <table class="table table-sm table-bordered">
                                <thead class="table-light">
                                    <tr>
                                ...
                                <tbody>
                                    @foreach($evaluasiSubmissions->take(5) as $submission)
                                        <tr>
                                            <td>{{ $submission->id }}</td>
                                            <td>{{ $submission->evaluasi_id }}</td>
                ...
                @else
                    <div class="alert alert-warning mb-3">
                        <strong>No Evaluasi Submissions Found!</strong><br>
                        Query: <code>SELECT * FROM evaluasi_submissions WHERE pengajuan_ami_id = {{ $pengajuan->id }}</code>
                    </div>
                @endif

                <!-- Penugasan Auditor Info -->
                <div class="mb-3">
                    <p><strong>Penugasan Auditor untuk Pengajuan ini:</strong></p>
                            ...
                            <table class="table table-sm table-bordered">
                                ...
                                <tbody>
                                    @foreach($pengajuan->auditors as $penugasan)
                                        <tr>
                                            <td>{{ $penugasan->id }}</td>
                                            <td>{{ $penugasan->auditor->name ?? 'NULL' }}</td>
                                            <td>{{ $penugasan->role }}</td>
                                            <td>
                                                <span class="badge badge-sm badge-light-{{ $penugasan->is_setuju ? 'success' : 'warning' }}">
                                                    {{ $penugasan->is_setuju ? 'Setuju' : 'Belum Setuju' }}
                                                </span>
                                            </td>
                                        </tr>
                                    @endforeach
                    ...
                    @else
                        <div class="alert alert-warning mb-0">
                            <strong>No Auditor Assignment Found!</strong>
                        </div>
                    @endif
                </div>
                ...
                <div class="mt-3 p-3 bg-light rounded">
                    <h6 class="fw-bold">Database Queries untuk Debug:</h6>
                    <ul class="mb-0">
                        <li><code>SELECT * FROM kuisioner_jawabans WHERE pengajuan_id = {{ $pengajuan->id }}</code></li>
                        <li><code>SELECT * FROM instrumen_prodi_nilais WHERE pengajuan_ami_id = {{ $pengajuan->id }}</code></li>
                        <li><code>SELECT * FROM evaluasi_submissions WHERE pengajuan_ami_id = {{ $pengajuan->id }}</code></li>
                        <li><code>SELECT * FROM pengajuan_amis WHERE id = {{ $pengajuan->id }}</code></li>
                        <li><code>SELECT * FROM penugasan_auditors WHERE pengajuan_ami_id = {{ $pengajuan->id }}</code></li>
                    </ul>
                </div>
            </div>
            ...
            @php
                ...
                $totalInstrumenProdiNilai = $instrumenProdiNilai->count();
                $totalNilaiProdi = $instrumenProdiNilai->sum('nilai');
                $avgProdi = $totalInstrumenProdiNilai > 0 ? round($totalNilaiProdi / $totalInstrumenProdiNilai, 2) : 0;
            @endphp
                            ...
                            <div class="row g-3">
                                <div class="col-6">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            <span class="text-muted d-block fs-8">Total Instrumen Prodi</span>
                                            <span class="fw-bold fs-5 text-dark">{{ $totalInstrumenProdiNilai }}</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-6">
            ...
            <div class="row g-5 g-xl-8">
                ...
                <!-- Detail Table RSB (Sama seperti Detail Nilai Sasaran Strategis di Admin) -->
                <div class="col-xl-7">
                    <div class="card shadow-sm border-0">
                        <div class="card-header border-0 pt-5">
                            <h3 class="card-title fw-bold text-dark">
                            ...
                            <div class="card-toolbar">
                                <button type="button" class="btn btn-sm btn-light-primary" onclick="exportRSBTable()">
                        ...
                        <div class="card-body pt-0">
                            @php
                                $hasMissingValuesRSB = false;
                                foreach($evaluasiSubmissions as $submission) {
                                    if($submission->nilai <= 0) {
                                        $hasMissingValuesRSB = true;
                                        break;
                                    }
                                }
                            @endphp
                            @if($hasMissingValuesRSB)
                            <div class="alert alert-info d-flex align-items-center p-4 mb-4">
                                ...
                                <div class="d-flex flex-column">
                                    <h5 class="mb-1 text-info">Informasi Nilai</h5>
                            ...
                            @if($evaluasiSubmissions->count() > 0)
                                <div class="table-responsive">
                                    <table class="table table-row-dashed table-row-gray-300 align-middle" id="rsbTable">
                                        <thead>
                                            <tr class="fw-bold text-muted bg-light">
                                                <th width="5%" class="ps-4">No</th>
                                                <th width="12%">Kode</th>
                                                <th width="30%">Pertanyaan Evaluasi</th>
                                            ...
                                            @foreach($evaluasiSubmissions as $index => $submission)
                                                @php
                                                    ...
                                                    if($submission->nilai <= 0) {
                                                        ...
                                                        $nilaiLabel = '-';
                                                        $nilaiClass = 'badge-light-danger text-danger';
                                                    } else {
                                                        ...
                                                        if ($submission->nilai < 2.0) $progressColor = 'danger';
                                                        elseif ($submission->nilai < 2.5) $progressColor = 'warning';
                                                    }
                                                @endphp
                                                <tr>
                                                    <td class="ps-4">
                                                    ...
                                                    <td>
                                                        <span class="badge badge-light-primary fw-bold">RSB {{ $loop->iteration }}</span>
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td class="text-center">
                                                        @if($submission->nilai > 0)
                                                            <span class="badge badge-light-{{ $progressColor }} fw-bold">{{ $nilaiLabel }}</span>
                                                        @else
                                                            <span class="badge badge-light-danger fw-bold text-danger">Belum Dinilai</span>
                                                        @endif
                                                    </td>
                                                    <td class="text-center">
                                                        @if($submission->nilai > 0)
                                                            <span class="text-muted fs-7">Sudah dinilai oleh auditor</span>
                                                        @else
                                                            <span class="text-muted fs-7">Menunggu penilaian auditor</span>
                                                        @endif
                                                    </td>
                                                </tr>
                                            @endforeach
                                    ...
                                    <table class="table table-row-dashed table-row-gray-300 align-middle">
                                        ...
                                        <tbody>
                                            @foreach($instrumenProdiNilai as $index => $nilai)
                                                @php
                                                    $kriteria = $nilai->instrumenProdi->kriteriaInstrumen ?? null;
                                                    $nilaiLabel = '';
                                                    $nilaiClass = '';
                                                    switch($nilai->nilai) {
...
<script>
    ...
    if (beritaAcaraForm) {
        beritaAcaraForm.addEventListener('submit', function(e) {
            ...
            // Spider Chart untuk Jaring Laba-laba Instrumen RSB (Sama seperti Visualisasi Nilai Sasaran di Admin)
    @if($evaluasiSubmissions->count() > 0)
        ...
        if (spiderChartRSBCtx) {
            ...
            // Siapkan data untuk chart RSB (sama seperti admin)
            const labelsRSB = [];
            const dataRSB = [];

            // Process each evaluasi item to populate labels and values
            evaluasiData.forEach(item => {
                // Use the evaluasi as label
                const label = item.evaluasi.length > 30 ? item.evaluasi.substring(0, 30) + '...' : item.evaluasi;
                labelsRSB.push(label);

                // Find submission and get nilai
                const submission = submissionData.find(s => s.evaluasi_id == item.id);
                const nilai = submission ? submission.nilai : 0;
                dataRSB.push(nilai);
            });

            // Data for RSB Radar Chart (sama seperti admin)
            const dataStandarRSB = {
                labels: labelsRSB,
                datasets: [{
                    label: 'Nilai Instrumen RSB',
                    data: dataRSB,
                    fill: true,
                        ...
                        tooltip: {
                            ...
                            callbacks: {
                                label: function(context) {
                                    ...
                                    const evaluasiItem = evaluasiData.find(item => item.evaluasi === context.label || item.evaluasi.substring(0, 30) + '...' === context.label);
                                    const submission = submissionData.find(s => s.evaluasi_id == evaluasiItem.id);
                                    return [
                                        `Evaluasi: ${evaluasiItem.evaluasi}`,
                                        `Nilai: ${context.raw}`,
                                        `Status: ${submission ? 'Sudah dinilai' : 'Belum dinilai'}`
                                    ];
                                }
                            }
                        }
            ...
            // Buat chart RSB
            new Chart(spiderChartRSBCtx, configRadarRSB);
        }
    @endif

                                                // Spider Chart untuk Jaring Laba-laba Instrumen Prodi (Dari Data Evaluasi Auditor)
    @if($evaluasiSubmissions->count() > 0)
        const spiderChartProdiCtx = document.getElementById('spiderChartProdi');
        if (spiderChartProdiCtx) {
            // Data untuk chart Prodi dari EvaluasiSubmission (data dari 2 auditor)
            const evaluasiData = @json($evaluasis->where('jenis_evaluasi', 'auditor')->values());
            const submissionData = @json($evaluasiSubmissions->values());

            // Siapkan data untuk chart Prodi (sama seperti RSB)
            const labelsProdi = evaluasiData.map(e => e.evaluasi.length > 30 ? e.evaluasi.substring(0, 30) + '...' : e.evaluasi);
            const dataProdi = evaluasiData.map(evaluasi => {
                const submission = submissionData.find(s => s.evaluasi_id == evaluasi.id);
                return submission ? submission.nilai : 0;
            });

            // Buat spider chart untuk Prodi (sama seperti RSB)
            new Chart(spiderChartProdiCtx, {
                type: 'radar',
                data: {
                    labels: labelsProdi,
                    datasets: [{
                        label: 'Instrumen Prodi (Evaluasi Auditor)',
                        data: dataProdi,
    ...
    @endif
        ...
        // Export functions
        function exportRSBTable() {
            // Simple table export functionality for RSB table
            const table = document.getElementById('rsbTable');
            const html = table.outerHTML;
            const url = 'data:application/vnd.ms-excel,' + encodeURIComponent(html);
            const downloadLink = document.createElement("a");
            document.body.appendChild(downloadLink);
            downloadLink.href = url;
            downloadLink.download = 'instrumen_rsb_laporan_ami.xls';
            downloadLink.click();
