                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center mb-4">
                            <h3 class="card-title align-items-start flex-column mb-0">
                                <span class="fw-bold text-dark">Progress Evaluasi</span>
                            </h3>
                            <div class="ms-auto">
                                <span class="badge {{ $allCompleted ? 'badge-success' : 'badge-primary' }} fs-7 fw-bold">
                                    {{ number_format(($totalCompleted / $totalIkss) * 100, 0) }}%
                                </span>
                            </div>
                        </div>

                        <div class="progress h-8px bg-light-primary mb-3">
                            <div class="progress-bar rounded bg-{{ $allCompleted ? 'success' : 'primary' }}"
                                role="progressbar"
                                style="width: {{ ($totalCompleted / $totalIkss) * 100 }}%"
                                aria-valuenow="{{ ($totalCompleted / $totalIkss) * 100 }}"
                                aria-valuemin="0"
                                aria-valuemax="100">
                            </div>
                        </div>
                                ...
                                <div class="d-flex align-items-center border-bottom py-3">
                                    <div class="flex-grow-1">
                                        <div class="d-flex align-items-center">
                                            <span class="text-dark fw-bold fs-6 me-2">{{ $ss->kode_satuan }}</span>
                                            @if($isCompleted)
                                                <span class="badge badge-success fs-8 fw-bold">Selesai</span>
                                            @else
                                                <span class="badge badge-light fs-8 fw-bold">Belum Selesai</span>
                                            @endif
                                        </div>
                                        <span class="text-gray-600 fw-semibold d-block fs-7">
                                            {{ $status['completed'] }} dari {{ $status['total'] }} instrumen telah dievaluasi
                                        </span>
                                    </div>
                                    <div class="progress h-6px w-100px bg-light-primary">
                                        <div class="progress-bar bg-{{ $isCompleted ? 'success' : 'primary' }}"
                                            role="progressbar"
