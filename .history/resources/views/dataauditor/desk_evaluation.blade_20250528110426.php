    ...
    $(document).ready(function() {
        ...
        $('.wizard-step').click(function() {
            ...
            if (stepElement.hasClass('completed') ||
                ...
                showStep(stepId);
            } else if (stepElement.hasClass('disabled')) {
                Swal.fire({
        ...
        function showStep(stepId) {
            const currentStepElement = $(`.wizard-step[data-step="${stepId}"]`);

            // Check if we can move to this step
            if (currentStepElement.hasClass('disabled')) {
                Swal.fire({
                    title: 'Tidak dapat melanjutkan',
                    text: 'Harap selesaikan evaluasi pada tahap sebelumnya terlebih dahulu',
                    icon: 'warning'
            ...
            if (currentStepElement.hasClass('completed')) {
                const nextStepElement = currentStepElement.next('.wizard-step');
                if (nextStepElement.length) {
                    nextStepElement.removeClass('disabled');
                }
            }
        ...
        $('#approve-desk-btn').click(function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `/auditor/audit/desk-evaluation/approve/${pengajuanId}`,
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
