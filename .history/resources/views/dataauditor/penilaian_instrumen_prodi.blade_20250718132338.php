...
@php
    ...
    foreach ($sortedKriteriaIds as $kriteriaId) {
        ...
        // Set active kriteria (first incomplete or first kriteria)
        if ($activeKriteriaId === null && (!$isKriteriaCompleted || $allKriterias->isEmpty())) {
            $activeKriteriaId = $kriteriaId;
        }
            ...
            <div class="mb-8">
                ...
                <form id="penilaianForm" action="{{ route('auditor.audit.submitPenilaianInstrumenProdi', $pengajuan->id) }}" method="POST">
                    ...
                    @foreach($allKriterias as $kriteria)
                        ...
                        @if($kriteria['id'] == $activeKriteriaId)
                            <!-- Navigation Buttons - Only show for active kriteria -->
                            <div class="d-flex justify-content-between align-items-center mt-8">
                                ...
                                @if($nextKriteria && $isKriteriaCompleted)
                                    <button type="button" class="btn btn-success" onclick="saveAndContinue('{{ $nextKriteria['id'] }}', {{ $nextKriteria['accessible'] ? 'true' : 'false' }})">
                                        <i class="fas fa-save me-2"></i>
                                        Simpan & Lanjutkan
                                    </button>
                                @elseif($nextKriteria && !$isKriteriaCompleted)
                                    <button type="button" class="btn btn-primary" onclick="saveCurrentKriteria()">
                                        <i class="fas fa-save me-2"></i>
                                        Simpan Kriteria Ini
                                    </button>
                                @else
        ...
        .then(data => {
            if (data.success) {
                ...
                Swal.fire({
                    ...
                    // Navigate to next kriteria
                    showKriteriaContent(nextKriteriaId, isAccessible);
                });
            } else {
                Swal.fire({
                    icon: 'error',
