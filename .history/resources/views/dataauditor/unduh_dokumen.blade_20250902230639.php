...
<script>
    ...
    @if($instrumenProdiNilai->count() > 0)
        ...
        if (spiderChartProdiCtx) {
            try {
                // Data untuk chart Prodi dari InstrumenProdiNilai
                const instrumenProdiData = @json($instrumenProdiNilai->values());
                
                // Debug: Log data untuk troubleshooting
                console.log('Instrumen Prodi Data:', instrumenProdiData);

                // Siapkan data untuk chart Prodi dengan error handling
                const labelsProdi = [];
                const dataProdi = [];
                
                instrumenProdiData.forEach((nilai, index) => {
                    try {
                        // Ambil nilai numerik
                        const nilaiNumerik = parseInt(nilai.nilai) || 0;
                        dataProdi.push(nilaiNumerik);
                        
                        // Ambil label dengan fallback
                        let label = `Kriteria ${index + 1}`;
                        if (nilai.instrumen_prodi && nilai.instrumen_prodi.kriteria_instrumen) {
                            const kriteria = nilai.instrumen_prodi.kriteria_instrumen.nama_kriteria;
                            if (kriteria) {
                                label = kriteria.length > 30 ? kriteria.substring(0, 30) + '...' : kriteria;
                            }
                        }
                        labelsProdi.push(label);
                        
                    } catch (error) {
                        console.error('Error processing nilai:', error, nilai);
                        // Fallback data
                        labelsProdi.push(`Kriteria ${index + 1}`);
                        dataProdi.push(0);
                    }
                });

                // Debug: Log processed data
                console.log('Processed Labels:', labelsProdi);
                console.log('Processed Data:', dataProdi);

                // Pastikan ada data yang valid
                if (labelsProdi.length > 0 && dataProdi.length > 0) {
                    ...
                    console.log('Chart Prodi berhasil dibuat');
                } else {
                    console.error('Tidak ada data valid untuk chart Prodi');
                    spiderChartProdiCtx.style.display = 'none';
                }
                
            } catch (error) {
                console.error('Error creating Chart Prodi:', error);
                // Hide canvas jika ada error
                if (spiderChartProdiCtx) {
                    spiderChartProdiCtx.style.display = 'none';
                }
            }
