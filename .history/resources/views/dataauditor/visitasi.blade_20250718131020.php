...
@section('menuPenilaianInstrumenProdi')
    @foreach($pengajuan->auditors as $penugasan)
        @if($penugasan->role == 'ketua' && $penugasan->user_id == Auth::id())
            @php
                // Check if visitasi is completed and approved
                $visitasiCompleted = $allCompleted && $setuju;
            @endphp
            <li class="nav-item mt-2">
                @if($visitasiCompleted)
                    <a class="nav-link text-active-primary ms-0 me-10 py-5 {{ Route::is('auditor.audit.penilaianInstrumenProdi') ? 'active' : '' }}"
                       href="{{ route('auditor.audit.penilaianInstrumenProdi', $pengajuan->id) }}">
                        <i class="fas fa-file-alt me-2"></i> Penilaian Instrumen Prodi
                    </a>
                @else
                    <a class="nav-link text-active-primary ms-0 me-10 py-5 disabled">
                        <i class="fas fa-file-alt me-2"></i> Penilaian Instrumen Prodi
                    </a>
                @endif
            </li>
...
@section('menuUnduhDokumen')
    @foreach($pengajuan->auditors as $penugasan)
        @if($penugasan->role == 'ketua' && $penugasan->user_id == Auth::id())
            @php
                // Check if penilaian prodi is completed and approved
                $penilaianProdiCompleted = isset($pengajuan->status_penilaian_prodi) && $pengajuan->status_penilaian_prodi === 'approved';
            @endphp
            <li class="nav-item mt-2">
                @if($penilaianProdiCompleted)
                    <a class="nav-link text-active-primary ms-0 me-10 py-5 {{ Route::is('auditor.audit.unduhDokumen') ? 'active' : '' }}"
                       href="{{ route('auditor.audit.unduhDokumen', $pengajuan->id) }}">
