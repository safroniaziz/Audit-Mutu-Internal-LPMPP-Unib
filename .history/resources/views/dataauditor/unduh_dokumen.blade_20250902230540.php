...
<script>
    ...
    @if($instrumenProdiNilai->count() > 0)
        ...
        if (spiderChartProdiCtx) {
            ...
            const labelsProdi = instrumenProdiData.map(nilai => {
                const kriteria = nilai.instrumen_prodi?.kriteria_instrumen?.nama_kriteria;
                return kriteria && kriteria.length > 30 ? kriteria.substring(0, 30) + '...' : kriteria || 'Kriteria tidak ditemukan';
            });
            const dataProdi = instrumenProdiData.map(nilai => nilai.nilai || 0);
