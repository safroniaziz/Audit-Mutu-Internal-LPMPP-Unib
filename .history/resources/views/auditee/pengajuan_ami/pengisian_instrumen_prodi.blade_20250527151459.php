...
@php
    ...
    foreach ($indikatorInstrumens as $instrumen) {
        // Skip if instrumen doesn't have indikatorInstrumenKriteria relation
        if (!$instrumen->indikatorInstrumenKriteria) {
            continue;
        }

        $indikatorId = $instrumen->id;
        $kriteriaId = $instrumen->indikatorInstrumenKriteria->id;
        ...
        if (!isset($groupedData[$indikatorId]['kriteria'][$kriteriaId])) {
            $groupedData[$indikatorId]['kriteria'][$kriteriaId] = [
                'data' => $instrumen->indikatorInstrumenKriteria,
                'instrumen_prodi' => []
            ];
        }

        $groupedData[$indikatorId]['kriteria'][$kriteriaId]['instrumen_prodi'][] = $instrumen;
    }
@endphp
        ...
        <div class="card-body border-top p-9">
            ...
            <div class="wizard-nav mb-5">
                @foreach($groupedData as $indikatorId => $indikatorGroup)
                    <div class="wizard-step {{ $loop->first ? 'active' : '' }}" data-indikator="{{ $indikatorId }}">
                        ...
                        <div class="step-desc">
                            {{ count($indikatorGroup['kriteria']) }} Kriteria
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            @foreach($groupedData as $indikatorId => $indikatorGroup)
                <div class="sub-wizard-nav {{ $loop->first ? 'active' : '' }}" data-indikator="{{ $indikatorId }}">
                    @foreach($indikatorGroup['kriteria'] as $kriteriaId => $kriteria)
                        <div class="wizard-step {{ $loop->first ? 'active' : '' }}" data-kriteria="{{ $kriteriaId }}">
                            <div class="step-number">{{ $loop->iteration }}</div>
                            <div class="step-label">{{ $kriteria['data']->nama_kriteria }}</div>
                            <div class="step-desc">
                                {{ count($kriteria['instrumen_prodi']) }} Instrumen
                            </div>
                        </div>
                    @endforeach
                </div>
                    ...
                    <div class="wizard-content {{ ($loop->parent->first && $loop->first) ? 'active' : '' }}"
                        ...
                        <div class="card card-bordered mb-5">
                            <div class="card-header">
                                <h3 class="card-title">{{ $indikatorGroup['indikator']->nama }} - {{ $kriteria['data']->nama_kriteria }}</h3>
                            </div>
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3">
                                        ...
                                        <tbody>
                                            @foreach($kriteria['instrumen_prodi'] as $instrumenProdi)
                                                <tr>
                                                    <td>{{ $loop->iteration }}</td>
                                                    <td>{{ $instrumenProdi->nama }}</td>
                                                    <td>
                                                        @if($instrumenProdi->submission)
                                                            <span class="badge badge-light-success">{{ $instrumenProdi->submission->nilai }}</span>
                                                        @else
                                                            <span class="badge badge-light-warning">Belum diisi</span>
                                                        @endif
                                                    </td>
                                                    <td>
                                                        <span class="{{ $instrumenProdi->submission ? '' : 'text-muted' }}">
                                                            {{ $instrumenProdi->submission ? Str::limit($instrumenProdi->submission->deskripsi, 50) : 'Belum diisi' }}
                                                        </span>
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td class="text-center">
                                                        <button type="button"
                                                                class="btn btn-sm {{ $instrumenProdi->submission ? 'btn-light-primary' : 'btn-primary' }} btn-edit"
                                                                data-instrumen-id="{{ $instrumenProdi->id }}"
                                                                data-nilai="{{ $instrumenProdi->submission ? $instrumenProdi->submission->nilai : '' }}"
                                                                data-deskripsi="{{ $instrumenProdi->submission ? $instrumenProdi->submission->deskripsi : '' }}">
                                                            <i class="bi {{ $instrumenProdi->submission ? 'bi-pencil' : 'bi-plus' }}"></i>
                                                            {{ $instrumenProdi->submission ? 'Edit' : 'Isi' }}
                                                        </button>
                                                    </td>
                                                </tr>
                                            @endforeach
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.wizard-nav .wizard-step').on('click', function() {
                ...
                const firstKriteria = $(`.sub-wizard-nav[data-indikator="${indikatorId}"] .wizard-step:first`).data('kriteria');
                showContent(indikatorId, firstKriteria);
            });
            ...
            $('.sub-wizard-nav .wizard-step').on('click', function() {
                ...
                showContent(indikatorId, kriteriaId);
            });

            function showContent(indikatorId, kriteriaId) {
                $('.wizard-content').removeClass('active');
                $(`.wizard-content[data-indikator="${indikatorId}"][data-kriteria="${kriteriaId}"]`).addClass('active');
            }
        });
    </script>
@endpush
