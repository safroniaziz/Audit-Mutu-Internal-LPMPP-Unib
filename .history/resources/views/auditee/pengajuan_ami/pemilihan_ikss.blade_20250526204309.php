        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function validateCurrentStep() {
                ...
                instrumentGroups.forEach(group => {
                    ...
                    if (instrumenId) {
                        // Jika ada hidden input (wajib), cek apakah sudah ada di database
                        if (hiddenInput) {
                            // Cek apakah data wajib ini sudah ada di dataTerpilih
                            if (!dataTerpilih[`pilihan_${instrumenId}`]) {
                                allValid = false;
                            }
                        } else {
                            // Untuk non-wajib, cek apakah sudah dipilih DAN ada di database
                            const isChecked = Array.from(radios).some(radio => radio.checked);
                            const isInDatabase = dataTerpilih.hasOwnProperty(`pilihan_${instrumenId}`);

                            if (!isChecked || !isInDatabase) {
                                allValid = false;
                            }
                        }
                    }
