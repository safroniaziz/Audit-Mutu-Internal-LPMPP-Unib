...
<style>
    ...
    .wizard-step:hover:not(.active):not(.completed):not(.disabled) {
        color: #009EF7;
    }
    ...
    .wizard-step.active:not(.completed) .step-number,
    .wizard-step.active:not(.completed) {
        color: #009EF7;
    }
    ...
    .wizard-step.active:not(.completed) .step-desc {
        color: #009EF7;
    }
</style>
@endpush
...
$(document).ready(function() {
    ...
    // Show kriteria content based on URL parameter or default to first
    const activeKriteriaId = getUrlParameter('active_kriteria');
    const firstKriteriaId = $('.wizard-step').first().data('kriteria-id');

    if (activeKriteriaId && $(`.wizard-step[data-kriteria-id="${activeKriteriaId}"]`).data('accessible') === true) {
    ...
    $('.wizard-step').click(function() {
        ...
        if (isAccessible === true) {
            // Update URL with active kriteria
            const newUrl = updateUrlParameter(window.location.href, 'active_kriteria', kriteriaId);
    ...
    $('.kriteria-form').on('submit', function(e) {
        ...
        // Find next kriteria step for URL parameter
                    ...
                    success: function(response) {
                        if (response.success) {
                            Swal.fire({
                                ...
                                // If there's a next step and not the last step, update URL before reload
                                if (nextStep.length && !isLastStep) {
                                    const redirectUrl = updateUrlParameter(window.location.href, 'active_kriteria', nextKriteriaId);
