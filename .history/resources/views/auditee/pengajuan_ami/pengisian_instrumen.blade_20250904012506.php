...
@section('dashboardProfile')
    <div class="card mb-5 mb-xl-10" id="kt_profile_details_view">
        ...
        <div class="card-body border-top p-9">
            <!-- Progress Overview -->
            @php
                // Use variables from controller instead of recalculating
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        const currentIndex = steps.indexOf(parseInt(currentStepId));
                        const nextStepId = steps[currentIndex + 1];

                        // Enable next step if exists
                        if (nextStepId) {
                            const nextStep = $(`.wizard-step[data-step="${nextStepId}"]`);
                        ...
                        Swal.fire({
                            ...
                            // Navigate to next step instead of reloading
                            if (nextStepId) {
                                activateStep(nextStepId);
                            } else {
                                // If this is the last step, reload to show completion status
                                window.location.reload();
                            }
                        });
                    } else {
