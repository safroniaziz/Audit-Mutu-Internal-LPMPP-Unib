                    ...
                    @foreach ($dataIkssProdi as $unit)
                        @foreach ($unit->indikatorKinerjas->groupBy('satuan_standar_id') as $satuanId => $indikators)
                            @php
                                ...
                                // Count total and selected instruments
                                foreach ($indikators as $indikator) {
                                    foreach ($indikator->instrumen as $instrumen) {
                                        $totalInstruments++;
                                        if (isset($dataTerpilih['pilihan_'.$instrumen->id]) || $instrumen->is_wajib == 1) {
                                            $selectedInstruments++;
                                        }
                                    }
                                }
                                ...
                                // Debug information
                                error_log("SS {$satuanStandar->kode_satuan}: Total={$totalInstruments}, Selected={$selectedInstruments}, IsSubmitted={$isSSSubmitted}");
                                if ($totalInstruments > 0) {
                                    foreach ($indikators as $indikator) {
                                        foreach ($indikator->instrumen as $instrumen) {
                                            $status = isset($dataTerpilih['pilihan_'.$instrumen->id]) ? $dataTerpilih['pilihan_'.$instrumen->id] : 'not set';
                                            error_log("  Instrumen {$instrumen->id}: Status={$status}, IsWajib={$instrumen->is_wajib}");
                                        }
                                    }
                                }
                            @endphp
