    ...
    <div class="card mb-5 mb-xl-10" id="kt_profile_details_view">
        ...
        <div class="card-body border-top p-9">
            @if($ikssAuditeeData->isNotEmpty())
                <div class="alert {{ $allCompleted ? 'alert-success' : 'alert-primary' }} d-flex align-items-start p-5 mb-10">
                    <div class="me-4">
                        <i class="bi {{ $allCompleted ? 'bi-check-circle-fill fs-2 text-success' : 'bi-info-circle-fill fs-2 text-primary' }}"></i>
                    </div>
                    <div class="flex-grow-1">
                        <h4 class="fw-bold text-dark mb-2">{{ $allCompleted ? '✨ Pengisian Instrumen Selesai' : '📝 Pengisian Instrumen' }}</h4>
                        <div class="fs-6 text-gray-700">
                            <p class="mt-4">
                                <strong>{{ $allCompleted ? 'Selamat!' : 'Status:' }}</strong>
                                <span class="fw-semibold {{ $allCompleted ? 'text-success' : 'text-primary' }}">
                                    @if($allCompleted)
