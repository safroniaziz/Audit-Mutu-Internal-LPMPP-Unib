        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            wizardContents.forEach((content, index) => {
                ...
                content.querySelectorAll('.d-flex.align-items-start.border').forEach(group => {
                    ...
                    // Check if this instrument is saved in database
                    if (instrumenId) {
                        const isInDatabase = dataTerpilih.hasOwnProperty(`pilihan_${instrumenId}`);

                        if (isWajib) {
                            // For wajib items, must be in database
                            if (isInDatabase) {
                ...
                console.log(`Checking step ${stepNumber}:`, {
                    total: stepTotalInstruments,
                    saved: stepSelectedInstruments,
                    isComplete: stepTotalInstruments > 0 && stepSelectedInstruments === stepTotalInstruments
                });

                // If this step is not complete (not all saved in database) and we haven't found an incomplete step yet
                if (!foundIncomplete && stepTotalInstruments > 0 && stepSelectedInstruments < stepTotalInstruments) {
                    firstIncompleteStep = stepNumber;
                    foundIncomplete = true;
                    console.log(`Found first incomplete step: ${stepNumber}`);
                }
            });
            ...
            // Function to check if a step is completed with database validation
            function isStepCompleted(stepNumber) {
                const content = document.querySelector(`.wizard-content[data-step="${stepNumber}"]`);
                if (!content) return false;
