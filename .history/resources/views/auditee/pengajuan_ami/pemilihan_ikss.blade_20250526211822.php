...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Find first incomplete or unsaved step
            ...
            wizardContents.forEach((content, index) => {
                ...
                // If this step is not complete (either not all selected or not all saved) and we haven't found an incomplete step yet
                if (!foundIncomplete && (stepTotalInstruments === 0 || stepSelectedInstruments < stepTotalInstruments || !allSavedInDatabase)) {
            ...
            // Add click event to wizard steps with database validation
            wizardSteps.forEach(step => {
                step.addEventListener('click', () => {
                    const stepNumber = parseInt(step.dataset.step);
                    const previousStep = stepNumber - 1;

                    // Check if previous step exists and is completed (including database state)
                    if (stepNumber === 1 || (previousStep > 0 && isStepCompleted(previousStep))) {
                        ...
                        // Show warning if trying to access a step without completing previous
                        Swal.fire({
                            text: "Harap selesaikan dan simpan Sasaran Strategis sebelumnya terlebih dahulu!",
                            icon: "warning",
            ...
            function updateWizardState() {
                // Update semua status SS terlebih dahulu
                wizardContents.forEach(content => {
                    const stepContent = content;
                    let totalInstruments = 0;
                    let selectedInstruments = 0;
                    let selectedYesCount = 0;
                    let allSavedInDatabase = true;

                    // Hitung status untuk setiap SS
                    stepContent.querySelectorAll('.d-flex.align-items-start.border').forEach(group => {
                        totalInstruments++;
                        const isWajib = group.querySelector('input[type="hidden"][name^="pilihan_"]') !== null;
                        const radioName = group.querySelector('input[type="radio"]')?.name;
                        const instrumenId = radioName?.replace('pilihan_', '');
                        ...
                        if (instrumenId) {
                            ...
                            if (isWajib) {
                                ...
                                if (isInDatabase) {
                                    selectedInstruments++;
                                    if (dataTerpilih[`pilihan_${instrumenId}`] == 1) {
                                        selectedYesCount++;
                                    }
                                }
                            }
                    ...
                    // Update status alert untuk setiap SS
                    const alertDiv = stepContent.querySelector('.alert');
                    const statusHeader = alertDiv.querySelector('h4');
                    const statusText = stepContent.querySelector('.text-muted.mt-1');

                    if (statusText) {
                        statusText.textContent = `Progress: ${selectedInstruments}/${totalInstruments} instrumen dipilih (${selectedYesCount} dipilih YA)`;
                    }

                    if (selectedInstruments === totalInstruments && totalInstruments > 0 && allSavedInDatabase) {
            ...
            function validateCurrentStep() {
                ...
                const ssId = currentContent.dataset.ssId;
                let allValid = true;
                let hasUnsavedWajib = false;

                // Get all instrument groups in current step
                const instrumentGroups = currentContent.querySelectorAll('.d-flex.align-items-start.border');

                instrumentGroups.forEach(group => {
                    const radios = group.querySelectorAll('input[type="radio"]');
                    const hiddenInput = group.querySelector('input[type="hidden"]');
                    const radioName = radios.length > 0 ? radios[0].name : null;
                    const instrumenId = radioName ? radioName.replace('pilihan_', '') : null;

                    if (instrumenId) {
                        // Untuk instrumen wajib
                        if (hiddenInput) {
                            // Cek apakah data wajib sudah ada di database
                            if (!dataTerpilih.hasOwnProperty(`pilihan_${instrumenId}`)) {
                                hasUnsavedWajib = true;
                                allValid = false;
                            }
                        } else {
                            // Untuk non-wajib, cek apakah sudah dipilih
                ...
                if (hasUnsavedWajib) {
                    Swal.fire({
                        text: "Ada instrumen wajib yang belum disimpan ke database. Silakan simpan terlebih dahulu!",
                        icon: "warning",
                        buttonsStyling: false,
                        confirmButtonText: "OK",
                        customClass: {
                            ...
                            $.ajax({
                                ...
                                success: function(response) {
                                    ...
                                    if (response.success) {
                                        ...
                                        // Update dataTerpilih dengan data baru
                                        if (response.data && typeof response.data === 'object') {
                                            Object.keys(response.data).forEach(key => {
                                                dataTerpilih[key] = response.data[key];
                                            });
                                        }

                                        // Debug log untuk memastikan dataTerpilih sudah diupdate
                                        console.log('dataTerpilih setelah update:', dataTerpilih);

                                        // Force update semua status
                                        wizardContents.forEach(content => {
                                            const stepNumber = content.dataset.step;
                                            console.log(`Updating status for step ${stepNumber}`);

                                            // Update status untuk setiap SS
                                            let totalInstruments = 0;
                                            let selectedInstruments = 0;
                                            let selectedYesCount = 0;
                                            let allSavedInDatabase = true;
                                            ...
                                            content.querySelectorAll('.d-flex.align-items-start.border').forEach(group => {
                                                ...
                                                if (instrumenId) {
                                                    console.log(`Checking instrumen ${instrumenId}:`, {
                                                        isWajib,
                                                        inDatabase: dataTerpilih.hasOwnProperty(`pilihan_${instrumenId}`),
                                                        value: dataTerpilih[`pilihan_${instrumenId}`]
                                                    });
                                            ...
                                            const statusHeader = alertDiv.querySelector('h4');
                                            const statusText = content.querySelector('.text-muted.mt-1');

                                            console.log(`Status for step ${stepNumber}:`, {
                                                totalInstruments,
                                                selectedInstruments,
                                                selectedYesCount,
                                                allSavedInDatabase
                                            ...
                                            if (selectedInstruments === totalInstruments && totalInstruments > 0 && allSavedInDatabase) {
                                                ...
                                                // Remove completed status if not all saved
                                                const wizardStep = document.querySelector(`.wizard-step[data-step="${stepNumber}"]`);
                                                if (wizardStep) {
                                        ...
                                        // Update overall progress dan accessibility
            ...
            function updateCurrentStepProgress() {
                ...
                let selectedInstruments = 0;
                let selectedYesCount = 0;
                let allSavedInDatabase = true; // Flag untuk mengecek semua data sudah di database
                ...
                activeContent.querySelectorAll('.d-flex.align-items-start.border').forEach(group => {
                    ...
                    if (instrumenId) {
                        ...
                        isInDatabase = dataTerpilih.hasOwnProperty(`pilihan_${instrumenId}`);

                        if (isWajib) {
                            isSelectedYes = true;
                            hasSelection = true;
                            // Untuk wajib, harus tetap cek database
                            if (!isInDatabase) {
                                allSavedInDatabase = false;
                            }
                        } else {
                            ...
                            // Untuk non-wajib, jika dipilih tapi belum di database
                            if (hasSelection && !isInDatabase) {
                                allSavedInDatabase = false;
                            }
                        }
                    }

                    if (isSelectedYes && isInDatabase) {
                        selectedYesCount++;
                    }
                    if (hasSelection && isInDatabase) {
                        selectedInstruments++;
                ...
                // Update alert status based on whether all instruments are selected AND saved
                const alertDiv = activeContent.querySelector('.alert');
                const statusHeader = alertDiv.querySelector('h4');

                // Step is completed if all instruments have been selected AND saved to database
                if (selectedInstruments === totalInstruments && totalInstruments > 0 && allSavedInDatabase) {
                    alertDiv.className = 'alert bg-light-success mb-5';
                    alertDiv.querySelector('i').className = 'ki-duotone ki-information-5 fs-2qx me-4 text-success';
                    statusHeader.className = 'mb-1 text-success';
