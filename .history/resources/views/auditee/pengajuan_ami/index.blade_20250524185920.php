                ...
                <form id="kt_account_profile_details_form" action="{{ route('auditee.pengajuanAmi.lengkapiProfil') }}" class="form" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="card-body border-top p-9">
                        @php
                            ...
                            // Check if perjanjian kinerja exists
                            $periodeAktif = App\Models\PeriodeAktif::whereNull('deleted_at')->first();
                            $hasPerjanjianKinerja = App\Models\PerjanjianKinerja::where('auditee_id', $user->unit_kerja_id)
                        ...
                        @if($completionPercentage == 100)
                            <div class="alert alert-{{ $hasPerjanjianKinerja ? 'success' : 'danger' }} d-flex align-items-start p-5 position-relative">
                                <div class="me-4">
                                    <i class="bi {{ $hasPerjanjianKinerja ? 'bi-check-circle-fill text-success' : 'bi-exclamation-triangle-fill text-danger' }} fs-2"></i>
                                </div>
                                <div class="flex-grow-1">
                                    <h4 class="fw-bold text-dark mb-2">{{ $hasPerjanjianKinerja ? '✨ Perjanjian Kinerja Telah Diunggah' : '📢 Unggah Perjanjian Kinerja' }}</h4>
                                    <div class="fs-6 {{ $hasPerjanjianKinerja ? 'text-success' : 'text-danger' }}">
                                        @if($hasPerjanjianKinerja)
                                            <p class="mt-4 mb-0">
                                                <strong>Selamat!</strong> Perjanjian Kinerja program studi Anda telah diunggah. Silakan lanjut ke proses Pemilihan IKSS.
                                            </p>
                                        @else
                                            <p class="mt-4 mb-0">
                                                <strong>Perhatian!</strong> Silakan unggah dokumen Perjanjian Kinerja program studi Anda. Dokumen ini akan menjadi dasar untuk proses audit mutu internal selanjutnya.
                                            </p>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="ms-auto d-flex gap-2">
                                    <a href="{{ route('auditee.pengajuanAmi.perjanjianKinerja') }}" class="btn btn-sm {{ $hasPerjanjianKinerja ? 'btn-light' : 'btn-danger' }}">
                                        <i class="fas fa-upload me-2"></i> {{ $hasPerjanjianKinerja ? 'Lihat Dokumen' : 'Unggah Dokumen' }}
                                    </a>

                                    <a href="{{ route('auditee.pengajuanAmi.pemilihanIkss') }}"
                                       class="btn btn-sm btn-success {{ !$hasPerjanjianKinerja ? 'disabled' : '' }}"
                                       @if(!$hasPerjanjianKinerja)
