        ...
        @foreach ($dataIkssProdi as $unit)
            @foreach ($unit->indikatorKinerjas as $indikatorIndex => $indikator)
                @if($indikator->instrumen->isNotEmpty())
                    <div class="card mb-4">
                        <!-- Collapsible header - Fixed by changing data attributes and adding role -->
                        <div class="card-header bg-light pt-4 pb-3 d-flex justify-content-between align-items-center"
                             role="button"
                             data-bs-toggle="collapse"
                             href="#collapse-{{ $indikatorIndex }}"
                             aria-expanded="false"
                             aria-controls="collapse-{{ $indikatorIndex }}">
                            <h4 class="mb-0">ID IKSS: {{ $indikator->kode_ikss }} – {{ $indikator->tujuan }}</h4>
                            <i class="fas fa-chevron-down"></i>
                        </div>

                        <!-- Collapsible content - Make sure it has the correct ID -->
...
@push('scripts')
    <script>
        ...
        // Additional script to ensure collapse functionality works
