...
@push('styles')
    <style>
        ...
        .penilaian-list li:hover {
            transform: translateX(5px);
            box-shadow: 0 5px 20px rgba(0, 158, 247, 0.15);
        }
        .penilaian-list li:last-child {
            margin-bottom: 0;
        }
        .penilaian-list .score {
            font-weight: 700;
            color: #009EF7;
            margin-right: 20px;
        ...
        .penilaian-list li.no-value {
            background: #F8F9FA;
            border-left-color: #A1A5B7;
        }
        .penilaian-list li.no-value .score {
            color: #A1A5B7;
            background: rgba(161, 165, 183, 0.1);
        }
        .penilaian-list li.no-value .score::after {
            background: rgba(161, 165, 183, 0.15);
        }
        .penilaian-list li.no-value .description {
            color: #7E8299;
        }
    </style>
@endpush
...
@section('dashboardProfile')
    <div class="card mb-5 mb-xl-10" id="kt_profile_details_view"></div>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <div class="bg-light-primary rounded p-4">
                                            @php
                                                $penilaianText = strip_tags($item['instrumen']->penilaian);
                                                // Ubah regex untuk mencocokkan angka yang diikuti spasi
                                                $penilaianArray = preg_split('/(\d+)\s+/', $penilaianText, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
                                                $formattedPenilaian = [];

                                                for ($i = 0; $i < count($penilaianArray); $i += 2) {
                                                    if (isset($penilaianArray[$i + 1])) {
                                                        $score = trim($penilaianArray[$i]);
                                                        $description = trim($penilaianArray[$i + 1]);
                                                        $formattedPenilaian[] = [
                                                            'score' => $score,
                                                ...
                                                // Sort array in descending order based on score
                                                usort($formattedPenilaian, function($a, $b) {
                                                    return $b['score'] - $a['score'];
                                                });
                                            @endphp

                                            <ul class="penilaian-list">
                                                @foreach($formattedPenilaian as $penilaian)
                                                    <li class="{{ $penilaian['description'] == '-' ? 'no-value' : '' }}">
                                                        <span class="score">{{ $penilaian['score'] }}</span>
                                                        <span class="description">
                                                            {{ $penilaian['description'] == '-' ? 'Tidak ada kriteria' : $penilaian['description'] }}
                                                        </span>
                                                    </li>
                                                @endforeach
                                            </ul>
