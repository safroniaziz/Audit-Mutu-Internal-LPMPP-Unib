    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateProgress() {
                ...
                const totalRadios = activeContent.querySelectorAll('input[type="radio"]').length;
                const totalInstruments = totalRadios / 2; // Since we have 2 radio buttons per instrument
                let selectedCount = 0;

                // Count selected and mandatory instruments
                activeContent.querySelectorAll('.d-flex.gap-4').forEach(instrumentGroup => {
                    const isWajib = instrumentGroup.querySelector('input[type="hidden"]') !== null;
                    const hasCheckedRadio = instrumentGroup.querySelector('input[type="radio"]:checked') !== null;

                    if (isWajib || hasCheckedRadio) {
                        selectedCount++;
                    }
                });
