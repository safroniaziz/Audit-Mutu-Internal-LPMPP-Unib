                            ...
                            @php
                                ...
                                foreach ($indikators as $indikator) {
                                    foreach ($indikator->instrumen as $instrumen) {
                                        $totalInstruments++;
                                        $key = 'pilihan_' . $instrumen->id;
                                        if (isset($dataTerpilih[$key]) && $dataTerpilih[$key] == 1) {
                                            $selectedInstruments++;
                                        }
                                    }
                                }

                                $isSSSubmitted = $selectedInstruments === $totalInstruments;
                            @endphp
                        ...
                        <div class="wizard-content" data-step="{{ $loop->iteration }}" data-ss-id="{{ $satuanId }}">
                            ...
                            <div class="progress h-8px bg-light rounded mb-5">
                                <div class="progress-bar-ss bg-primary rounded h-8px" role="progressbar"
                                     style="width: {{ ($selectedInstruments / $totalInstruments) * 100 }}%"
                                     aria-valuenow="{{ ($selectedInstruments / $totalInstruments) * 100 }}"
                                     aria-valuemin="0"
                                     aria-valuemax="100">
                                </div>
                            </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function calculateOverallProgress() {
                ...
                document.querySelectorAll('.instrument-group').forEach(group => {
                    ...
                    // Check if instrument is selected in database
                    const isSelectedInDB = instrumenId && dataTerpilih[`pilihan_${instrumenId}`] === '1';

                    if (isWajib || isSelectedInDB) {
                        selectedCount++;
                    }
                });

                return {
                    percentage: (selectedCount / totalInstruments) * 100,
