        ...
        @foreach ($dataIkssProdi as $unit)
            @foreach ($unit->indikatorKinerjas as $indikatorIndex => $indikator)
                @if($indikator->instrumen->isNotEmpty())
                    <div class="card mb-4">
                        <!-- Simplified collapsible header using buttons instead -->
                        <div class="card-header bg-light pt-4 pb-3" id="heading-{{ $indikatorIndex }}">
                            <button class="btn btn-link text-decoration-none w-100 text-start d-flex justify-content-between align-items-center"
                                   type="button"
                                   onclick="toggleCollapse('collapse-{{ $indikatorIndex }}')">
                                <h4 class="mb-0 text-dark">ID IKSS: {{ $indikator->kode_ikss }} – {{ $indikator->tujuan }}</h4>
                                <i class="fas fa-chevron-down toggle-icon" id="icon-{{ $indikatorIndex }}"></i>
                            </button>
                        </div>

                        <!-- Collapsible content -->
                        <div class="collapse-section" id="collapse-{{ $indikatorIndex }}" style="display: none;">
                            <div class="card-body">
...
@push('scripts')
    <script>
        ...
        // Pure JavaScript approach without relying on Bootstrap's collapse
    function toggleCollapse(id) {
        var content = document.getElementById(id);
        var iconId = 'icon-' + id.split('-')[1];
    ...
    document.addEventListener('DOMContentLoaded', function() {
        var collapseSections = document.querySelectorAll('.collapse-section');
