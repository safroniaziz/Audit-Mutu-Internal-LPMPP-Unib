...
@push('styles')
    <style>
        ...
        .wizard-nav::-webkit-scrollbar {
            height: 12px !important; /* Make scrollbar more visible */
        ...
        .wizard-nav::-webkit-scrollbar-track {
            ...
            border-radius: 10px;
            margin: 0 20px; /* Add margin to track */
        }

        .wizard-nav::-webkit-scrollbar-thumb {
            background: #888 !important;
            border-radius: 10px;
            border: 2px solid #f1f1f1; /* Add border for visibility */
        }
        ...
        /* Force minimum width for scroll container */
        .wizard-nav {
            min-width: 100% !important;
            width: 100% !important;
        }

        /* Ensure steps take proper space */
        .wizard-nav .wizard-step:first-child {
            margin-left: 10px;
        }

        .wizard-nav .wizard-step:last-child {
            margin-right: 10px;
        }
        ...
        .wizard-step {
            flex: 0 0 auto !important; /* Force no flex grow/shrink */
            ...
            /* Ensure each step is a separate block */
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateCurrentStepProgress() {
                ...
                currentContent.querySelectorAll('.d-flex.align-items-start.border').forEach(group => {
                    ...
                    if (isWajib || (radioName && group.querySelector(`input[name="${radioName}"]:checked`))) {
                        selectedInstruments++;
                    }
                });
        ...
        // Force enable horizontal scroll for wizard navigation
        $(document).ready(function() {
            ...
            wizardNav.on('mousedown', function(e) {
                // Only enable drag scroll when not clicking on wizard steps
            ...
            setTimeout(function() {
                const nav = wizardNav[0];
                const steps = wizardNav.find('.wizard-step');
                console.log('=== WIZARD SCROLL DEBUG ===');
                console.log('Wizard nav scroll width:', nav.scrollWidth);
                console.log('Wizard nav client width:', nav.clientWidth);
                console.log('Wizard nav offset width:', nav.offsetWidth);
                console.log('Number of steps:', steps.length);
                console.log('Can scroll horizontally:', nav.scrollWidth > nav.clientWidth);
                console.log('Overflow X style:', window.getComputedStyle(nav).overflowX);
                console.log('Display style:', window.getComputedStyle(nav).display);
                
                // Test manual scroll
                if (nav.scrollWidth > nav.clientWidth) {
                    console.log('✅ Scrolling should work - testing...');
                    nav.scrollLeft = 50;
                    setTimeout(() => {
                        console.log('Scroll position after test:', nav.scrollLeft);
                        nav.scrollLeft = 0; // Reset
                    }, 100);
                } else {
                    console.log('❌ Content not wide enough to scroll');
                    console.log('Total steps width calculated:', steps.length * 250 + 'px (250px min-width each)');
                }
            }, 500);
        });
    </script>
