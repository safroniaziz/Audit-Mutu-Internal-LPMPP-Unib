        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateProgress() {
                ...
                activeContent.querySelectorAll('.d-flex.align-items-start.border').forEach(group => {
                    ...
                    const isSelectedInUI = group.querySelector('input[type="radio"][value="1"]:checked') !== null;
                ...
                const progressPercentage = totalInstruments > 0 ? (selectedCount / totalInstruments) * 100 : 0;
                ...
                if (statusText) {
                    statusText.textContent = `Progress: ${selectedCount}/${totalInstruments} instrumen dipilih YA`;
                }
            ...
            // Update progress for all SS sections initially
            function updateAllProgress() {
                ...
                // Restore active state to current step
                const currentContent = document.querySelector(`.wizard-content[data-step="${currentStep}"]`);
                if (currentContent) {
            ...
            function calculateOverallProgress() {
                ...
                document.querySelectorAll('.d-flex.align-items-start.border').forEach(group => {
                    totalInstruments++;
                    const isWajib = group.querySelector('input[type="hidden"][name^="pilihan_"]') !== null;
                    const radioName = group.querySelector('input[type="radio"]')?.name;
                    const instrumenId = radioName?.replace('pilihan_', '');

                    if (isWajib || (instrumenId && dataTerpilih[`pilihan_${instrumenId}`] == 1)) {
                        selectedCount++;
                    }
                });

                return {
                    percentage: totalInstruments > 0 ? (selectedCount / totalInstruments) * 100 : 0,
            ...
            function updateMainProgress() {
                ...
                if (mainStatus) {
                    mainStatus.textContent = `${progress.selected} dari ${progress.total} instrumen dipilih YA`;
                }
            }

            // Initial setup when page loads
            function initializeProgress() {
                // Set initial radio button states from database
                Object.entries(dataTerpilih).forEach(([key, value]) => {
                    const instrumenId = key.replace('pilihan_', '');
                    const radio = document.querySelector(`input[type="radio"][name="pilihan_${instrumenId}"][value="${value}"]`);
                    if (radio) {
