...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateCurrentStepProgress() {
                ...
                // Update status text for current SS
                const statusText = activeContent.querySelector('.text-muted.mt-1');
                if (statusText) {
                    statusText.textContent = `Progress: ${selectedYesCount}/${totalInstruments} instrumen dipilih YA`;
                }

                // Update alert status based on whether all instruments are selected (not just "Ya")
                const alertDiv = activeContent.querySelector('.alert');
                const statusHeader = alertDiv.querySelector('h4');

                if (allSelected && totalInstruments > 0) {
                    alertDiv.className = 'alert bg-light-success mb-5';
                    alertDiv.querySelector('i').className = 'ki-duotone ki-information-5 fs-2qx me-4 text-success';
            ...
            unction updateOverallProgress() {
    let totalSteps = wizardContents.length;
    let completedStepsCount = 0;

    // Count completed steps based on current status
    wizardContents.forEach((content, index) => {
        const stepNumber = index + 1;

        let totalInstruments = 0;
        let selectedInstruments = 0; // Changed: count all selected instruments (both Ya and Tidak)
        ...
        if (totalInstruments > 0 && selectedInstruments === totalInstruments) {
            completedStepsCount++;
    ...
    if (progressBar && progressPercentage && progressStatus) {
        const overallPercentage = totalSteps > 0 ? Math.round((completedStepsCount / totalSteps) * 100) : 0;
        ...
        if (overallPercentage === 100) {
            progressStatus.textContent = 'Semua Sasaran Strategis telah selesai';
        } else if (overallPercentage > 0) {
            progressStatus.textContent = `${completedStepsCount}/${totalSteps} Sasaran Strategis selesai`;
        } else {
            progressStatus.textContent = 'Belum ada Sasaran Strategis yang selesai';
        }
    }
}
