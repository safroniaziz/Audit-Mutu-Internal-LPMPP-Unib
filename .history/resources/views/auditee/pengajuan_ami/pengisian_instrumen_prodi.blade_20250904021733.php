        ...
        <div class="card-body border-top p-9">
            @if($allKriterias->count() > 0)
                ...
                <div class="wizard-nav mb-5" style="overflow-x: scroll !important;">
                    <div style="min-width: calc(100% + 1px); height: 1px; position: absolute; pointer-events: none;"></div>
...
$(document).ready(function() {
    // Force scrollbar to appear on load - multiple approaches
    function forceScrollbar() {
        $('.wizard-nav').each(function() {
            const nav = $(this);
            
            // Method 1: Force overflow
            nav.css({
                'overflow-x': 'scroll',
                'scrollbar-width': 'thin',
                'min-width': '100%'
            ...
            // Method 2: Add invisible content to force overflow
            if (!nav.find('.force-scroll').length) {
                nav.append('<div class="force-scroll" style="width: 1px; height: 1px; flex-shrink: 0; opacity: 0;"></div>');
            }
            
            // Method 3: Trigger reflow
            this.style.display = 'none';
            this.offsetHeight; // Trigger reflow
            this.style.display = 'flex';
            
            // Method 4: Force browser to calculate scrollbar
            this.scrollLeft = 1;
            this.scrollLeft = 0;
        });
    }
    ...
    forceScrollbar();
    setTimeout(forceScrollbar, 50);
    setTimeout(forceScrollbar, 200);
