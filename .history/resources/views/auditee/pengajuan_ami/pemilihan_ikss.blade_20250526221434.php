    ...
    <div class="card mb-5 mb-xl-10" id="kt_profile_details_view">
        ...
        <div class="card-body border-top p-9">
            @php
                ...
                $ssInstrumenCounts = [];
                $satuanStandars = [];
                $allSSCompleted = true; // Add this flag
                ...
                foreach ($dataIkssProdi as $unit) {
                    foreach ($unit->indikatorKinerjas->groupBy('satuan_standar_id') as $ssId => $ssIndikators) {
                        ...
                        // If any SS is not completed, set allSSCompleted to false
                        if (!$ssInstrumenCounts[$ssId]['is_submitted']) {
                            $allSSCompleted = false;
                        }
                    }
                }

                // Update status sudah mengisi hanya jika SEMUA SS telah diisi
                $semuaInstrumenDiisi = $allSSCompleted;

                // Store the status in session for the layout to use
                session(['sudahMengisiIkss' => $semuaInstrumenDiisi]);

                // Debug information
                error_log("=== Overall Stats ===");
                error_log("Total Instrumen: " . $totalInstrumen);
                error_log("Total Diisi: " . $totalDiisi);
                error_log("All SS Completed: " . ($allSSCompleted ? 'Yes' : 'No'));
