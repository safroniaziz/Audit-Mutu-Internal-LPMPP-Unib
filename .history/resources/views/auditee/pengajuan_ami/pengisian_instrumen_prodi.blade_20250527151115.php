...
@php
    ...
    // Group by Instrumen -> Kriteria -> Indikator
    $groupedInstrumen = [];
    foreach ($indikatorInstrumens as $instrumen) {
        $instrumenName = $instrumen->instrumen->nama_instrumen;
        $kriteriaName = $instrumen->kriteria->nama_kriteria;

        if (!isset($groupedInstrumen[$instrumenName])) {
            $groupedInstrumen[$instrumenName] = [
                'nama_instrumen' => $instrumenName,
                'kriteria' => []
            ];
        }

        if (!isset($groupedInstrumen[$instrumenName]['kriteria'][$kriteriaName])) {
            $groupedInstrumen[$instrumenName]['kriteria'][$kriteriaName] = [
                'nama_kriteria' => $kriteriaName,
                'indikator' => []
            ];
        }

        $groupedInstrumen[$instrumenName]['kriteria'][$kriteriaName]['indikator'][] = $instrumen;
    }
@endphp
    ...
    <div class="card mb-5 mb-xl-10" id="kt_profile_details_view">
        ...
        <div class="card-body border-top p-9">
            ...
            <div class="wizard-nav mb-5">
                @foreach($groupedInstrumen as $instrumenName => $instrumenGroup)
                    <div class="wizard-step {{ $loop->first ? 'active' : '' }}" data-instrumen="{{ $instrumenName }}">
                        <div class="step-number">{{ $loop->iteration }}</div>
                        <div class="step-label">{{ $instrumenName }}</div>
                        <div class="step-desc">
                            {{ count($instrumenGroup['kriteria']) }} Kriteria
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            @foreach($groupedInstrumen as $instrumenName => $instrumenGroup)
                <div class="sub-wizard-nav {{ $loop->first ? 'active' : '' }}" data-instrumen="{{ $instrumenName }}">
                    @foreach($instrumenGroup['kriteria'] as $kriteriaName => $kriteria)
                        <div class="wizard-step {{ $loop->first ? 'active' : '' }}" data-kriteria="{{ $kriteriaName }}">
                            <div class="step-number">{{ $loop->iteration }}</div>
                            <div class="step-label">{{ $kriteriaName }}</div>
                            <div class="step-desc">
                                {{ count($kriteria['indikator']) }} Indikator
                            </div>
                        </div>
                    @endforeach
                </div>
            ...
            @foreach($groupedInstrumen as $instrumenName => $instrumenGroup)
                @foreach($instrumenGroup['kriteria'] as $kriteriaName => $kriteria)
                    <div class="wizard-content {{ ($loop->parent->first && $loop->first) ? 'active' : '' }}"
                         data-instrumen="{{ $instrumenName }}"
                         data-kriteria="{{ $kriteriaName }}">
                        <div class="card card-bordered mb-5">
                            <div class="card-header">
                                <h3 class="card-title">{{ $instrumenName }} - {{ $kriteriaName }}</h3>
                            </div>
                            <div class="card-body">
                                        ...
                                        <tbody>
                                            @foreach($kriteria['indikator'] as $indikator)
                                                <tr>
                                                    ...
                                                    <td>
                                                        @if($indikator->submission)
                                                            <span class="badge badge-light-success">{{ $indikator->submission->nilai }}</span>
                                                        @else
                                                            <span class="badge badge-light-warning">Belum diisi</span>
                                                        @endif
                                                    </td>
                                                    <td>
                                                        <span class="{{ $indikator->submission ? '' : 'text-muted' }}">
                                                            {{ $indikator->submission ? Str::limit($indikator->submission->deskripsi, 50) : 'Belum diisi' }}
                                                        </span>
                                                    </td>
                                                    <td>
                                                        @if($indikator->submission && $indikator->submission->dokumen->count() > 0)
                                                            <div class="d-flex flex-column gap-2">
                                                                @foreach($indikator->submission->dokumen as $dokumen)
                                                    ...
                                                    <td class="text-center">
                                                        <button type="button"
                                                                class="btn btn-sm {{ $indikator->submission ? 'btn-light-primary' : 'btn-primary' }} btn-edit"
                                                                data-instrumen-id="{{ $indikator->id }}"
                                                                data-nilai="{{ $indikator->submission ? $indikator->submission->nilai : '' }}"
                                                                data-deskripsi="{{ $indikator->submission ? $indikator->submission->deskripsi : '' }}">
                                                            <i class="bi {{ $indikator->submission ? 'bi-pencil' : 'bi-plus' }}"></i>
                                                            {{ $indikator->submission ? 'Edit' : 'Isi' }}
                                                        </button>
                                                    </td>
                                                </tr>
                                            @endforeach
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.wizard-nav .wizard-step').on('click', function() {
                const instrumenName = $(this).data('instrumen');
                ...
                $(`.sub-wizard-nav[data-instrumen="${instrumenName}"]`).addClass('active');

                // Show first content of this instrumen
                const firstKriteria = $(`.sub-wizard-nav[data-instrumen="${instrumenName}"] .wizard-step:first`).data('kriteria');
                showContent(instrumenName, firstKriteria);
            });

            // Handle sub-wizard navigation
            $('.sub-wizard-nav .wizard-step').on('click', function() {
                const instrumenName = $(this).closest('.sub-wizard-nav').data('instrumen');
                ...
                showContent(instrumenName, kriteriaName);
            });

            function showContent(instrumenName, kriteriaName) {
                $('.wizard-content').removeClass('active');
                $(`.wizard-content[data-instrumen="${instrumenName}"][data-kriteria="${kriteriaName}"]`).addClass('active');
            }
        });
    </script>
@endpush
