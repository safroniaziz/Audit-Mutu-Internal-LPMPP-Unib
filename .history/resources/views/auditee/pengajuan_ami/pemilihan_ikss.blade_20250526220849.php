                ...
                <div class="wizard-nav">
                    @foreach ($dataIkssProdi as $unit)
                        ...
                        @php
                            ...
                            $selectedYesInstruments = 0;

                            // Get all instrumen IDs for current SS
                            $currentSSInstrumenIds = collect($indikators)->flatMap(function ($indikator) {
                                return $indikator->instrumen->pluck('id');
                            })->toArray();

                            // Filter dataTerpilih for current SS only
                            $currentSSPilihan = array_filter(
                                $dataTerpilih ?? [],
                                function($key) use ($currentSSInstrumenIds) {
                                    $id = (int)str_replace('pilihan_', '', $key);
                                    return in_array($id, $currentSSInstrumenIds);
                                },
                                ARRAY_FILTER_USE_KEY
                            );

                            // Count instruments for current SS
                            foreach ($indikators as $indikator) {
                                foreach ($indikator->instrumen as $instrumen) {
                                    ...
                                    if ($instrumen->is_wajib == 1 &&
                                        ($instrumen->jenjang == 'Semua' || $instrumen->jenjang == optional(Auth::user()->unitKerja)->jenjang)) {
                                        $selectedInstruments++;
                                        $selectedYesInstruments++;
                                    } elseif (isset($currentSSPilihan['pilihan_'.$instrumen->id])) {
                                        $selectedInstruments++;
                                        if ($currentSSPilihan['pilihan_'.$instrumen->id] == 1) {
                                            $selectedYesInstruments++;
                                        }
                                    }
                                }
                            ...
                            // Step is completed if ALL instruments in this SS have been selected
                            $isSSSubmitted = $totalInstruments > 0 && $selectedInstruments === $totalInstruments;

                            // Debug information for current SS
                            error_log("=== SS {$satuanStandar->kode_satuan} ===");
                            error_log("Instrumen IDs in this SS: " . implode(', ', $currentSSInstrumenIds));
                            error_log("Filtered dataTerpilih for this SS: " . json_encode($currentSSPilihan));
                            error_log("Total Instruments: {$totalInstruments}");
                            error_log("Selected Instruments: {$selectedInstruments}");
                            error_log("Selected Yes: {$selectedYesInstruments}");
                        @endphp
