...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initialize completed steps based on database data
            let completedSteps = [];

            // Check each step to see if it's completed based on database data
            wizardContents.forEach((content, index) => {
                const stepNumber = index + 1;
                if (isStepCompletedFromDB(content)) {
                    completedSteps.push(stepNumber);
                }
            });

            // Update sessionStorage with correct completed steps
            ...
            // Update overall progress immediately after initialization
            updateOverallProgress();

            // Function to check if step is completed from database
            function isStepCompletedFromDB(stepContent) {
                ...
                stepContent.querySelectorAll('.d-flex.align-items-start.border').forEach(group => {
                    ...
                    // Check if instrument is selected (either wajib or from database)
                    if (isWajib) {
                        selectedInstruments++;
                    } else if (instrumenId && dataTerpilih[`pilihan_${instrumenId}`] !== undefined) {
                        selectedInstruments++; // Count any selection from database (Ya or Tidak)
                    }
                });
            ...
            function updateOverallProgress() {
                ...
                let allInstrumentsFilled = true; // Flag untuk mengecek semua instrumen terisi
                    ...
                    content.querySelectorAll('.d-flex.align-items-start.border').forEach(group => {
                        ...
                        if (isWajib) {
                            ...
                            if (checkedRadio) {
                                ...
                                allInstrumentsFilled = false; // Set false jika ada instrumen yang belum diisi
                            }
                        }
                    });
                });
                ...
                const nextButton = document.querySelector('.btn-primary[href="{{ route("auditee.pengajuanAmi.pengisianInstrumen") }}"]');
                const disabledButton = document.querySelector('.btn-secondary.disabled');
                ...
                if (progressBar && progressPercentage && progressStatus) {
                    ...
                    // Update alert status based on whether all instruments are filled
                    const alertDiv = document.querySelector('.alert');
                    const alertIcon = alertDiv.querySelector('.bi');
                    const alertTitle = alertDiv.querySelector('.fw-bold.text-dark');
                    const alertMessage = alertDiv.querySelector('.fw-semibold');

                    if (allInstrumentsFilled && overallPercentage === 100) {
                        // Semua instrumen telah diisi
                        alertDiv.className = 'alert alert-success d-flex align-items-start p-5 mb-10';
                        alertIcon.className = 'bi bi-check-circle-fill fs-2 text-success';
                        alertTitle.textContent = '✨ IKSS Telah Dipilih';
                        alertMessage.className = 'fw-semibold text-success';
                        alertMessage.innerHTML = 'IKSS telah dipilih dengan lengkap. Silakan lanjut ke tahap pengisian Instrumen Audit.';

                        // Show next button
                        if (nextButton) nextButton.style.display = 'inline-flex';
                        if (disabledButton) disabledButton.style.display = 'none';
                    } else {
                        // Masih ada instrumen yang belum diisi
                        alertDiv.className = 'alert alert-danger d-flex align-items-start p-5 mb-10';
                        alertIcon.className = 'bi bi-exclamation-triangle-fill fs-2 text-danger';
                        alertTitle.textContent = '📢 Pilih IKSS';
                        alertMessage.className = 'fw-semibold text-danger';
                        alertMessage.innerHTML = 'Silakan lengkapi pengisian data <strong>IKSS</strong> di bawah ini secara menyeluruh untuk dapat melanjutkan ke tahap <strong>pengisian Instrumen Audit</strong>.';

                        // Hide next button
                        if (nextButton) nextButton.style.display = 'none';
                        if (disabledButton) disabledButton.style.display = 'inline-flex';
                    }
            ...
            // Add event listener for radio button changes to update alert status
