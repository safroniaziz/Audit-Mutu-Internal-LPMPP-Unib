                                    ...
                                    <form class="kriteria-form" data-kriteria-id="{{ $kriteriaId }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="d-flex justify-content-between">
                                            @php
                                                ...
                                                // Debug info
                                                $currentKriteriaKode = $allKriterias[$currentKriteriaIndex]['kriteria']->kode_kriteria ?? 'Unknown';
                                                $totalKriterias = $allKriterias->count();
                                            @endphp

                                            <!-- Debug Info (temporary) -->
                                            <div class="alert alert-info mb-3" style="font-size: 12px;">
                                                <strong>Debug:</strong> Kriteria {{ $currentKriteriaKode }}
                                                (Index: {{ $currentKriteriaIndex }}/{{ $totalKriterias - 1 }},
                                                Total: {{ $totalKriterias }},
                                                Is Last: {{ $isLastKriteria ? 'Yes' : 'No' }})
                                                <br><strong>All Kriterias:</strong>
                                                @foreach($allKriterias as $idx => $krit)
                                                    {{ $krit['kriteria']->kode_kriteria }}{{ !$loop->last ? ', ' : '' }}
                                                @endforeach
                                            </div>                                            @if($prevKriteria)
