            ...
            <form action="{{ route('auditee.submitAllInstrumen') }}" method="POST" enctype="multipart/form-data" id="formInstrumen">
                ...
                @foreach($groupedInstrumen as $ssId => $group)
                    <div class="wizard-content {{ $loop->first ? 'active' : '' }}" data-step="{{ $ssId }}">
                        ...
                        <div class="alert {{ $isCompleted ? 'alert-primary' : 'alert-danger' }} d-flex align-items-center p-5 mb-10">
                            <i class="ki-duotone {{ $isCompleted ? 'ki-check-circle fs-2 text-primary' : 'ki-cross-circle fs-2 text-danger' }} me-4">
                                <span class="path1"></span>
                                <span class="path2"></span>
                            </i>
                            <div class="d-flex flex-column">
                                <h4 class="mb-1 text-{{ $isCompleted ? 'primary' : 'danger' }}">Status Pengisian {{ $group['satuan_standar']->kode_satuan }}</h4>
                                <span>{{ $completedInstrumen }} dari {{ $totalInstrumen }} instrumen telah diisi lengkap</span>
                            </div>
                        </div>
