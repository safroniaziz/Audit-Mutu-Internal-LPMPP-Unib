        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    // Handle success lainnya (optional)
                    console.log(response.message);
                }
            },
            error: function(xhr) {
                // Menangani kesalahan, menampilkan pesan validasi menggunakan SweetAlert
                if (xhr.status === 422) {
                    var errors = xhr.responseJSON.errors;
                    var errorMessage = '';

                    // Menyusun pesan error menjadi daftar
                    var errorList = '<ul>';
                    ...
                    // Menampilkan SweetAlert dengan error sebagai list
                    Swal.fire({
                        icon: 'error',
                        title: 'Validasi Gagal!',
                        html: errorList, // Menggunakan HTML untuk menampilkan list
                    });
                } else {
                    // Menampilkan error umum jika terjadi kesalahan lain
                    Swal.fire({
                        icon: 'error',
