        ...
        <div class="card-body border-top p-9">
            ...
            @foreach($groupedInstrumen as $ssId => $group)
                <div class="wizard-content {{ $loop->first ? 'active' : '' }}" data-step="{{ $ssId }}">
                    ...
                    <form action="{{ route('auditee.submitInstrumenSS', ['ss_id' => $ssId]) }}" method="POST" enctype="multipart/form-data" class="form-ss" id="formInstrumen_{{ $ssId }}">
                        @csrf
                            ...
                            <div class="d-flex gap-2">
                                @if(!$ikssAuditeeData->isNotEmpty())
                                    <button type="submit" class="btn btn-primary">
                                        <i class="fas fa-save"></i> Simpan {{ $group['satuan_standar']->kode_satuan }}
                                    </button>
                                @endif
