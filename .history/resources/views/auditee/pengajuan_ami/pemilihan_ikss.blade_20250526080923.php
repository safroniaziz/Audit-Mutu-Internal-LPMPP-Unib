                    ...
                    @foreach ($dataIkssProdi as $unit)
                        @foreach ($unit->indikatorKinerjas->groupBy('satuan_standar_id') as $satuanId => $indikators)
                            @php
                                ...
                                $mandatoryInstruments = 0;

                                // Count instruments and selected ones
                                foreach ($indikators as $indikator) {
                                    foreach ($indikator->instrumen as $instrumen) {
                                        if ($instrumen->is_wajib == 1) {
                                            ...
                                            $totalInstruments++;
                                            if (isset($dataTerpilih['pilihan_'.$instrumen->id]) &&
                                                $dataTerpilih['pilihan_'.$instrumen->id] == 1) {
                                                $selectedInstruments++;
                                            }
                                        }
                                    }
                                ...
                                $totalRequired = $totalInstruments + $mandatoryInstruments;
                                $isSSSubmitted = $selectedInstruments > 0;
                                $progressPercentage = $totalRequired > 0 ? ($selectedInstruments / $totalRequired) * 100 : 0;
                            @endphp
                    ...
                    @foreach ($unit->indikatorKinerjas->groupBy('satuan_standar_id') as $satuanId => $indikators)
                        ...
                        <div class="wizard-content" data-step="{{ $loop->iteration }}" data-ss-id="{{ $satuanId }}">
                            <div class="alert bg-light-{{ $isSSSubmitted ? 'success' : 'warning' }} mb-5">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="d-flex flex-column">
                                        <h4 class="mb-1 text-{{ $isSSSubmitted ? 'success' : 'warning' }}">
                                            Status: <span class="alert-status">{{ $isSSSubmitted ? 'Sudah Diisi' : 'Belum Diisi' }}</span>
                                        </h4>
                                        <span class="satuan-standar-info">{{ $satuanStandar->kode_satuan }} - {{ $satuanStandar->sasaran }}</span>
                                        <span class="text-muted mt-1 progress-status-ss">Progress: {{ $selectedInstruments }}/{{ $totalInstruments }} instrumen</span>
                                    </div>
                                </div>
                            </div>

                            <div class="progress h-8px bg-light rounded mb-5">
                                <div class="progress-bar-ss bg-primary rounded h-8px" role="progressbar"
                            ...
                            <div class="timeline timeline-border-dashed">
                                @foreach ($indikators as $indikator)
                                    @foreach ($indikator->instrumen as $instrumen)
                                        <div class="instrument-group timeline-item mb-5">
                                            <div class="timeline-label">
                                                <span class="fw-bold text-gray-800">{{ $instrumen->kode }}</span>
                                            </div>
                                            <div class="timeline-badge">
                                                <i class="fa fa-genderless text-{{ isset($dataTerpilih['pilihan_'.$instrumen->id]) && $dataTerpilih['pilihan_'.$instrumen->id] == 1 ? 'success' : 'warning' }} fs-1"></i>
                                            </div>
                                            <div class="timeline-content ps-3">
                                                <div class="d-flex justify-content-between align-items-center mb-2">
                                                    <span class="fs-6 fw-bold text-gray-800">{{ $instrumen->nama }}</span>
                                                </div>
                                                <div class="d-flex gap-4">
                                                    @if($instrumen->is_wajib)
                                                        <input type="hidden" name="is_wajib" value="1">
                                                    @endif
                                                    <div class="form-check form-check-custom form-check-success">
                                                        <input class="form-check-input" type="radio" name="pilihan_{{ $instrumen->id }}" value="1"
                                                               {{ isset($dataTerpilih['pilihan_'.$instrumen->id]) && $dataTerpilih['pilihan_'.$instrumen->id] == 1 ? 'checked' : '' }}>
                                                        <label class="form-check-label">Ya</label>
                                                    </div>
                                                    <div class="form-check form-check-custom form-check-danger">
                                                        <input class="form-check-input" type="radio" name="pilihan_{{ $instrumen->id }}" value="0"
                                                               {{ isset($dataTerpilih['pilihan_'.$instrumen->id]) && $dataTerpilih['pilihan_'.$instrumen->id] == 0 ? 'checked' : '' }}>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function calculateOverallProgress() {
                ...
                document.querySelectorAll('.instrument-group').forEach(group => {
                    const isWajib = group.querySelector('input[name="is_wajib"]')?.value === '1';
                    ...
                    if (isWajib) {
                        ...
                        totalInstruments++;
                        // Check if instrument is selected in database or UI
                        const isSelectedInDB = instrumenId && dataTerpilih[`pilihan_${instrumenId}`] === 1;
                        const isSelectedInUI = group.querySelector('input[type="radio"][value="1"]:checked');

                        if (isSelectedInDB || isSelectedInUI) {
                            selectedCount++;
                        }
                    }
                });

                const totalRequired = totalInstruments + mandatoryCount;
                return {
                    percentage: totalRequired > 0 ? (selectedCount / totalRequired) * 100 : 0,
            ...
            function updateMainProgress() {
                ...
                const mainPercentage = document.querySelector('.progress-percentage');
                const mainStatus = document.querySelector('.progress-status');

                if (mainProgressBar) {
                    mainProgressBar.style.width = `${progress.percentage}%`;
                    mainProgressBar.setAttribute('aria-valuenow', progress.percentage);
                }

                if (mainPercentage) {
                    mainPercentage.textContent = `${Math.round(progress.percentage)}%`;
                }

                if (mainStatus) {
                    mainStatus.textContent = `Total instrumen yang telah dipilih: ${progress.selected} dari ${progress.total} instrumen`;
                }
            }

            // Set initial radio button states and update progress
            ...
            updateMainProgress();
            updateProgress(); // This updates the current SS progress
            ...
            function updateProgress() {
                ...
                // Get SS ID for current step
                const ssId = activeContent.dataset.ssId;

                // Calculate current SS progress including mandatory instruments
                let totalInstruments = 0;
                let selectedCount = 0;

                // Count all instruments and their status
                activeContent.querySelectorAll('.instrument-group').forEach(group => {
                    totalInstruments++;
                    ...
                    // Check if instrument is selected in database or UI
                    const isSelectedInDB = instrumenId && dataTerpilih[`pilihan_${instrumenId}`] === '1';
                    const isSelectedInUI = group.querySelector('input[type="radio"]:checked')?.value === '1';

                    if (isWajib || isSelectedInDB || isSelectedInUI) {
                        selectedCount++;
                    }
                });

                const progressPercentage = (selectedCount / totalInstruments) * 100;

                // Get current SS info
                const satuanStandar = activeContent.querySelector('.satuan-standar-info')?.textContent || '';

                // Update progress bar
                const progressBar = activeContent.querySelector('.progress-bar-ss');
                if (progressBar) {
                ...
                const percentageDisplay = activeContent.querySelector('.progress-percentage-ss');
                if (percentageDisplay) {
                    percentageDisplay.textContent = `${Math.round(progressPercentage)}%`;
                }

                // Update status text
                const statusText = activeContent.querySelector('.progress-status-ss');
                if (statusText) {
                    statusText.textContent = `Instrumen yang telah dipilih: ${selectedCount} dari ${totalInstruments} instrumen`;
                }

                // Update completion status
                const alertStatus = activeContent.querySelector('.alert-status');
                if (alertStatus) {
                    if (selectedCount === totalInstruments) {
                        alertStatus.textContent = 'Sudah Diisi';
                        alertStatus.classList.remove('text-warning');
                        alertStatus.classList.add('text-success');
                    } else {
                        alertStatus.textContent = 'Belum Diisi';
                        alertStatus.classList.remove('text-success');
