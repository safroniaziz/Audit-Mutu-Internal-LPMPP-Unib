                                    ...
                                    <td class="text-center">
                                        <div class="button-container">
                                            <button type="button" class="btn btn-sm btn-light-success edit-unitkerja"
                                                ...
                                                data-url="{{ route('indikator.edit', $indikator->id) }}"
...
@push('scripts')
    ...
    <script>
        ...
        $(document).on('click', '.restore-data', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('indikator.restore', ':id') }}".replace(':id', id),
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            _method: 'PUT'
        ...
        $('.edit-unitkerja').click(function() {
            ...
            $('#methodField').val('PUT');
            $('#kt_modal_form').attr('action', "{{ route('indikator.update', '') }}/" + id);
        ...
        $('button[data-bs-target="#kt_modal"]:not(.edit-unitkerja)').click(function() {
            ...
            $('#kt_modal_form').attr('action', "{{ route('indikator.store') }}");
        ...
        $(document).on('click', '.nonaktifkan-unitKerja', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('indikator.nonaktifkan', '') }}/" + id,
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            _method: 'DELETE'
        ...
        $(document).on('click', '[data-kt-unker-table-select="delete_selected"]', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('indikator.nonaktifkanSelected') }}",
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            ids: ids
        ...
        $(document).on('click', '.delete-permanent', function () {
            var id = $(this).data('id');
            var url = "{{ route('indikator.hapus_permanen', ':id') }}";
