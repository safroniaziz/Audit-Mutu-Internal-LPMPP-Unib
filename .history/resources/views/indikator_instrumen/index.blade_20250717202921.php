...
@push('scripts')
    ...
    <script>
        ...
        // Handle view elemen button click
        $(document).on('click', '.view-elemen', function() {
            let id = $(this).data('id');

            // Show loading
            $('#elemenTableBody').html('<tr><td colspan="6" class="text-center"><i class="fas fa-spinner fa-spin"></i> Loading...</td></tr>');

            $.ajax({
                url: "{{ route('indikatorInstrumen.getElemen', ':id') }}".replace(':id', id),
                type: 'GET',
                success: function(response) {
                    if (response.success) {
                        $('#indikatorName').text(response.indikator);
                            ...
                            response.data.forEach(function(item, index) {
                                tableBody += `
                                    <tr>
                                        ...
                                        <td>${item.standar_digunakan}</td>
                                        <td>${item.uraian || '-'}</td>
                                    </tr>
                                `;
                            });
                        } else {
                            tableBody = '<tr><td colspan="6" class="text-center">Tidak ada elemen yang ditemukan</td></tr>';
                        }

                        $('#elemenTableBody').html(tableBody);
                    } else {
                        $('#elemenTableBody').html('<tr><td colspan="6" class="text-center text-danger">Gagal memuat data elemen</td></tr>');
                    }
                },
                error: function(xhr) {
                    $('#elemenTableBody').html('<tr><td colspan="6" class="text-center text-danger">Terjadi kesalahan saat memuat data</td></tr>');
                }
            });
        });
