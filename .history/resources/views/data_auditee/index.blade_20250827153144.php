...
<script>
    ...
    $('.editAuditeeButton').click(function() {
        ...
        $.ajax({
            ...
            type: 'GET',
            success: function(response) {
                // Jika unit kerja saat ini milik auditee tidak ada di dropdown (karena difilter), tambahkan sementara
                if (response.data.unit_kerja_id && response.data.unit_kerja_nama && response.data.unit_kerja_jenis) {
                    const select = $('[name="unit_kerja_id"]');
                    const exists = select.find(`option[value="${response.data.unit_kerja_id}"]`).length > 0;
                    if (!exists) {
                        // Pastikan ada optgroup untuk jenis yang sesuai
                        let optgroup = select.find(`optgroup[label="${response.data.unit_kerja_jenis.charAt(0).toUpperCase() + response.data.unit_kerja_jenis.slice(1)}"]`);
                        if (optgroup.length === 0) {
                            optgroup = $(`<optgroup label="${response.data.unit_kerja_jenis.charAt(0).toUpperCase() + response.data.unit_kerja_jenis.slice(1)}"></optgroup>`);
                            select.append(optgroup);
                        }
                        optgroup.append(`<option value="${response.data.unit_kerja_id}">${response.data.unit_kerja_nama}</option>`);
                    }
                }
