...
<head>
    ...
    <style>
        ...
        .custom-error-icon {
            ...
            color: white;
            font-size: 12px;
            animation: bounceError 1s infinite;
        }
            ...
            <div class="w-full md:w-2/5 p-8 flex flex-col justify-center space-y-6">
                ...
                @if (session('success'))
                    <div class="custom-success">
                        ...
                        <div class="custom-error-content">
                            <div>
                                <div class="custom-error-title">Berhasil!</div>
                                <div class="custom-error-message">{{ session('success') }}</div>
                            </div>
                        </div>
                    </div>
                @endif
                ...
                @if (session('error'))
                    <div class="custom-error">
                        ...
                        <div class="custom-error-content">
                            <div>
                                <div class="custom-error-title">Terjadi Kesalahan</div>
                                <div class="custom-error-message">{{ session('error') }}</div>
                            </div>
                        </div>
                    </div>
                @endif
                ...
                @if ($errors->any())
                    @foreach ($errors->all() as $error)
                        <div class="bg-gradient-to-r from-amber-50 to-orange-100 border-l-4 border-amber-500 p-6 rounded-xl shadow-lg" data-aos="fade-right" data-aos-delay="400">
                            <div class="flex items-start">
                                <div class="bg-gradient-to-r from-amber-500 to-orange-500 rounded-full p-3 mr-4 text-white flex-shrink-0 shadow-md">
                                    <i class="fas fa-clock text-xl"></i>
                                </div>
                                <div class="flex-1">
                                    <div class="flex items-center justify-between mb-3">
                                        <h3 class="font-bold text-amber-800 text-lg">Jadwal Akses Terbatas</h3>
                                        <span class="bg-amber-100 text-amber-800 text-xs font-semibold px-3 py-1 rounded-full border border-amber-300">
                                            <i class="fas fa-calendar-alt mr-1"></i>Periode Aktif
                                        </span>
                                    </div>

                                    <!-- Main Error Message -->
                                    <div class="bg-white bg-opacity-60 rounded-lg p-4 border border-amber-200 mb-4">
                                        <div class="flex items-center mb-2">
                                            <i class="fas fa-exclamation-circle text-amber-600 mr-2"></i>
                                            <span class="font-semibold text-amber-800">Pesan Sistem:</span>
                                        </div>
                                        <p class="text-amber-700 font-medium">{{ $error }}</p>
                                    </div>

                                    <!-- Additional Info Box -->
                                    <div class="bg-white bg-opacity-50 rounded-lg p-4 border border-amber-200 mb-4">
                                        <div class="flex items-center mb-3">
                                            <i class="fas fa-info-circle text-amber-600 mr-2"></i>
                                            <span class="font-semibold text-amber-800">Informasi Penting:</span>
                                        </div>
                                        <ul class="text-amber-700 text-sm space-y-2">
                                            <li class="flex items-start">
                                                <i class="fas fa-check-circle text-amber-500 mr-2 mt-0.5 text-xs"></i>
                                                <span>Sistem hanya dapat diakses pada periode yang telah ditentukan</span>
                                            </li>
                                            <li class="flex items-start">
                                                <i class="fas fa-check-circle text-amber-500 mr-2 mt-0.5 text-xs"></i>
                                                <span>Silakan coba kembali pada waktu yang telah dijadwalkan</span>
                                            </li>
                                            <li class="flex items-start">
                                                <i class="fas fa-check-circle text-amber-500 mr-2 mt-0.5 text-xs"></i>
                                                <span>Hubungi administrator jika ada pertanyaan terkait jadwal</span>
                                            </li>
                                        </ul>
                                    </div>

                                    <!-- Current Time & Contact Info -->
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        <div class="text-center">
                                            <div class="inline-flex items-center bg-amber-100 text-amber-800 px-4 py-2 rounded-lg text-sm font-medium border border-amber-300">
                                                <i class="fas fa-calendar-day mr-2"></i>
                                                <span>Waktu Sekarang: {{ now()->format('d/m/Y H:i:s') }}</span>
                                            </div>
                                        </div>
                                        <div class="text-center">
                                            <div class="inline-flex items-center bg-blue-100 text-blue-800 px-4 py-2 rounded-lg text-sm font-medium border border-blue-300">
                ...
                <div class="text-center text-sm text-gray-500 pt-4 border-t">
                    <p>LPMPP Universitas Bengkulu &copy; 2025</p>
                </div>
            </div>
                    ...
                    <div>
                        ...
                        <div class="documents-container" data-aos="fade-up" data-aos-delay="300">
                            <div class="space-y-2 max-h-[550px] overflow-y-auto pr-2" id="documents-list">
                                <ul class="space-y-2">
                                    @php
                                        $dokumenAmis = \App\Models\DokumenAmi::where('kategori_dokumen', 'umum')->get();
                                    @endphp
                                    @foreach ($dokumenAmis as $dokumen)
                                        <li class="download-item flex items-center bg-white bg-opacity-10 backdrop-blur-sm p-3 rounded-xl hover:cursor-pointer my-2">
                                            <div class="mr-3 bg-white rounded-lg p-2 text-blue-600">
                                                <i class="fas fa-file-alt text-xl"></i>
                                            </div>
                                            <div class="flex-1">
                                                <h3 class="font-medium text-sm md:text-base">{{ $dokumen->nama_dokumen }}</h3>
                                                <p class="text-xs text-blue-100">Dokumen - {{ $dokumen->created_at->format('d M Y') }}</p>
                                            </div>
                                            <a href="{{ $dokumen->path }}" target="_blank" class="download-btn bg-white text-blue-600 rounded-lg p-2 hover:bg-blue-50 transition">
