                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <div class="button-container">
                                            <button type="button" class="btn btn-sm btn-light-success edit-satuan"
                                                data-id="{{ $satuanStandar->id }}"
                                                data-url="{{ route('satuan.edit', $satuanStandar->id) }}"
                                            ...
                                            @else
                                                <button type="button" class="btn btn-sm btn-light-danger nonaktifkan-satuan" data-id="{{ $satuanStandar->id }}">
...
@push('scripts')
    ...
    <script>
        ...
        $(document).on('click', '.restore-data', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('satuan.restore', ':id') }}".replace(':id', id),
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            _method: 'PUT'
        ...
        $('.edit-satuan').click(function() {
            ...
            $('#methodField').val('PUT');
            $('#kt_modal_form').attr('action', "{{ route('satuan.update', '') }}/" + id);
        ...
        $('button[data-bs-target="#kt_modal"]:not(.edit-satuan)').click(function() {
            $('#kt_modal form')[0].reset();
            $('#methodField').val('POST');
            $('#kt_modal_form').attr('action', "{{ route('satuan.store') }}");
        ...
        // Script untuk delete
        $(document).on('click', '.nonaktifkan-satuan', function() {
            let id = $(this).data('id');
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('satuan.nonaktifkan', '') }}/" + id,
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            _method: 'DELETE'
        ...
        $(document).on('click', '[data-kt-unker-table-select="delete_selected"]', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('satuan.nonaktifkanSelected') }}",
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            ids: ids
        ...
        $(document).on('click', '.delete-permanent', function () {
            var id = $(this).data('id');
            var url = "{{ route('satuan.hapus_permanen', ':id') }}";
