...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    // Route untuk testing tanpa role restriction
    Route::put('/test-lengkapiProfil-all-roles', [AuditeePengajuanAmiController::class, 'lengkapiProfil'])->name('test.lengkapiProfil.all.roles');

    Route::middleware('check.role:Auditee')->group(function () {
        // Debug route to check user roles (bypasses role middleware)
        Route::get('/debug-user-roles', function () {
            ...
            return response()->json([
                ...
                'has_role_auditee' => $user ? $user->hasRole('Auditee') : false,
                'has_role_administrator' => $user ? $user->hasRole('Administrator') : false,
                'has_role_auditor' => $user ? $user->hasRole('Auditor') : false,
                'is_authenticated' => \Illuminate\Support\Facades\Auth::check(),
            ]);
        })->name('debug.user.roles')->withoutMiddleware(['role:Auditee']);

        // Test route without any middleware
        Route::put('/test-lengkapiProfil', [AuditeePengajuanAmiController::class, 'lengkapiProfil'])->name('test.lengkapiProfil')->withoutMiddleware(['auth', 'verified']);

        // Test route that bypasses ALL middleware
        Route::put('/test-lengkapiProfil-no-middleware', [AuditeePengajuanAmiController::class, 'lengkapiProfil'])->name('test.lengkapiProfil.no.middleware')->withoutMiddleware(['web', 'auth', 'verified', 'role:Auditee']);
        ...
        // Route lengkapiProfil di luar group untuk menghindari konflik middleware
        Route::put('/auditee/lengkapiProfil', [AuditeePengajuanAmiController::class, 'lengkapiProfil'])->name('auditee.pengajuanAmi.lengkapiProfil')->middleware('check.role:Auditee');
