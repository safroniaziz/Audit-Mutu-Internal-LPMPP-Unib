    ...
    public function daftarPertanyaan()
{
    ...
    // Menentukan opsi sebelum load view
    $options = new \Dompdf\Options();
    $options->set('isHtml5ParserEnabled', true);
    $options->set('isRemoteEnabled', true);
    $options->set('defaultFont', 'sans-serif');
    $options->set('margin_left', 2);
    $options->set('margin_right', 2);
    $options->set('margin_top', 2);
    $options->set('margin_bottom', 2);

    // Membuat instance PDF dengan opsi kustom
    $pdf = PDF::loadView('dataauditor.pdf.daftar_pertanyaan', $data);
    $pdf->setOptions($options->__toArray());
    $pdf->setPaper('A4');
