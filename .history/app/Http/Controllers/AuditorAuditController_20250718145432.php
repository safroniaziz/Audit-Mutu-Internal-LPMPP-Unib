...
class AuditorAuditController extends Controller
{
    ...
    public function approvePenilaianProdi(PengajuanAmi $pengajuan)
    {
        try {
            ...
            // Update pengajuan status to approved for penilaian prodi
            $pengajuan->update([
                'status_penilaian_prodi' => 'approved',
                'approved_penilaian_prodi_at' => now(),
                'approved_penilaian_prodi_by' => Auth::id()
            ]);
    ...
    public function laporanAmi(Request $request, PengajuanAmi $pengajuan)
    {
        ...
        // Get Instrumen Prodi data - simplified calculation per kriteria
        ...
        foreach ($kriterias as $kriteria) {
            $auditor1Scores = collect();
            $auditor2Scores = collect();
                    ...
                    if ($nilai->nilai) {
                        ...
                        if ($penugasan) {
                            if ($penugasan->role == 'ketua') {
                                $auditor1Scores->push((float)$nilai->nilai);
                            } elseif ($penugasan->role == 'pendamping') {
                                $auditor2Scores->push((float)$nilai->nilai);
                            }
                        }
                    }
                }
            ...
            $totalScore = $auditor1Scores->sum() + $auditor2Scores->sum();
            $averageScore = ($auditor1Scores->count() + $auditor2Scores->count()) > 0
                ? $totalScore / ($auditor1Scores->count() + $auditor2Scores->count())
                : 0;

            $kriteriaScores[] = [
                'kode_kriteria' => $kriteria->kode_kriteria,
                'kriteria' => $kriteria->kriteria,
                'auditor1_score' => $auditor1Scores->sum(),
                'auditor2_score' => $auditor2Scores->sum(),
                'total_score' => $totalScore,
                'average_score' => $averageScore,
                'auditor1_count' => $auditor1Scores->count(),
                'auditor2_count' => $auditor2Scores->count()
            ];
        }
        ...
        $data = [
            ...
            'jawabanKuisioner'   =>  $jawabanKuisioner,
            'groupedInstrumenProdi' => $groupedInstrumenProdi,
        ];
