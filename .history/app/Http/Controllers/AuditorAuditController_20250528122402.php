...
class AuditorAuditController extends Controller
{
    ...
    public function visitasi(PengajuanAmi $pengajuan)
    {
        ...
        // Debug statements
        dd([
            'auditee_id' => $pengajuan->auditee_id,
            'periode_id' => $pengajuan->periode_id,
            'dataIkss count' => $dataIkss->count(),
            'groupedIkss count' => $groupedIkss->count(),
            'groupedIkss keys' => $groupedIkss->keys(),
            'first group sample' => $groupedIkss->first()->groupBy(function($item) {
