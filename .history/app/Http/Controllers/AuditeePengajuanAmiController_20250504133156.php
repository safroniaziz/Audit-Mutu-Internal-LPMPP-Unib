...
class AuditeePengajuanAmiController extends Controller
{
    ...
    public function submitAllInstrumen(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            // Pesan error untuk array
            'realisasi.required' => 'Data realisasi wajib diisi.',
            'realisasi.array' => 'Format data realisasi tidak valid.',
            'akar_penyebab.required' => 'Data akar penyebab wajib diisi.',
            'akar_penyebab.array' => 'Format data akar penyebab tidak valid.',
            'rencana_perbaikan.required' => 'Data rencana perbaikan wajib diisi.',
            'rencana_perbaikan.array' => 'Format data rencana perbaikan tidak valid.',
            'uraian.required' => 'Data uraian wajib diisi.',
            'uraian.array' => 'Format data uraian tidak valid.',
            'bukti_file.required' => 'Data bukti file wajib diisi.',
            'bukti_file.array' => 'Format data bukti file tidak valid.',

            // Pesan error untuk elemen array
            'realisasi.*.required' => 'Semua kolom realisasi wajib diisi.',
            'uraian.*.required' => 'Semua kolom uraian wajib diisi.',
            'akar_penyebab.*.required' => 'Semua kolom akar penyebab wajib diisi.',
            'rencana_perbaikan.*.required' => 'Semua kolom rencana perbaikan wajib diisi.',
            'bukti_file.*.required' => 'Semua bukti file wajib diunggah.',
        ...
        if ($validator->fails()) {
            // Kita akan membuat kumpulan error custom
            $customErrors = new \Illuminate\Support\MessageBag();

            // Kita akan membuat daftar field yang ingin kita gabungkan pesannya
            $arrayFields = [
                'realisasi' => 'Semua kolom realisasi wajib diisi.',
                'uraian' => 'Semua kolom uraian wajib diisi.',
                'akar_penyebab' => 'Semua kolom akar penyebab wajib diisi.',
                'rencana_perbaikan' => 'Semua kolom rencana perbaikan wajib diisi.',
                'bukti_file' => 'Semua bukti file wajib diunggah.'
            ];
            ...
            foreach ($errors as $key => $messages) {
                // Pisahkan key untuk cek apakah ini error pada elemen array
                $keyParts = explode('.', $key);

                // Jika ini adalah error pada elemen array (misal realisasi.0, realisasi.1)
                if (count($keyParts) > 1 && is_numeric($keyParts[1]) && isset($arrayFields[$keyParts[0]])) {
                    $baseField = $keyParts[0]; // field dasar (realisasi, akar_penyebab, dll)

                    // Jika belum ada error untuk field dasar ini, tambahkan
                    if (!$customErrors->has($baseField)) {
                        $customErrors->add($baseField, $arrayFields[$baseField]);
                    }
                } else {
                    // Jika bukan elemen array, tambahkan error seperti biasa
