...
class ActivityLogController extends Controller
{
    ...
    public function getActivities(Request $request): JsonResponse
    {
        try {
            ...
            $readActivities = \App\Models\ReadActivity::where('user_id', $userId)
                ->pluck('activity_id')
                ->toArray();

            // Transform activities to frontend format
            $transformedActivities = $activities->getCollection()->map(function ($activity) use ($readActivities) {
                return $this->transformActivity($activity, $readActivities);
            });

            // Count unread activities (activities not in read_activities table)
            $unreadCount = Activity::whereNotIn('id', $readActivities)->count();
     ...
     */
    private function transformActivity(Activity $activity, array $readActivities): array
    {
        $isUnread = !in_array($activity->id, $readActivities);
