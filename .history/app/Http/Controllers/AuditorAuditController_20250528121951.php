...
class AuditorAuditController extends Controller
{
    ...
    public function visitasi(PengajuanAmi $pengajuan)
    {
        ...
        $dataIkss = IkssAuditee::with([
                'instrumen.indikatorKinerja',
                'visitasi' => function($query) use ($pengajuan) {
        ...
        $auditor = PengajuanAmi::with(['auditors' => function($query) {
        ...
        $setuju = $auditor ? $auditor->is_setuju_visitasi : false;

        // For debugging
        \Log::info('Visitasi Data:', [
            'dataIkss count' => $dataIkss->count(),
            'visitasi count' => $visitasi->count(),
            'auditor' => $auditor,
            'setuju' => $setuju
        ]);
