...
class AuditeePengajuanAmiController extends Controller
{
    ...
    public function pengisianInstrumen()
    {
        ...
        // ambil semua data IKSS Auditee untuk instrumen yang dipilih (baik yang sudah diisi maupun belum)
    ...
    public function submitAllInstrumen(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'realisasi.*.present' => 'Realisasi wajib diisi.',
        ...
        // Additional validation for realisasi - allow 0 but not empty string or null
        foreach ($instrumenIds as $instrumenId) {
            $realisasi = $request->input('realisasi.' . $instrumenId);
            if ($realisasi === null || $realisasi === '') {
                return response()->json([
                    'success' => false,
                    'message' => 'Realisasi harus diisi untuk semua instrumen',
                    'errors' => ['realisasi.' . $instrumenId => ['Realisasi wajib diisi']]
                ], 422);
            }
            ...
            if (!is_numeric($realisasi)) {
                return response()->json([
                    ...
                    'errors' => ['realisasi.' . $instrumenId => ['Realisasi harus berupa angka']]
                ], 422);
            }
        }
    ...
    public function submitInstrumenProdi(Request $request, $kriteria_id)
    {
        try {
            ...
            foreach ($instrumenProdis as $instrumenProdi) {
                ...
                // Validate required fields - allow 0 as valid value
                if ($realisasi === null || $realisasi === '') {
