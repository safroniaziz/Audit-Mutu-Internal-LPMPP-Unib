...
class SatuanStandarController extends Controller
{
    public function index(){
        $ikss = SatuanStandar::with(['kriteriaInstrumen'])->orderBy('created_at','desc')->withTrashed()->get();
    ...
    {
        $instrumen = SatuanStandar::with('kriteriaInstrumen')->findOrFail($id);
        return response()->json([
            'kriteria' => $instrumen->kriteriaInstrumen->nama_kriteria,
            'indikator' => $instrumen->indikator,
            'elemen' => $instrumen->elemen,
            'uraian' => $instrumen->uraian,
            'sumber_data' => $instrumen->sumber_data,
            'standar_digunakan' => $instrumen->standar_digunakan,
            'metode_perhitungan' => $instrumen->metode_perhitungan,
            'target' => $instrumen->target,
            'indikator_penilaian' => $instrumen->indikator_penilaian,
        ]);
    }

    public function edit(SatuanStandar $instrumen)
    {
        if (!$instrumen) {
    ...
    public function update(Request $request, SatuanStandar $instrumen)
    {
        $validator = Validator::make($request->all(), [
            'indikator_instrumen_id' => 'required|exists:indikator_instrumens,id',
            'indikator_instrumen_kriteria_id' => 'required|exists:indikator_instrumen_kriterias,id',
    ...
    public function nonaktifkan(SatuanStandar $instrumen)
    {
        try {
            $instrumen->delete();
            return response()->json([
    ...
    {
        $instrumen = SatuanStandar::withTrashed()->findOrFail($id);
