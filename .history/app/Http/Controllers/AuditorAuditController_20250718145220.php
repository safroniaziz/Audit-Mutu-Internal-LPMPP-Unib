...
class AuditorAuditController extends Controller
{
    ...
    public function laporanAmi(Request $request, PengajuanAmi $pengajuan)
    {
        ...
        // Get Instrumen Prodi data
        $indikatorInstrumens = IndikatorInstrumen::with([
            'kriterias.instrumenProdi.nilaiAuditor' => function ($query) use ($pengajuan) {
                $query->where('pengajuan_ami_id', $pengajuan->id);
            },
            'kriterias.instrumenProdi'
        ])->get();

        // Group by IndikatorInstrumen -> IndikatorInstrumenKriteria -> InstrumenProdi
        $groupedInstrumenProdi = [];
        foreach ($indikatorInstrumens as $instrumen) {
            $indikatorId = $instrumen->id;
            if (!isset($groupedInstrumenProdi[$indikatorId])) {
                $groupedInstrumenProdi[$indikatorId] = [
                    'indikator' => $instrumen,
                    'kriterias' => []
                ];
            }
            ...
            foreach ($instrumen->kriterias as $kriteria) {
                ...
                if (!isset($groupedInstrumenProdi[$indikatorId]['kriterias'][$kriteriaId])) {
                    $groupedInstrumenProdi[$indikatorId]['kriterias'][$kriteriaId] = [
                        'kriteria' => $kriteria,
                        'instrumens' => []
                    ];
                }
                ...
                foreach ($kriteria->instrumenProdi as $instrumenProdi) {
                    ...
                    $groupedInstrumenProdi[$indikatorId]['kriterias'][$kriteriaId]['instrumens'][] = $instrumenProdi;
                }
            }
        }
