...
class PenugasanAuditorController extends Controller
{
    ...
    /**
     * Check if any auditor has started audit activities for a specific pengajuan
     */
    public function checkAuditActivities($pengajuanId)
    {
        try {
            // Check for desk evaluation activities
            $deskEvaluationCount = \App\Models\IkssAuditeeNilai::where('pengajuan_ami_id', $pengajuanId)->count();

            // Check for visitasi activities
            $visitasiCount = \App\Models\IkssAuditeeVisitasi::where('pengajuan_ami_id', $pengajuanId)->count();

            // Check for penilaian instrumen prodi activities
            $penilaianProdiCount = \App\Models\InstrumenProdiNilai::where('pengajuan_ami_id', $pengajuanId)->count();

            $hasAuditActivities = $deskEvaluationCount > 0 || $visitasiCount > 0 || $penilaianProdiCount > 0;

            // Debug log
            \Log::info('Check audit activities for pengajuan ID: ' . $pengajuanId, [
                'desk_evaluation_count' => $deskEvaluationCount,
                'visitasi_count' => $visitasiCount,
                'penilaian_prodi_count' => $penilaianProdiCount,
                'has_audit_activities' => $hasAuditActivities
            ]);
            ...
            return response()->json([
                ...
                'activities' => [
                    ...
                    'penilaian_prodi' => $penilaianProdiCount
                ]
            ]);
        } catch (\Exception $e) {
            \Log::error('Error checking audit activities for pengajuan ID: ' . $pengajuanId, [
                'error' => $e->getMessage()
            ]);
            return response()->json([
                'success' => false,
