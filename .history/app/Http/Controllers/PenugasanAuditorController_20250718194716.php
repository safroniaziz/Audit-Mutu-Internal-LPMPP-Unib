...
class PenugasanAuditorController extends Controller
{
    ...
    public function getFilteredData(Request $request)
    {
        ...
        if ($request->filled('period')) {
            // Get the periode_id based on nomor_surat
            $periode = \App\Models\PeriodeAktif::where('nomor_surat', $request->period)->first();

            if ($periode) {
                $query->where('periode_id', $periode->id);
                Log::info('Filtering by period:', [
                    'nomor_surat' => $request->period,
                    'periode_id' => $periode->id
                ]);
            } else {
                Log::warning('Period not found:', ['nomor_surat' => $request->period]);
            }
        }
