...
class DashboardController extends Controller
{
    ...
    private function getChartData()
    {
        ...
        // Data untuk chart status pengajuan AMI dengan detail audit progress
        $pengajuanStatus = $this->getDetailedPengajuanStatus();
    ...
    private function getDetailedPengajuanStatus()
    {
        // Ambil semua pengajuan AMI yang sudah disetujui
        $approvedPengajuan = PengajuanAmi::where('is_disetujui', 1)->get();

        $belumDisetujui = PengajuanAmi::where('is_disetujui', 0)->count();
        ...
        foreach ($approvedPengajuan as $pengajuan) {
            ...
            if ($totalAssignedAuditors > 0) {
                foreach ($pengajuan->auditors as $penugasan) {
                    ...
                    $hasEvaluasi = \App\Models\EvaluasiSubmission::where('pengajuan_ami_id', $pengajuan->id)
                        ->where('user_id', $auditorId)
                        ->where('jenis', 'auditor')
                        ->exists();

                    $hasMasukan = \App\Models\EvaluasiMasukan::where('pengajuan_ami_id', $pengajuan->id)
                        ->where('user_id', $auditorId)
                        ->exists();
        ...
        return [
            ...
            'sudah_selesai_diaudit' => $sudahSelesaiDiaudit,
            'masih_proses_audit' => $masihProsesAudit
        ];
    }
}
