...
class AuditorAuditController extends Controller
{
    public function daftarAuditee(){
        $kuisioners = Kuisioner::with(['opsis'])->all();
    ...
    public function submitLaporanAmi(Request $request, PengajuanAmi $pengajuan)
    {
        $request->validate([
        ...
        // Generate PDF
        $kuisioners = Kuisioner::whereIn('id', array_keys($request->jawaban))
                              ->with(['opsis', 'jawabans' => function($query) use ($pengajuan) {
                                  $query->where('pengajuan_id', $pengajuan->id);
                              }])
                              ->orderBy('urutan')
                              ->get();
        ...
        $pdf = PDF::loadView('auditor.pdf.laporan-ami', $data);

        // Simpan PDF atau kembalikan untuk diunduh
        return $pdf->download('laporan-ami-'.$pengajuan->id.'.pdf');
    }
}
