...
class AuditeePengajuanAmiController extends Controller
{
    ...
    public function pemilihanIkss()
    {
        ...
        Log::info('Fetching IKSS data with:', [
        ...
        Log::info('Sudah mengisi status:', ['status' => $sudahMengisi]);

        // Get all IkssAuditee records for debugging
        $allRecords = IkssAuditee::where('auditee_id', $unitKerjaId)
                    ->where('periode_id', $periodeAktif->id)
                    ->get();

        Log::info('All IkssAuditee records:', $allRecords->toArray());
        ...
        if ($sudahMengisi) {
            ...
            foreach ($ikssAuditees as $ikssAuditee) {
                $key = 'pilihan_' . $ikssAuditee->instrumen_id;
                $dataTerpilih[$key] = $ikssAuditee->status_target;
                Log::info("Setting dataTerpilih[$key]", [
                    'instrumen_id' => $ikssAuditee->instrumen_id,
                    'status_target' => $ikssAuditee->status_target
                ]);
            }
        }

        // Log the final data being sent to view
        Log::info('Final dataTerpilih array:', $dataTerpilih);

        // Log structure of dataIkssProdi for first SS
        if ($dataIkssProdi->isNotEmpty() && $dataIkssProdi->first()->indikatorKinerjas->isNotEmpty()) {
            $firstSS = $dataIkssProdi->first()->indikatorKinerjas->groupBy('satuan_standar_id')->first();
            Log::info('First SS structure:', [
                'satuan_standar_id' => $firstSS->first()->satuan_standar_id,
                'instrumen_count' => $firstSS->flatMap->instrumen->count(),
                'instrumen_ids' => $firstSS->flatMap->instrumen->pluck('id')->toArray()
            ]);
        }
