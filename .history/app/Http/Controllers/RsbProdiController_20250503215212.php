...
class RsbProdiController extends Controller
{
    ...
    public function getIndikatorBySatuan($satuanId, Request $request)
    {
        ...
        return $unit
        // Ambil semua indikator untuk satuan tertentu
        $indikators = IndikatorKinerja::where('satuan_standar_id', $satuanId);

        if ($unitKerjaId) {
            // Ambil ID indikator yang sudah dimiliki unit kerja ini
            $indikatorTerpasang = DB::table('rsb_prodis')
                ->where('unit_kerja_id', $unitKerjaId)
                ->pluck('indikator_kinerja_id')
                ->toArray();

            // Filter yang belum terpasang
            $indikators = $indikators->whereNotIn('id', $indikatorTerpasang);
        }

        return response()->json($indikators->get());
    }
}
