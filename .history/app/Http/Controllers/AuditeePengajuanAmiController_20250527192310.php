...
class AuditeePengajuanAmiController extends Controller
{
    ...
    public function submitInstrumenProdi(Request $request, $instrumen_id)
    {
        $validator = Validator::make($request->all(), [
        ...
        try {
            ...
            // Get nilai and deskripsi values properly
            $nilai = $request->input('nilai')[$instrumen_id] ?? null;
            $deskripsi = $request->input('deskripsi')[$instrumen_id] ?? null;

            if (!$nilai || !$deskripsi) {
                throw new \Exception('Nilai dan deskripsi harus diisi');
            }

            // Save or update the instrumen submission
            $submission = InstrumenProdiSubmission::updateOrCreate(
                [
                    'indikator_instrumen_id' => $instrumen_id,
