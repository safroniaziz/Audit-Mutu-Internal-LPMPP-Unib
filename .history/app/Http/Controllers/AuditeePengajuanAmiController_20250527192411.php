...
class AuditeePengajuanAmiController extends Controller
{
    ...
    public function pengisianInstrumenProdi()
    {
        ...
        // Get IndikatorInstrumen for this Prodi
        $indikatorInstrumens = IndikatorInstrumen::with(['kriterias.instrumenProdi', 'submission'])
    ...
    public function submitInstrumenProdi(Request $request, $kriteria_id)
    {
        ...
        try {
            ...
            // Get all nilai and deskripsi values
            $nilaiData = $request->input('nilai', []);
            $deskripsiData = $request->input('deskripsi', []);

            // Loop through each instrumen submission
            foreach ($nilaiData as $instrumenId => $nilai) {
                $deskripsi = $deskripsiData[$instrumenId] ?? null;

                if (!$nilai || !$deskripsi) {
                    continue; // Skip if either nilai or deskripsi is missing
                }
                ...
                if ($request->hasFile("dokumen.{$instrumenId}")) {
                    foreach ($request->file("dokumen.{$instrumenId}") as $file) {
                        $path = $file->store('dokumen-instrumen-prodi');
