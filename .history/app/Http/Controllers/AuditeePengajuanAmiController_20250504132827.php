...
class AuditeePengajuanAmiController extends Controller
{
    ...
    public function submitAllInstrumen(Request $request)
    {
        ...
        // Tampilkan hanya 1 error per jenis field
        $errors = $validator->errors();
        $filteredErrors = collect([
            'periode_id' => $errors->first('periode_id'),
            'instrumen_ids' => $errors->first('instrumen_ids'),
            'realisasi' => $errors->first('realisasi.*'),
            'akar_penyebab' => $errors->first('akar_penyebab.*'),
            'rencana_perbaikan' => $errors->first('rencana_perbaikan.*'),
            'bukti_file' => $errors->first('bukti_file.*'),
        ])->filter();

        if ($filteredErrors->isNotEmpty()) {
            return redirect()->back()->withErrors($filteredErrors)->withInput();
        }
