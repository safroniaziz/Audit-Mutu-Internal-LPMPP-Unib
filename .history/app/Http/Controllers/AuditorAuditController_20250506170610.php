...
class AuditorAuditController extends Controller
{
    ...
    public function laporanAmi(Request $request, PengajuanAmi $pengajuan)
    {
        ...
        // Ambil penugasan untuk identifikasi ketua dan anggota tim
        $penugasanAmi = PenugasanAuditor::where('pengajuan_ami_id', $pengajuan->id)->get();
        $ketuaTimId = $penugasanAmi->where('jabatan', 'Ketua Tim')->first()->auditor_id ?? null;
                ...
                foreach ($ikssItems as $ikssItem) {
                    foreach ($ikssItem->nilai as $nilai) {
                        if (!is_null($nilai->nilai)) {
                            ...
                            // Cek apakah nilai ini dari ketua tim atau anggota
                            if ($nilai->auditor_id == $ketuaTimId) {
