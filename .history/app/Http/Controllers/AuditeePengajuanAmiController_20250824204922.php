...
class AuditeePengajuanAmiController extends Controller
{
    ...
    public function pengisianInstrumen()
    {
        ...
        // Hapus filter status_target = 1, gunakan pendekatan yang sama dengan pemilihanIkss
        $currentUnitKerja = UnitKerja::find($unitKerjaId);
        ...
        // Hitung total instrumen (semua instrumen, bukan hanya yang status_target = 1)
        $totalInstrumen = 0;
        foreach ($allIndikatorKinerjas as $indikator) {
            $totalInstrumen += $indikator->instrumen->count();
        }
        ...
        // ambil semua data yang sudah diisi
