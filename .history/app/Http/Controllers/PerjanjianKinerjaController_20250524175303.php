...
class PerjanjianKinerjaController extends Controller
{
    ...
    public function download($id)
    {
        ...
        if ($perjanjianKinerja->auditee_id !== Auth::id()) {
            return redirect()->back()->with('error', 'Anda tidak memilikokei akses untuk mengunduh file ini');
        }

        $filePath = storage_path('app/public/' . $perjanjianKinerja->path);

        if (!file_exists($filePath)) {
            return redirect()->back()->with('error', 'File tidak ditemukan');
        }

        return response()->download($filePath, $perjanjianKinerja->nama_file);
    }
}
