...
class AuditeePengajuanAmiController extends Controller
{
    public function index(){
        ...
        $hasPerjanjianKinerja = PerjanjianKinerja::where('auditee_id', Auth::user()->unit_kerja_id)
            ->where('periode_id', $periodeAktif->id)
            ->exists();

        // Check if user has filled IKSS
        $sudahMengisiIkss = IkssAuditee::where('auditee_id', Auth::user()->unit_kerja_id)
            ->where('periode_id', $periodeAktif->id)
            ->exists();

        // Check if user has filled Instrumen
        $sudahMengisiInstrumen = IkssAuditee::where('auditee_id', Auth::user()->unit_kerja_id)
            ->where('periode_id', $periodeAktif->id)
        ...
        return view('auditee/pengajuan_ami/index',[
            ...
            'hasPerjanjianKinerja' => $hasPerjanjianKinerja,
            'sudahMengisiIkss' => $sudahMengisiIkss,
            'sudahMengisiInstrumen' => $sudahMengisiInstrumen
        ]);
    }
    ...
    public function perjanjianKinerjaIndex()
    {
        ...
        // Check profile completion (25%)
        if ($user->name && $user->email && $user->no_hp && $user->alamat) {
            $completionPercentage += 25;
        }

        // Check Perjanjian Kinerja (25%)
        if ($perjanjianKinerja) {
            $completionPercentage += 25;
        }

        // Check IKSS selection (25%)
        $hasIkss = IkssAuditee::where('auditee_id', Auth::user()->unit_kerja_id)
            ->where('periode_id', $periodeAktif->id)
            ->exists();
        if ($hasIkss) {
            $completionPercentage += 25;
        ...
        // Check Instrumen completion (25%)
        $hasInstrumen = IkssAuditee::where('auditee_id', Auth::user()->unit_kerja_id)
            ->where('periode_id', $periodeAktif->id)
            ->whereNotNull('realisasi')
            ->exists();
        if ($hasInstrumen) {
    ...
    public function perjanjianKinerjaDestroy($id)
    {
        try {
    ...
    public function perjanjianKinerjaDownload($id)
    {
        try {
            $perjanjianKinerja = PerjanjianKinerja::findOrFail($id);
            ...
            return response()->download(
                storage_path('app/public/' . $perjanjianKinerja->file_path),
