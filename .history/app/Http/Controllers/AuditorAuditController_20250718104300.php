...
class AuditorAuditController extends Controller
{
    ...
    private function checkVisitasiTimeValidation($pengajuan)
    {
        ...
        // Define time window (same day, from 6 AM to 6 PM)
        $startTime = $scheduledTime->copy()->startOfDay()->addHours(6); // 06:00
        $endTime = $scheduledTime->copy()->startOfDay()->addHours(18); // 18:00

        if ($currentTime->lt($startTime)) {
            // Too early (before 6 AM)
            return [
                'is_valid' => false,
                'message' => 'Visitasi belum dapat dilakukan. Visitasi dapat dilakukan mulai pukul 06:00.',
            ...
            return [
                'is_valid' => false,
                'message' => 'Waktu visitasi telah berakhir. Visitasi hanya dapat dilakukan hingga pukul 18:00.',
                'scheduled_time' => $scheduledTime->format('d/m/Y H:i'),
                'start_time' => $startTime->format('d/m/Y H:i'),
                'end_time' => $endTime->format('d/m/Y H:i'),
                'type' => 'too_late'
