...
class PenugasanAuditorController extends Controller
{
    ...
    {
        try {
            // Get penugasan with explicit query
            $penugasan = PengajuanAmi::findOrFail($id);

            // Get auditors separately with explicit query
            $auditors = PenugasanAuditor::with('auditor')
                ->where('pengajuan_ami_id', $id)
                ->get();

            Log::info('Raw penugasan data:', [
                'id' => $penugasan->id,
                'auditors_count' => $auditors->count(),
                'auditors' => $auditors->map(function($auditor) {
                    return [
