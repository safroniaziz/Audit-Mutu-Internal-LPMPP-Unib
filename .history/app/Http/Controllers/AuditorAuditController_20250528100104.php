...
class AuditorAuditController extends Controller
{
    ...
    public function submitDeskEvaluation(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'ikss_auditee_ids.*' => 'exists:ikss_auditees,id',
            ...
            'pengajuan_id.required' => 'ID pengajuan harus ada.',
            'pengajuan_id.exists' => 'ID pengajuan tidak valid.',
            'ikss_auditee_ids.required' => 'ID IKSS Auditee harus ada.',
            'ikss_auditee_ids.array' => 'Format ID IKSS Auditee tidak valid.',
            'ikss_auditee_ids.*.exists' => 'ID IKSS Auditee tidak ditemukan.',
            'deskripsi.required' => 'Deskripsi penilaian harus diisi.',
            'deskripsi.array' => 'Format deskripsi tidak valid.',
            'deskripsi.*.required' => 'Deskripsi penilaian wajib diisi.',
            'deskripsi.*.string' => 'Deskripsi penilaian harus berupa teks.',
        ...
        try {
            ...
            $existingEvaluations = IkssAuditeeNilai::where('pengajuan_ami_id', $pengajuanId)
            ...
            foreach ($request->ikss_auditee_ids as $ikssAuditeeId) {
                ...
                if (isset($existingEvaluations[$ikssAuditeeId])) {
                    continue;
                }
            ...
            // Check if all IKSS have been evaluated
            $pengajuan = PengajuanAmi::find($pengajuanId);
            $totalIkss = IkssAuditee::where('auditee_id', $pengajuan->auditee_id)
                ->where('periode_id', $pengajuan->periode_id)
                ->where('status_target', true)
                ->count();

            $evaluatedIkss = IkssAuditeeNilai::where('pengajuan_ami_id', $pengajuanId)
                ->where('auditor_id', $auditorId)
                ->count();
            ...
            return response()->json([
                ...
                'all_completed' => ($evaluatedIkss === $totalIkss),
                'redirect_url' => route('auditor.audit.deskEvaluation', $pengajuanId)
            ]);
