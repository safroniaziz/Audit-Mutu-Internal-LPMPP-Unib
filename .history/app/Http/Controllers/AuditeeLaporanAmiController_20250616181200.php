...
class AuditeeLaporanAmiController extends Controller
{
    public function index(){
        $pengajuan = PengajuanAmi::with(['auditors','auditee','ikssAuditee.nilai'])
            ->withCount(['auditors'])
            ->orderBy('created_at','desc')
            ->first();

        if (!$pengajuan) {
            return redirect()->back()->with('error', 'Tidak ada data pengajuan AMI yang tersedia');
        }
        ...
        $evaluasis = Evaluasi::orderBy('nomor')->get();
