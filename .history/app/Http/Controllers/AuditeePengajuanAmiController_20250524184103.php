...
class AuditeePengajuanAmiController extends Controller
{
    ...
    public function perjanjianKinerja()
    {
        ...
        $perjanjianKinerja = PengajuanAmi::where('auditee_id', $unitKerjaId)
            ->where('periode_id', $periodeAktif->id)
            ->first();
    ...
    public function uploadPerjanjianKinerja(Request $request)
    {
        ...
        try {
            ...
            // Create or update pengajuan record
            $pengajuan = PengajuanAmi::updateOrCreate(
                [
                    'auditee_id' => $unitKerjaId,
                    'periode_id' => $periodeAktif->id,
                ],
                [
                    'file_perjanjian_kinerja' => $path,
                    'waktu_upload_perjanjian' => now(),
                ]
            );
