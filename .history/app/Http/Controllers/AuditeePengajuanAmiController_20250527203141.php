...
class AuditeePengajuanAmiController extends Controller
{
    ...
    public function pengisianInstrumenProdi()
    {
        ...
        $indikatorInstrumens = IndikatorInstrumen::with([
            'kriterias.instrumenProdi' => function($query) {
                $query->with('kriteriaInstrumen.indikatorInstrumen', 'submission.dokumen');
            }
        ])
    ...
    public function submitInstrumenProdi(Request $request, $kriteria_id)
    {
        $validator = Validator::make($request->all(), [
            'instrumen_ids' => 'required|array',
            'instrumen_ids.*' => 'required|exists:indikator_instrumens,id',
            ...
            'dokumen.*' => 'nullable|array',
            'dokumen.*.*' => 'nullable|file|mimes:pdf,doc,docx,xls,xlsx|max:10240', // max 10MB
        ], [
            'instrumen_ids.required' => 'Tidak ada instrumen yang dipilih',
            'instrumen_ids.*.exists' => 'Instrumen tidak valid',
            'nilai.required' => 'Data nilai harus diisi',
            'nilai.*.required' => 'Nilai harus diisi',
            'nilai.*.numeric' => 'Nilai harus berupa angka',
            'nilai.*.min' => 'Nilai minimal 0',
            'nilai.*.max' => 'Nilai maksimal 100',
            'deskripsi.required' => 'Data deskripsi harus diisi',
            'deskripsi.*.required' => 'Deskripsi harus diisi',
            'dokumen.*.*.mimes' => 'Dokumen harus berformat PDF, DOC, DOCX, XLS, atau XLSX',
        ...
        try {
            ...
            \Log::info('Received instrumen_ids:', $request->input('instrumen_ids', []));
            \Log::info('Received nilai:', $request->input('nilai', []));
            \Log::info('Received deskripsi:', $request->input('deskripsi', []));
            return $request->instrumen_ids;
            // Loop through each instrumen_id from the form
            foreach ($request->input('instrumen_ids', []) as $instrumenId) {
                $nilai = $request->input("nilai.{$instrumenId}");
                $deskripsi = $request->input("deskripsi.{$instrumenId}");

                if (!$nilai || !$deskripsi) {
                    throw new \Exception("Nilai dan deskripsi harus diisi untuk instrumen ID: {$instrumenId}");
                }

                // Save or update the instrumen submission
                $submission = InstrumenProdiSubmission::updateOrCreate(
                    [
                        'indikator_instrumen_id' => $instrumenId,
                ...
                if ($request->hasFile("dokumen.{$instrumenId}")) {
                    foreach ($request->file("dokumen.{$instrumenId}") as $file) {
                        $path = $file->store('dokumen-instrumen-prodi', 'public');

                        DokumenInstrumenProdi::create([
