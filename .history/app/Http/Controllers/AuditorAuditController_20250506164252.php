...
class AuditorAuditController extends Controller
{
    ...
    public function laporanAmi(Request $request, PengajuanAmi $pengajuan)
    {
        ...
        // Calculate statistics for each satuan_standar group
        foreach ($groupedBySatuanId as $satuanStandarId => $ikssItems) {
            // Get the satuan_standar details from the first item in the group
            $satuanStandar = $ikssItems->first()->instrumen->indikatorKinerja->satuanStandar;

            // Calculate aggregate scores for this satuan_standar
            $allScores = collect();

            foreach ($ikssItems as $ikssItem) {
                foreach ($ikssItem->nilai as $nilai) {
                    if (!is_null($nilai->nilai)) {
                        $allScores->push((float)$nilai->nilai);
                    }
                }
            }
        ...
        $sortedGrouped = $groupedBySatuanStandar->sortBy('kode_satuan');
