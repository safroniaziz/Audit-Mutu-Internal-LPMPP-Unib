...
class AuditorAuditController extends Controller
{
    ...
    private function isAuditPeriodActive($pengajuan)
    {
        ...
        $auditSchedule = \App\Models\PeriodeAktifJadwal::where('periode_aktif_id', $pengajuan->periode_aktif_id)
            ->where('jenis', 'audit')
            ->first();

        // Debug logging
        \Log::info('Audit Period Check', [
            'pengajuan_id' => $pengajuan->id,
            'periode_aktif_id' => $pengajuan->periode_aktif_id,
            'audit_schedule_found' => $auditSchedule ? true : false,
            'schedule_data' => $auditSchedule ? [
                'waktu_mulai' => $auditSchedule->waktu_mulai,
                'waktu_selesai' => $auditSchedule->waktu_selesai,
                'jenis' => $auditSchedule->jenis
            ] : null
        ]);
        ...
        \Log::info('Time Comparison', [
            ...
            'is_before_start' => $startTime ? $currentTime->lt($startTime) : false,
            'is_after_end' => $endTime ? $currentTime->gt($endTime) : false
        ]);
