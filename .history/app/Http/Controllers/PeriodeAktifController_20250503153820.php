...
class PeriodeAktifController extends Controller
{
    ...
    public function aturJadwal(Request $request)
    {
        try {
            ...
            $range = explode(' s/d ', $validated['tanggal']);
            $waktuMulai = isset($range[0]) ? trim($range[0]) : null;
            $waktuSelesai = isset($range[1]) ? trim($range[1]) : null;
            ...
            if ($existingJadwal) {
                ...
                $jadwal->jenis_jadwal = $validated['jadwal']; // pastikan nama kolom sesuai
