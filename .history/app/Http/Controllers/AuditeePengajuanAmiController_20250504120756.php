...
class AuditeePengajuanAmiController extends Controller
{
    ...
    public function pengisianInstrumen()
    {
        ...
        // Get the selected instrument details with their related indikator
        $selectedInstrumen = InstrumenIkss::whereIn('id', $instrumenIdsWithStatusTarget1)
                    ->where('jenis_auditee', 'prodi')
                    ->first();

        // Get the IKSS data for display
        if ($selectedInstrumen) {
            $indikatorId = $selectedInstrumen->indikator_kinerja_id;

            $dataIkss = IndikatorKinerja::where('id', $indikatorId)->first();
        } else {
            $dataIkss = null;
        }

        // Check if the user has already filled the form for this instrument
        $ikssAuditeeData = null;
        if ($selectedInstrumen) {
            $ikssAuditeeData = IkssAuditee::where('auditee_id', $unitKerjaId)
                        ->where('periode_id', $periodeAktif->id)
                        ->where('instrumen_id', $selectedInstrumen->id)
                        ->first();
        }

        return view('auditee/pengajuan_ami/pengisian_instrumen', [
            'instrumen' => $selectedInstrumen,
            'dataIkss' => $dataIkss,
    ...
    public function saveInstrumen(Request $request)
    {
        // Validasi input
        $validator = Validator::make($request->all(), [
            'auditee_id' => 'required|exists:unit_kerjas,id',
