...
class AuditeeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        UnitKerja::where('id',$request->unit_kerja_id)->update([
            'ttd'   =>  $filePathTtd,
        ]);
    ...
    {
        $auditee = User::withTrashed()->find($auditee->id);
        if (!$auditee) {
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile('ttd')) {
            $folder = "ttd/auditee";
            $file = $request->file('ttd');
            $ttdPath = $file->store($folder, 'public');

            UnitKerja::where('id',$user->unitKerja->id)->update([
                'ttd'   =>  $ttdPath,
            ]);
        }
    ...
    {
        $auditee = User::findOrFail($id);

        // Validasi input password dengan pesan Bahasa Indonesia
        $request->validate([
            'password' => 'required|string|min:8|confirmed',  // pastikan konfirmasi password sesuai
        ], [
            'password.required' => 'Password wajib diisi.',
