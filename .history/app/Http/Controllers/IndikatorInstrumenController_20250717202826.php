...
class IndikatorInstrumenController extends Controller
{
    public function index(){
        ...
        $indikators = IndikatorInstrumen::with(['prodis', 'kriterias.instrumenProdi'])->orderBy('created_at','desc')->withTrashed()->get();

        // Hitung jumlah elemen untuk setiap indikator
        foreach ($indikators as $indikator) {
            $elemenCount = 0;
            foreach ($indikator->kriterias as $kriteria) {
                $elemenCount += $kriteria->instrumenProdi->count();
            }
            $indikator->elemen_count = $elemenCount;
        }
    ...
    public function getElemen($id)
    {
        try {
            $indikator = IndikatorInstrumen::with(['kriterias.instrumenProdi'])->findOrFail($id);

            $elemenData = [];
            foreach ($indikator->kriterias as $kriteria) {
                foreach ($kriteria->instrumenProdi as $instrumen) {
                    $elemenData[] = [
