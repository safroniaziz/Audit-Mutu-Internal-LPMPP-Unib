...
class LaporanHasilAuditController extends Controller
{
    ...
    public function show($id)
    {
        ...
        // Get Kuisioner data for ketua auditor yang aktif saja
        $kuisioners = \App\Models\Kuisioner::with(['opsis'])->get();
        $auditorKuisionerData = [];

        // Ambil penugasan auditor yang aktif untuk pengajuan ini
        $activePenugasan = \App\Models\PenugasanAuditor::where('pengajuan_ami_id', $id)
            ->where('role', 'ketua')
            ->first();

        if ($activePenugasan) {
            // Coba ambil jawaban kuisioner dari ketua auditor yang aktif
            $allKuisionerJawaban = \App\Models\KuisionerJawaban::with(['kuisioner', 'opsi'])
                ->where('pengajuan_id', $id)
                ->where('penugasan_auditor_id', $activePenugasan->id)
                ->get();

            // Jika tidak ada kuisioner dari penugasan aktif, cari dari penugasan soft deleted dengan user yang sama
            if ($allKuisionerJawaban->count() == 0) {
                $deletedPenugasan = \App\Models\PenugasanAuditor::withTrashed()
                    ->where('pengajuan_ami_id', $id)
                    ->where('role', 'ketua')
                    ->where('user_id', $activePenugasan->user_id)
                ...
                if ($deletedPenugasan) {
                    $allKuisionerJawaban = \App\Models\KuisionerJawaban::with(['kuisioner', 'opsi'])
                        ->where('pengajuan_id', $id)
                        ->where('penugasan_auditor_id', $deletedPenugasan->id)
                        ->get();
                }
            }
