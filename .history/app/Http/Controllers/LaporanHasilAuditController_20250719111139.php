...
class LaporanHasilAuditController extends Controller
{
    ...
    public function show($id)
    {
        ...
        // Ambil semua jawaban kuisioner untuk pengajuan ini
        $allKuisionerJawaban = \App\Models\KuisionerJawaban::with(['kuisioner', 'opsi', 'penugasan.auditor'])
            ->where('pengajuan_id', $id)
            ->get();

        // Debug: log data kuisioner
        \Log::info('Kuisioner data for pengajuan ' . $id, [
            'total_jawaban' => $allKuisionerJawaban->count(),
            'jawaban_details' => $allKuisionerJawaban->map(function($jawaban) {
                return [
                    'kuisioner_id' => $jawaban->kuisioner_id,
                    'kuisioner_pertanyaan' => $jawaban->kuisioner->pertanyaan ?? 'N/A',
                    'opsi_jawaban' => $jawaban->opsi->opsi ?? 'N/A',
                    'penugasan_auditor_id' => $jawaban->penugasan_auditor_id,
                    'auditor_name' => $jawaban->penugasan->auditor->name ?? 'N/A',
                    'auditor_role' => $jawaban->penugasan->role ?? 'N/A',
                    'created_at' => $jawaban->created_at ?? 'N/A'
                ];
            })
        ]);
        ...
        foreach ($allKuisionerJawaban as $jawaban) {
            if ($jawaban->penugasan && $jawaban->penugasan->auditor) {
                $auditorId = $jawaban->penugasan->user_id;
                $auditorName = $jawaban->penugasan->auditor->name;
                $auditorRole = $jawaban->penugasan->role;
