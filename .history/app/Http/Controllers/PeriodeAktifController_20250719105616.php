...
class PeriodeAktifController extends Controller
{
    ...
    {
        // Gunakan withTrashed() untuk mencari termasuk yang sudah di-soft delete
        $periodeAktif = PeriodeAktif::withTrashed()->find($id);
        ...
        try {
            ...
            if (isset($range[0])) {
                ...
                if (preg_match('/^(\d{2})\/(\d{2})\/(\d{4})$/', $startDate, $matches)) {
                    $waktuMulai = $matches[3] . '-' . $matches[2] . '-' . $matches[1] . ' 00:00:00';
                }
            }
            ...
            if (isset($range[1])) {
                ...
                if (preg_match('/^(\d{2})\/(\d{2})\/(\d{4})$/', $endDate, $matches)) {
                    $waktuSelesai = $matches[3] . '-' . $matches[2] . '-' . $matches[1] . ' 23:59:59';
                }
            }
    ...
    public function getJadwalData(Request $request)
    {
        try {
            ...
            $jadwal = PeriodeAktifJadwal::where('periode_aktif_id', $validated['periode_id'])
                        ->where('jenis', $validated['jenis'])
                        ->first();

            if ($jadwal) {
                // Pastikan format tanggal yang dikembalikan benar
                $jadwal->waktu_mulai = $jadwal->waktu_mulai ? date('Y-m-d H:i:s', strtotime($jadwal->waktu_mulai)) : null;
                $jadwal->waktu_selesai = $jadwal->waktu_selesai ? date('Y-m-d H:i:s', strtotime($jadwal->waktu_selesai)) : null;
