...
class LaporanHasilAuditController extends Controller
{
    ...
    public function show($id)
    {
        ...
        // Get Evaluasi data for each Auditor
        ...
        // Get Kuisioner data for each Auditor
        $kuisioners = \App\Models\Kuisioner::with(['opsis'])->get();
        $auditorKuisionerData = [];

        foreach ($pengajuanAmis->auditors as $auditor) {
            $penugasanAuditor = \App\Models\PenugasanAuditor::where('pengajuan_ami_id', $id)
                ->where('user_id', $auditor->user_id)
                ->first();

            if ($penugasanAuditor) {
                $kuisionerJawaban = \App\Models\KuisionerJawaban::with(['kuisioner', 'opsi'])
                    ->where('pengajuan_id', $id)
                    ->where('penugasan_auditor_id', $penugasanAuditor->id)
                    ->get()
                    ->keyBy('kuisioner_id');
                ...
                $auditorKuisionerData[] = [
                    'auditor' => $auditor->auditor,
                    'role' => $auditor->role,
                    'kuisioner_jawaban' => $kuisionerJawaban
                ];
            }
        }
