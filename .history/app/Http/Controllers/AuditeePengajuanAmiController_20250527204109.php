...
class AuditeePengajuanAmiController extends Controller
{
    ...
    public function submitInstrumenProdi(Request $request, $kriteria_id)
    {
        try {
            ...
            Log::info('Received instrumen_ids:', $instrumenIds);
            Log::info('Received nilai:', $request->input('nilai', []));
            Log::info('Received deskripsi:', $request->input('deskripsi', []));

            // Validate that we have all required instrumen IDs
            if (empty($instrumenIds)) {
                throw new \Exception('Tidak ada instrumen yang dipilih');
            }

            // Get all InstrumenProdi records for the given kriteria and instrumen IDs
            $instrumenProdis = InstrumenProdi::whereIn('indikator_instrumen_kriteria_id', $instrumenIds)
                ->get();

            // Loop through each instrumen_id from the form
            foreach ($instrumenProdis as $instrumenProdi) {
                $instrumenId = $instrumenProdi->kriteriaInstrumen->indikator_instrumen_id;
                $nilai = $request->input("nilai.{$instrumenId}");
                $deskripsi = $request->input("deskripsi.{$instrumenId}");

                // Validate required fields
                if (!$nilai || !$deskripsi) {
                    throw new \Exception("Nilai dan deskripsi harus diisi untuk semua instrumen");
                }
