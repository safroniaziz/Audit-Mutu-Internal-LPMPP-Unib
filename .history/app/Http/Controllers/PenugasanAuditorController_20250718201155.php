...
class PenugasanAuditorController extends Controller
{
    ...
    public function getExistingAssignments($id)
    {
        try {
            ...
            // Debug: Check all roles in database for this penugasan
            $allRoles = \App\Models\PenugasanAuditor::where('pengajuan_ami_id', $id)
                ->select('user_id', 'role')
                ->get()
                ->map(function($item) {
                    return [
            ...
            Log::info('All roles in database for this penugasan:', $allRoles);
