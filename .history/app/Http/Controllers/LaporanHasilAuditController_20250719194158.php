...
class LaporanHasilAuditController extends Controller
{
    ...
    public function show($id)
    {
        ...
        // Get Kuisioner data for all auditors
        $kuisioners = \App\Models\Kuisioner::with(['opsis'])->get();
        $auditorKuisionerData = [];

        // Ambil semua penugasan auditor untuk pengajuan ini
        $allPenugasan = \App\Models\PenugasanAuditor::where('pengajuan_ami_id', $id)
            ->with('auditor')
            ->get();

        foreach ($allPenugasan as $penugasan) {
            // Ambil jawaban kuisioner untuk setiap auditor
            $allKuisionerJawaban = \App\Models\KuisionerJawaban::with(['kuisioner', 'opsi'])
                ->where('pengajuan_id', $id)
                ->where('penugasan_auditor_id', $penugasan->id)
                ->get();

            if ($allKuisionerJawaban->count() > 0) {
                $auditorKuisionerData[] = [
                    'auditor_id' => $penugasan->user_id,
                    'auditor_name' => $penugasan->auditor->name,
                    'role' => $penugasan->role,
