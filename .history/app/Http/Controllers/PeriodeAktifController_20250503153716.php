...
class PeriodeAktifController extends Controller
{
    ...
    public function aturJadwal(Request $request)
    {
        try {
            ...
            $existingJadwal = PeriodeAktifJadwal::where('periode_id', $validated['periode_id'])
                                    ->where('jenis_jadwal', $validated['jadwal'])
                                    ->first();

            if ($existingJadwal) {
                // Update jadwal yang sudah ada
                $existingJadwal->tanggal = $validated['tanggal'];
                $existingJadwal->save();
            } else {
                ...
                $jadwal->waktu_mulai = $validated['tanggal'];
                $jadwal->waktu_selesai = $validated['tanggal'];
                $jadwal->save();
            }
