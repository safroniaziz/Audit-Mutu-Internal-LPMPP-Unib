...
class InstrumenIkssController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'jenjang.in' => 'Jenjang harus berupa D3, D4, S1, S2, S3, Profesi, atau Semua.',
        ]);
        ...
        $instrumenIkss = InstrumenIkss::create([
    ...
    {
        $instrumen = InstrumenIkss::with('indikatorKinerja')->findOrFail($id);
        $indikatorKinerja = $instrumen->indikatorKinerja;

        return response()->json([
            'tujuan'         => $indikatorKinerja->tujuan ?? null,
            'indikator'     => $instrumen->indikator,
            'satuan'     => $instrumen->satuan,
            'pertanyaan' => $instrumen->pertanyaan,
            'target' => $instrumen->target,
            'sumber'     => $instrumen->sumber,
            'uraian'     => $instrumen->uraian,
            'penilaian'  => $instrumen->penilaian,
            'jenis_auditee'  => $instrumen->jenis_auditee,
            'is_wajib'  => $instrumen->is_wajib,
            'jenjang'  => $instrumen->jenjang,
        ]);
    }


    public function edit(InstrumenIkss $instrumenIkss)
    {
        if (!$instrumenIkss) {
    ...
    public function update(Request $request, InstrumenIkss $instrumenIkss)
    {
        $validator = Validator::make($request->all(), [
            'indikator_kinerja_id' => 'required|exists:indikator_kinerjas,id',
            'indikator' => 'nullable|string',
        ...
        $instrumenIkss->jenjang = $request->jenjang;
        $instrumenIkss->save();
    ...
    public function nonaktifkan(InstrumenIkss $instrumenIkss)
    {
        try {
    ...
    public function nonaktifkanSelected(Request $request)
    {
        try {
            $ids = $request->ids;
            InstrumenIkss::whereIn('id', $ids)->delete();
    ...
    {
        $instrumenIkss = InstrumenIkss::withTrashed()->findOrFail($id);
