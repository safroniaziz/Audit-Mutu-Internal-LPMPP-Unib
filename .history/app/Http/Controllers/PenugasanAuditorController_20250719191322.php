...
class PenugasanAuditorController extends Controller
{
    ...
    public function updatePenugasanAuditor(Request $request)
    {
        ...
        try {
            ...
            // Get existing assignments to preserve approval status
            ...
            // Create new assignments with preserved approval status
            $penugasanData = [
                ['auditor_id' => $request->auditor1, 'role' => 'ketua'],
                ['auditor_id' => $request->auditor2, 'role' => 'pendamping'],
            ];
            ...
            foreach ($penugasanData as $data) {
                ...
                PenugasanAuditor::create([
                    ...
                    'user_id'          => $auditorId,
                    'role'             => $data['role'],
                    // Preserve approval status if auditor is still assigned
                    'is_setuju' => $existingAssignment ? $existingAssignment->is_setuju : false,
                    'is_setuju_visitasi' => $existingAssignment ? $existingAssignment->is_setuju_visitasi : false,
                    'is_setuju_indikator_prodi' => $existingAssignment ? $existingAssignment->is_setuju_indikator_prodi : false,
                ]);
            }
