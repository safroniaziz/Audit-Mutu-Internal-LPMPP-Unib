...
class DashboardController extends Controller
{
    ...
    private function getRecentActivities()
    {
        ...
        $recentPengajuan = PengajuanAmi::with('auditee')
            ...
            ->map(function ($item) {
                ...
                return [
                    ...
                    'description' => 'Unit Kerja: ' . ($item->auditee->nama_unit_kerja ?? 'Tidak diketahui'),
                    'time' => $item->created_at ? $item->created_at->diffForHumans() : '-',
        ...
        $recentEvaluasi = Evaluasi::latest()
            ...
            ->get()
            ->map(function ($item) {
                $jenisText = ucfirst($item->jenis_evaluasi ?? 'Tidak diketahui');
                return [
                    'type' => 'evaluasi',
                    'title' => 'Evaluasi ' . $jenisText . ' Selesai',
                    'description' => 'Evaluasi ' . $jenisText . ' telah diselesaikan dan dicatat dalam sistem',
                    'time' => $item->created_at ? $item->created_at->diffForHumans() : '-',
