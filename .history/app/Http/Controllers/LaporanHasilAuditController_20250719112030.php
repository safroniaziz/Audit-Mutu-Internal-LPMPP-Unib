...
class LaporanHasilAuditController extends Controller
{
    public function index(){
        ...
        $query = PengajuanAmi::with(['auditors','auditee','ikssAuditee.nilai','periodeAktif'])
            ->withCount(['auditors'])
    ...
    public function show($id)
    {
        ...
        // Get Kuisioner data for all auditors (tampilkan semua yang mengisi)
        $kuisioners = \App\Models\Kuisioner::with(['opsis'])->get();
        $auditorKuisionerData = [];

        // Debug: Cek data kuisioner yang ada
        $debugKuisioner = \App\Models\KuisionerJawaban::where('pengajuan_id', $id)->get();
        \Illuminate\Support\Facades\Log::info('Debug Kuisioner Data', [
            'pengajuan_id' => $id,
            'total_kuisioner_jawaban' => $debugKuisioner->count(),
            'data' => $debugKuisioner->map(function($item) {
                return [
                    'id' => $item->id,
                    'pengajuan_id' => $item->pengajuan_id,
                    'penugasan_auditor_id' => $item->penugasan_auditor_id,
                    'kuisioner_id' => $item->kuisioner_id,
                    'kuisioner_opsi_id' => $item->kuisioner_opsi_id,
                    'created_at' => $item->created_at
                ];
            })
        ]);

        // Cek penugasan auditor untuk pengajuan ini (termasuk yang soft deleted)
        $penugasanAuditors = \App\Models\PenugasanAuditor::withTrashed()->where('pengajuan_ami_id', $id)->get();
        \Illuminate\Support\Facades\Log::info('Debug Penugasan Auditor', [
            'pengajuan_id' => $id,
            'total_penugasan' => $penugasanAuditors->count(),
            'penugasan_data' => $penugasanAuditors->map(function($item) {
                return [
                    'id' => $item->id,
                    'pengajuan_ami_id' => $item->pengajuan_ami_id,
        ...
        // Ambil semua jawaban kuisioner untuk pengajuan ini dengan relasi yang benar
        $allKuisionerJawaban = \App\Models\KuisionerJawaban::with(['kuisioner', 'opsi'])
            ->where('pengajuan_id', $id)
            ->get();

        // Group jawaban by auditor
        foreach ($allKuisionerJawaban as $jawaban) {
            // Cari penugasan auditor berdasarkan penugasan_auditor_id (termasuk yang soft deleted)
            $penugasan = \App\Models\PenugasanAuditor::withTrashed()->with('auditor')
                ->where('id', $jawaban->penugasan_auditor_id)
                ->first();

            if ($penugasan && $penugasan->auditor) {
                $auditorId = $penugasan->user_id;
                $auditorName = $penugasan->auditor->name;
                $auditorRole = $penugasan->role;

                // Cek apakah auditor sudah ada di array
                $existingIndex = null;
                foreach ($auditorKuisionerData as $index => $data) {
                    if ($data['auditor_id'] == $auditorId) {
                        $existingIndex = $index;
                        break;
                    }
                }
                ...
                if ($existingIndex !== null) {
                    // Tambahkan jawaban ke auditor yang sudah ada
                    $auditorKuisionerData[$existingIndex]['kuisioner_jawaban'][$jawaban->kuisioner_id] = $jawaban;
                } else {
                    // Buat data baru untuk auditor
                    $auditorKuisionerData[] = [
                        'auditor_id' => $auditorId,
                        'auditor_name' => $auditorName,
                        'role' => $auditorRole,
                        'created_at' => $jawaban->created_at,
                        'kuisioner_jawaban' => [$jawaban->kuisioner_id => $jawaban]
                    ];
                }
            }
        }
